...
//
//   node bin/promptset.mjs --promptset <f> --flow <events.json> --expected <frozen.json> --profile <f> --sut <base>
...
function main() {
  ...
  const need = ['promptset', 'flow', 'expected', 'profile', 'sut'];
  const missing = need.filter((k) => !opts[k] || opts[k] === true);
  if (missing.length) {
    console.error(col(C.red, `[promptset] 缺必填参 → 用参错误(64)：${missing.map((m) => '--' + m).join(' ')}`));
    console.error('用法: node bin/promptset.mjs --promptset <f> --flow <events.json> --expected <frozen.json> --profile <f> --sut <本地基址>');
