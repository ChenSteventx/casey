...
import { expandLearnedFlow, mergeAtomRegistries } from '../lib/capture-atom-promotion.mjs';
import { PROJECT_ROOT } from '../lib/paths.mjs';

const { chromium } = pw;
const SNAPSHOT_FILE = join(PROJECT_ROOT, 'lib', 'atoms-registry.snapshot.json');
const LEARNED_FILE = join(PROJECT_ROOT, 'lib', 'learned-atoms.registry.json');
...
function loadRegistry() {
  const base = readJson(SNAPSHOT_FILE, '原子注册表快照');
  const learned = readJson(LEARNED_FILE, '学习原子注册表');
  const merged = mergeAtomRegistries(base, learned);
  if (!merged.ok) {
    console.error(`compile: 学习原子注册表未过合并门（${merged.problems.slice(0, 4).join(' / ')}）`);
    process.exit(65);
  }
  return merged.registry;
}

function validateAndExpand(flow, { prefix, registry, initialStates }) {
  const raw = validateDraft(flow, { prefix, registry, initialStates });
  if (!raw.ok) return { ok: false, problems: raw.problems, flow: null };
  const expanded = expandLearnedFlow(flow, registry);
  if (!expanded.ok) return { ok: false, problems: expanded.problems.map((p) => `学习宏展开：${p}`), flow: null };
  const after = validateDraft(expanded.flow, { prefix, registry, initialStates });
  if (!after.ok) return { ok: false, problems: after.problems, flow: null };
  return { ok: true, problems: [], flow: expanded.flow };
}
...
function gateMode(caseId, args) {
  ...
  const { ok, problems } = validateAndExpand(flow, { prefix: tc.uniquePrefix, registry, initialStates: tc.preconditions || [] });
  if (!ok) {
  ...
  const doc = {
    ...
    gate: { ok: true, atomsFrom: ['lib/atoms-registry.snapshot.json', 'lib/learned-atoms.registry.json'] },
...
async function executeMode(caseId, args) {
  ...
  {
    const registry = loadRegistry();
    const re = validateAndExpand(flowDoc.flow, { prefix: tc.uniquePrefix, registry, initialStates: tc.preconditions || [] });
    if (!re.ok) {
      console.error(`compile: 执行前重验闸未过（${re.problems.length} 问题，疑 confirm 后被改）：`);
      for (const p of re.problems) console.error(`  - ${p}`);
      process.exit(65);
    }
    executableFlow = re.flow;
  }
  ...
  try {
    ...
    await compileFlow(run, executableFlow);
  } catch (e) {
