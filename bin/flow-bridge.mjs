...
import { mergeAtomRegistries } from '../lib/capture-atom-promotion.mjs';
import { credentialGate } from '../lib/cred-gate.mjs';

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const SNAPSHOT_FILE = join(ROOT, 'lib', 'atoms-registry.snapshot.json');
const LEARNED_FILE = join(ROOT, 'lib', 'learned-atoms.registry.json');
...
const baseRegistry = readJson(SNAPSHOT_FILE, '原子注册表快照');
const learnedRegistry = readJson(LEARNED_FILE, '学习原子注册表');
const merged = mergeAtomRegistries(baseRegistry, learnedRegistry);
if (!merged.ok) die(65, `学习原子注册表未过合并门（${merged.problems.slice(0, 4).join(' / ')}）`);
...
const flow = buildFlow(testcase, mapping, { registry });
