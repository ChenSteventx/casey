...
| 候选原子 | Candidate Atom | 已入账示教流程经真实相 3 回放与相 4 确定性全 PASS 后产生的 pending 宏原子提议；只由已可编译原子组成，LLM 只能提议语义与参数化，未人签与晋升前不进正式注册表 | 录制直通原子 |
| 原子晋升 | Atom Promotion | 把绑定真实 PASS 证据哈希的 候选原子 经显式人签后追加到学习原子注册表；按稳定指纹去重、保留 revision 历史、禁止覆盖内置原子，不自动发生 | 自动学习直通 |
