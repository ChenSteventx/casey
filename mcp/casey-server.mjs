...
export const TOOLS = [
  ...
  {
    name: 'casey_cef_record',
    description: '医生站/Hi 小助 CEF 真人示教：经回环 CDP 连接已登录真实窗口，F8 收口，产 teach-in-capture + screencast。须先由人确认专用测试账户且窗口无真实患者数据；只作蒸馏语料，不产 verdict。',
    inputSchema: { type: 'object', required: ['caseId', 'profile', 'outDir', 'safeSession'], properties: { caseId: { type: 'string' }, profile: { type: 'string' }, outDir: { type: 'string' }, safeSession: { type: 'boolean' }, maxMs: { type: 'string' } } },
    toArgs: (a) => ['cef-record', ...(a.caseId ? [a.caseId] : []), ...flag('profile', a.profile), ...flag('out-dir', a.outDir), ...boolFlag('safe-session', a.safeSession), ...flag('max-ms', a.maxMs)],
  },
  {
    name: 'casey_intake',
  ...
  },
  {
    ...
    description: '已入账 CEF 真人示教包的确定性机械回放 + screencast。只产 formalVerdictEligible=false 的动作收据与视觉复核请求；没有已签断言/三轴，不是正式 PASS。',
    inputSchema: { type: 'object', required: ['caseId', 'capture', 'profile', 'outDir', 'safeSession'], properties: { caseId: { type: 'string' }, capture: { type: 'string' }, profile: { type: 'string' }, outDir: { type: 'string' }, safeSession: { type: 'boolean' } } },
    toArgs: (a) => ['cef-replay', ...(a.caseId ? [a.caseId] : []), ...flag('capture', a.capture), ...flag('profile', a.profile), ...flag('out-dir', a.outDir), ...boolFlag('safe-session', a.safeSession)],
  },
  {
    name: 'casey_verdict',
