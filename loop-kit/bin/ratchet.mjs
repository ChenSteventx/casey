#!/usr/bin/env node
// loop-kit/bin/ratchet.mjs —— 薄转发层（shim，由单一模板生成，勿手改；真实现在独立包 loop-kit，兄弟目录
...
export let normalizeRepoPath;
export let buildRatchetIndex;
export let verifyRatchet;
export let findAffectedPrds;
...
if (isMain) {
  if (__bootError) {
    ...
    try {
      process.exitCode = __boot.runCli({ script: "ratchet.mjs", kind: "cli" });
    } catch (e) {
...
if (!isMain) {
  ...
  const __lib = await __boot.loadLib({ script: "ratchet.mjs" });
  normalizeRepoPath = __lib.normalizeRepoPath;
  buildRatchetIndex = __lib.buildRatchetIndex;
  verifyRatchet = __lib.verifyRatchet;
  findAffectedPrds = __lib.findAffectedPrds;
}
