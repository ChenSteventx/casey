#!/usr/bin/env node
// loop-kit/bin/hook-loop-guard.mjs —— 薄转发层（shim，由单一模板生成，勿手改；真实现在独立包 loop-kit，兄弟目录
...
if (isMain) {
  if (__bootError) {
    process.exitCode = __fallbackDegrade("guard", __bootError);
  } else {
    try {
      process.exitCode = __boot.runCli({ script: "hook-loop-guard.mjs", kind: "guard" });
    } catch (e) {
      process.exitCode = __fallbackDegrade("guard", e);
    }
  }
}
