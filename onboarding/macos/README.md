...
agent 会识别芯片架构、补齐运行时、安装 Casey 与 Chromium、接入真实支持的 skill / 指令入口和 MCP。开始前请对照 [`../PERMISSIONS.md`](../PERMISSIONS.md) 核对最小权限；agent 不得索要管理员、全盘访问或 Docker，系统前置若确需批准应报告并交给你本人处理。
