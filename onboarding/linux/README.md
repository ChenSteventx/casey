...
Linux 支持源码安装、Chromium 和零 SUT 检验。真实 web 回放还要求部署方提供组织批准的本地回环代理；本仓没有通用 Linux 真站代理安装器。

## 首选：把安装提示词交给 agent

在 Linux 中启动对应 agent，并复制提示词全文：
...
使用发行版或组织批准的软件源安装 Git、Node.js 22.12 或更高版本、fontconfig 与中文字体。不要运行来源不明的 `curl | shell`。然后：
...
字体查询没有结果时安装 Noto Sans CJK 后复查。零 SUT 检验全部 PASS 后仍显示 `REAL_SUT NOT_VERIFIED`；它没有连接任何被测系统。

## 人工接入 agent

Claude Code 从仓根打开并读取 `.claude/skills/casey/SKILL.md`，模型使用 Fable + high。Codex 先读 `AGENTS.md`，按当前官方能力安装仓库 plugin，或通过官方 skill installer 安装 `skills/casey` 薄路由。MCP 从当前 Linux 运行面生成：
...
只执行对应 agent 的一条并安全合并。其它 agent 先读 `AGENTS.md`，确认官方 stdio MCP 格式，再将 Codex 输出的实际 command / args 转换为该格式；只有刷新后实际枚举到 Casey 工具才算成功。
...
隐藏输入不能工作时不要把秘密改贴进聊天。账户留在 gitignored 的 `.auth/`，目录尽量为 `0700`、文件为 `0600`，且不要放共享目录。真目标只写 gitignored 的 `site.json`，命令和日志只使用回环基址。

`node bin/casey.mjs doctor` 主要检查本机运行面，不能证明真站可达。正式回放前，部署方必须提供本地回环代理，并取得 Linux 网络侧到真站、经 `target.devProxyUrl` 到真站的真实 HTTP 证据。做不到时只能停在“本机安装完成”。
