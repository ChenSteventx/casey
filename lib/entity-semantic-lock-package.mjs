...
import { snapshotClosedOptions, snapshotPlainOwnData } from './safe-own-data.mjs';
...
function bytesOf(value, label) {
  if ((typeof value === 'object' || typeof value === 'function') && value !== null && isProxy(value)) {
    throw new TypeError('UNSAFE_DATA_SHAPE');
  }
...
function inspectSidecar(sidecarBytes, caseId) {
  ...
  // `identity-observations` 是 wiring 冻结契约的通用旁车；`teach-in-identity-observations` 是
  // observation sidecar 的真实生产形状。package 层兼容两者，只做完整字节/元数据绑定；后者的闭合
  // schema 与敏感门由 verifyAcceptedIdentityObservationBundle 再验证，不能由 package authority 越权。
  if (!['identity-observations', 'teach-in-identity-observations'].includes(doc.artifactKind) || doc.caseId !== caseId) {
    return { ok: false, reason: 'SIDECAR_IDENTITY_MISMATCH' };
  }
...
export function verifiedTeachInPackageAuthorityFacts(handle) {
  if ((typeof handle === 'object' || typeof handle === 'function') && handle !== null && isProxy(handle)) {
    return Object.freeze({ ok: false, reason: 'VERIFIED_PACKAGE_AUTHORITY_INVALID' });
  }
...
export function buildTeachInPackageManifest(options = {}) {
  const {
    caseId,
    captureBytes,
    sidecarBytes,
    ...
    'caseId', 'captureBytes', 'sidecarBytes', 'observationCount', 'observationSchemaVersion',
  ]));
  ...
  // capture v2 合法地单向引用 sidecar hash；这形成 DAG，不形成哈希环。只禁止 sidecar 反向引用
  // 已包含自身 hash 的 capture hash，否则会构成无法稳定生成的互锁环。
  if (sidecar.includes(Buffer.from(captureHash))) {
...
export function serializeTeachInPackageManifest(manifest) {
  const snapshot = snapshotPlainOwnData(manifest);
  const checked = validateManifestShape(snapshot, snapshot?.caseId);
  if (!checked.ok) throw new TypeError(checked.reason);
  return JSON.stringify(canonicalManifest(snapshot), null, 2) + '\n';
}

export function verifyTeachInPackage(options = {}) {
  let input;
  try {
    input = snapshotClosedOptions(options, new Set(['caseId', 'captureBytes', 'sidecarBytes', 'manifestBytes']));
  } catch {
    return denied('UNSAFE_DATA_SHAPE');
  }
  const { caseId, captureBytes, sidecarBytes, manifestBytes } = input;
  ...
  if (sidecar.includes(Buffer.from(captureSha256))) return denied('CROSS_ARTIFACT_HASH_REFERENCE');
...
export function deriveTeachInPackagePaths(options = {}) {
  const { capturePath } = snapshotClosedOptions(options, new Set(['capturePath']));
