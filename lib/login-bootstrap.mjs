...
// strict（codex R1-F2）：文件在场但坏 JSON/不可读时抛错（报错不回显内容，护栏 #7）而非静默回落默认——
// 登录预备动作等 fail-closed 消费者必须开；缺文件仍回默认（site.json 本就可选）。
export function loadSiteConfig(sitePath = process.env.AT_SITE_JSON || path.join(PROJECT_ROOT, 'site.json'), { strict = false } = {}) {
  try {
    ...
    // 坏 JSON：非 strict 退回默认（既有语义）；strict 抛消毒错——绝不带 JSON.parse 的内容片段。
    if (strict) throw new Error(`站点配置 ${sitePath} 不是合法 JSON/不可读（内容不回显，护栏 #7）；fail-closed 拒绝静默回落默认`);
  }
  return DEFAULT_SITE;
}

// 凭据读取：env 覆盖优先（AT_CREDS_USER/AT_CREDS_PASS，只进内存不落盘）；否则读 credsFile
// （默认 .auth/credentials.json，可经 AT_CREDS_FILE env 换路径——与 AT_SITE_JSON 同范式；
// hermetic 测试靠它隔离凭据源，绝不误读仓里真 .auth/）。
// 缺文件/缺字段一律抛错 fail-closed——绝不静默无凭据继续。
export function loadCreds({ credsFile = process.env.AT_CREDS_FILE || CREDS_FILE } = {}) {
  ...
  let c;
  try { c = JSON.parse(fs.readFileSync(credsFile, 'utf8')); }
  catch {
    // codex R1-F1：V8 的 JSON.parse 报错自带内容片段（实测形如 …"pass":p_fake_log"…）——
    // 原样上抛会把凭据字节漏进 stderr/日志。此处消毒重抛，绝不引用文件内容（护栏 #7）。
    throw new Error(`${credsFile} 不是合法 JSON/不可读（凭据文件内容不回显，护栏 #7）`);
  }
...
export async function loginBootstrap(page, {
  site,
  creds,
  startUrl,
  executionTargetAuthority,
  // 预算是天花板不是等待（login-nav-budget）：登录页现役经隧道 load 实测 13.8-14.9s，15s 骑线必抖——
  // 上限 30s 只给慢页面跑道，快路径零行为差；再逼近 30s 属环境恶化另议，不无限抬。
  timeoutMs = 30000,
}) {
  ...
  const firstNavigation = await navigateExecutionTargetPage({
    page,
    authority: targetAuthority,
    targetUrl: startUrl,
    gotoOptions: { waitUntil: 'load', timeout: timeoutMs },
  });
  if (!firstNavigation.ok) return firstNavigation;
  ...
  const afterFormProbe = await verifyExecutionTargetPageOrigin({
    page,
    authority: targetAuthority,
  });
  if (!afterFormProbe.ok) return afterFormProbe;
  if (!formVisible) return { loggedIn: true, viaForm: false };
  const beforeUserFill = await verifyExecutionTargetPageOrigin({
    page,
    authority: targetAuthority,
  });
  if (!beforeUserFill.ok) return beforeUserFill;
  await userBox.fill(creds.user, { timeout: timeoutMs });
  const beforePassFill = await verifyExecutionTargetPageOrigin({
    page,
    authority: targetAuthority,
  });
  if (!beforePassFill.ok) return beforePassFill;
  await page.getByRole(login.pass.role, { name: login.pass.name })
    .fill(creds.pass, { timeout: timeoutMs });
  const beforeSubmit = await verifyExecutionTargetPageOrigin({
    page,
    authority: targetAuthority,
  });
  if (!beforeSubmit.ok) return beforeSubmit;
  await page.getByRole(login.submit.role, { name: login.submit.name })
  ...
  const submittedOrigin = await verifyExecutionTargetPageOrigin({
    page,
    authority: targetAuthority,
  });
  if (!submittedOrigin.ok) return submittedOrigin;
