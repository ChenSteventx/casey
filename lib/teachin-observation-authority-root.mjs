...
function commitAcceptedTransaction({ canonical, caseId, initial, receipt, captureReview }) {
  ...
  try {
    ...
    if (!currentTemp.isFile() || currentTemp.isSymbolicLink()
      || statIdentity(currentTemp) !== tempIdentity) throw new TypeError('LEDGER_TEMP_IDENTITY_CHANGED');
    ...
    return Object.freeze({
      ...
      ledgerIdentity: statIdentity(tempStat),
      ledgerSha256: sha256(completeBytes),
    });
  } finally {
    if (tempFd !== undefined) {
