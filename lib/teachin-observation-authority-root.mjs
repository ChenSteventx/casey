...
import { reviewClosedIdentityObservationPackage } from './teachin-identity-package-validator.mjs';
...
const CONTROL_RE = /[\u0000-\u001f\u007f-\u009f]/;
const SIGNED_V1_PAYLOAD_KEYS = Object.freeze([
...
const SIGNED_V2_PAYLOAD_KEYS = Object.freeze([
  'schemaVersion', 'artifactKind', 'source', 'keyId', 'algorithm', 'sessionNonce', 'caseId',
  'captureSha256', 'sidecarSha256', 'manifestSha256', 'observations',
]);
const SIGNED_V1_ENVELOPE_KEYS = new Set([...SIGNED_V1_PAYLOAD_KEYS, 'signature']);
const SIGNED_V2_ENVELOPE_KEYS = new Set([...SIGNED_V2_PAYLOAD_KEYS, 'signature']);
...
const EXPECTED_FACTS_KEYS = Object.freeze([
  'captureSha256', 'sidecarSha256', 'manifestSha256', 'observationCount', 'observationSchemaVersion',
]);

// 命令行层预检事实（五字段）的可选闭合校验。缺省（undefined）时保持既有行为不变；
// 传入时须为精确五字段 plain object，形状不合即 fail-closed（EXPECTED_FACTS_INVALID）。
function requireExpectedFacts(value) {
  if (value === undefined) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TypeError('EXPECTED_FACTS_INVALID');
  const keys = Object.keys(value);
  if (keys.length !== EXPECTED_FACTS_KEYS.length || keys.some((key) => !EXPECTED_FACTS_KEYS.includes(key))) {
    throw new TypeError('EXPECTED_FACTS_INVALID');
  }
  requireHash(value.captureSha256, 'EXPECTED_FACTS_INVALID');
  requireHash(value.sidecarSha256, 'EXPECTED_FACTS_INVALID');
  requireHash(value.manifestSha256, 'EXPECTED_FACTS_INVALID');
  if (!Number.isSafeInteger(value.observationCount) || value.observationCount < 0) throw new TypeError('EXPECTED_FACTS_INVALID');
  if (!Number.isSafeInteger(value.observationSchemaVersion) || value.observationSchemaVersion < 1) throw new TypeError('EXPECTED_FACTS_INVALID');
  return Object.freeze({ ...value });
}

function expectedFactsMismatch(expected, record) {
  return EXPECTED_FACTS_KEYS.some((key) => expected[key] !== record[key]);
}
...
function readPackage(capturePath, caseId) {
  ...
  const identityReview = reviewClosedIdentityObservationPackage({
    caseId,
    captureRaw: capture.bytes.toString('utf8'),
    observationRaw: sidecar.bytes.toString('utf8'),
  });
  if (!identityReview.ok) throw new TypeError(identityReview.reason || 'CLOSED_IDENTITY_PACKAGE_INVALID');
  return Object.freeze({ paths, capture, sidecar, manifest, packageReview, identityReview });
}

function reviewCaptureV2(packageSnapshot, caseId) {
  if (!packageSnapshot.identityReview?.ok || packageSnapshot.identityReview.capture.caseId !== caseId) {
    throw new TypeError(packageSnapshot.identityReview?.reason || 'CLOSED_IDENTITY_PACKAGE_INVALID');
  }
  return Object.freeze({
    doc: packageSnapshot.identityReview.capture,
    eventCount: packageSnapshot.identityReview.eventCount,
  });
}
...
function validateReceiptAgainstPackage(packageSnapshot, canonicalCapturePath, caseId) {
  ...
  const receiptDoc = validateReceiptDocument(
    ...
    packageSnapshot.identityReview.sidecar,
  );
...
function commitAcceptedTransaction({ canonical, caseId, initial, receipt, captureReview, expectedFacts = null }) {
  ...
  try {
    ...
    // 事务内、rename commit 之前的最后一道闸：命令行层预检事实若与本次将落账的事实不符
    // （命令行层读后、内核读前发生换包竞态），在此拒绝——committed 台账零新增，fail 必须干净。
    if (expectedFacts && expectedFactsMismatch(expectedFacts, record)) throw new TypeError('EXPECTED_FACTS_MISMATCH');
...
export function platformIdentityReadbackReceiptFacts(handle) {
  ...
  const {
    ...
    observations, provenance, ledgerGeneration, transactionId,
  } = internal;
  return Object.freeze({
    ok: true,
    facts: Object.freeze({
...
export function appendAcceptedObservationPackage(options = {}) {
  try {
    const { caseId, capturePath, expectedFacts } = snapshotClosedOptions(
      options,
      new Set(['caseId', 'capturePath', 'expectedFacts']),
    );
    const safeCaseId = requireCaseId(caseId);
    const safeExpectedFacts = requireExpectedFacts(expectedFacts);
    ...
    const committed = commitAcceptedTransaction({
      canonical, caseId: safeCaseId, initial, receipt, captureReview, expectedFacts: safeExpectedFacts,
    });
...
function canonicalSignedPayload(doc) {
  const keys = doc.schemaVersion === 2 ? SIGNED_V2_PAYLOAD_KEYS : SIGNED_V1_PAYLOAD_KEYS;
  return Object.fromEntries(keys.map((key) => [key, doc[key]]));
}
...
function exactReceiptObservation(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TypeError('READBACK_OBSERVATION_INVALID');
  const keys = Object.keys(value);
  if (keys.length !== RECEIPT_OBSERVATION_KEYS.length
    || keys.some((key) => !RECEIPT_OBSERVATION_KEYS.includes(key))) {
    throw new TypeError('READBACK_OBSERVATION_UNKNOWN_OR_MISSING_FIELD');
  }
  for (const [key, field] of [['kind', value.kind], ['name', value.name], ['code', value.code], ['platformId', value.platformId], ['evidenceKind', value.evidenceKind]]) {
    if ((key === 'code' || key === 'platformId') && field === null) continue;
    requireSafeString(field, `READBACK_${key.toUpperCase()}_INVALID`);
  }
  if (value.parent !== null) requireHash(value.parent, 'READBACK_PARENT_INVALID');
  requireHash(value.scopeFingerprint, 'READBACK_SCOPE_INVALID');
  requireHash(value.evidenceSha256, 'READBACK_EVIDENCE_INVALID');
  if (!Number.isSafeInteger(value.eventSeq) || value.eventSeq < 1) throw new TypeError('READBACK_EVENTSEQ_INVALID');
  return Object.freeze(Object.fromEntries(RECEIPT_OBSERVATION_KEYS.map((key) => [key, value[key]])));
}

function validateReceiptDocument(doc, caseId, packageReview, sidecar) {
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) throw new TypeError('READBACK_RECEIPT_SHAPE_INVALID');
  const keys = Object.keys(doc);
  const envelopeKeys = doc.schemaVersion === 2 ? SIGNED_V2_ENVELOPE_KEYS : SIGNED_V1_ENVELOPE_KEYS;
  ...
  if (![1, 2].includes(doc.schemaVersion)
    || doc.artifactKind !== 'platform-identity-readback-receipt'
    || doc.source !== 'platform-runtime'
    || doc.algorithm !== 'Ed25519'
    || doc.caseId !== caseId) throw new TypeError('READBACK_RECEIPT_SCHEMA_INVALID');
  ...
  const sidecarObservations = Array.isArray(sidecar?.observations) ? sidecar.observations : [];
  let observations;
  if (doc.schemaVersion === 1) {
    if (sidecarObservations.length !== 1 || !Number.isSafeInteger(doc.eventSeq) || doc.eventSeq < 1) {
      throw new TypeError('READBACK_RECEIPT_COVERAGE_MISMATCH');
    }
    ...
    const sidecarObservation = sidecarObservations[0];
    for (const key of ['eventSeq', 'kind', 'name', 'code', 'platformId', 'scopeFingerprint', 'evidenceSha256']) {
      if (doc[key] !== sidecarObservation[key]) throw new TypeError(`READBACK_${key.toUpperCase()}_MISMATCH`);
    }
    observations = Object.freeze([sidecarObservation]);
  } else {
    if (!Array.isArray(doc.observations) || doc.observations.length !== sidecarObservations.length || doc.observations.length < 1) {
      throw new TypeError('READBACK_RECEIPT_COVERAGE_MISMATCH');
    }
    observations = Object.freeze(doc.observations.map(exactReceiptObservation));
    for (let index = 0; index < observations.length; index += 1) {
      for (const key of RECEIPT_OBSERVATION_KEYS) {
        if (observations[index][key] !== sidecarObservations[index][key]) {
          throw new TypeError(`READBACK_${key.toUpperCase()}_MISMATCH`);
        }
      }
    }
  }
...
export function readPlatformIdentityReadbackReceipt(options = {}) {
  try {
    ...
    const receipt = mintReadbackReceipt(Object.freeze({
      ...
      observations: receiptDoc.observations,
