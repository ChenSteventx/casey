...
// readback receipt 当前只证明 canonical I/O + exact binding；真实 platform driver provenance 仍须独立 authority。
...
const RECEIPT_KEYS = new Set([
  'schemaVersion', 'artifactKind', 'source', 'caseId', 'eventSeq', 'kind', 'name', 'code',
...
function validateReceiptDocument(doc, caseId) {
...
export function readPlatformIdentityReadbackReceipt(options = {}) {
  try {
    ...
    const receiptDoc = validateReceiptDocument(parseJsonObject(receiptSnapshot.bytes, 'READBACK_RECEIPT'), safeCaseId);
    ...
    const receipt = mintReadbackReceipt(Object.freeze({
      ...
      // 诚实边界：当前仅有 canonical file + accepted package 背书；没有真实 driver authority。
      provenance: 'canonical-receipt-io-only',
    }));
    return Object.freeze({ ok: true, reason: null, receipt, provenance: 'canonical-receipt-io-only' });
  } catch (error) {
