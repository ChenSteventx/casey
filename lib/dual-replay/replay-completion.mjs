...
export async function completeResolvedSourceReplay({
  ...
  BASELINE_GRANT_STATE.set(authoringBaselineGrant, {
    ...
    sourceRunNamespace: raw.record.runNamespace,
    // 私有 record 引用只存于 grant WeakMap entry；公开 grant 仍为空 opaque capability。
    // authoring owner seam 用引用相等闭合同一 source plan，不能以同标签 plan 换绑。
    sourcePlanRecord: raw.record.planRecord,
    // exact profile 只从 source authority 的私有复制传播；legacy grant 保持 null。
    channelProfileDigest: raw.record.planDigests.channelProfileDigest,
    channelProfileBytes: raw.record.planRecord.bytes.channelProfileBytes
      ? Buffer.from(raw.record.planRecord.bytes.channelProfileBytes)
      : null,
  });
