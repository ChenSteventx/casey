...
//   { state:'unsettled' } 文件在场但内容未落定——**不许落进 BATCH_SESSION_MISMATCH**，
//                         那会把「写者还没写完」误报成「这是另一批的重放」，拒因指错方向。
// 上界放宽到 200 轮（约 200ms），仍有界、到期仍 fail-closed。
function readSessionSettled(absPath, attempts = 200) {
  let sawFile = false;
  for (let i = 0; i < attempts; i += 1) {
    if (!existsSync(absPath)) {
      if (sawFile) { // 建了又没了：当未落定处理，绝不当作「未开会话」放行
    ...
    if (parsed && typeof parsed.batchToken === 'string') return { state: 'settled', session: parsed };
...
// 会话内容必须先落到盘上再放行：胜者写完即 fsync，
// 免得「放行了、但崩溃后台账上没这条」——那等于票据白烧还查不出。
function writeExclusiveDurable(absPath, document) {
  let fd;
  try { fd = openSync(absPath, 'wx'); }
  catch (error) { return error && error.code === 'EEXIST' ? 'exists' : false; }
  try {
...
export function occupyReplayGrantMember({ ledgerRoot, grantNonce, batchToken, caseId, grantCaseIds } = {}) {
  ...
  const seen = readSessionSettled(sessionPath);
  if (seen.state === 'unsettled') return refuse('REPLAY_GRANT_SESSION_NOT_SETTLED');
  const pre = judgeReplayGrantBatchSession({
    grantNonce,
    batchToken,
    caseId,
    grantCaseIds: caseIds,
    sessionSnapshot: seen.state === 'settled' ? seen.session : null,
  ...
  if (pre.opensSession) {
    const created = writeExclusiveDurable(sessionPath, { batchToken, occupiedAt: new Date().toISOString() });
    if (created === false) return refuse('REPLAY_GRANT_LEDGER_WRITE_FAILED');
    if (created === 'exists') {
      // 竞态败者：读回已在场的会话再判同值。未落定单列拒因，不冒充重放。
      const existing = readSessionSettled(sessionPath);
      if (existing.state === 'unsettled') return refuse('REPLAY_GRANT_SESSION_NOT_SETTLED');
      if (existing.state !== 'settled' || existing.session.batchToken !== batchToken) {
