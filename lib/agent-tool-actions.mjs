...
export function advanceAgentDeleteZeroEvidence(previous, sample) {
  ...
  const prior = Number.isInteger(previous?.zeroSamples) && previous.zeroSamples >= 0 ? previous.zeroSamples : 0;
  const zeroSamples = !busy && matches === 0 ? prior + 1 : 0;
  return {
    zeroSamples,
    ok: elapsedMs >= AGENT_DELETE_ZERO_MIN_MS && zeroSamples >= AGENT_DELETE_ZERO_SAMPLES,
  };
}
