...
async function verifyDeleteZero(page, ev, ctx) {
  ...
  if (!target) return axis('action_failed', 1);
  const deadline = Date.now() + 5000;
  while (true) {
    const matches = await targetRecordCount(page, target);
    if (matches === 0) return axis('unique', 1, true);
...
export async function performAgentToolAction(page, ev, ctx = {}) {
  ...
  if (ev.atom === 'agent.delete' && ev.action === 'click') {
    ...
    if (ev.text === '删除') return performWorkflowDeleteTrigger(page, target, { searchBoxName: AGENT_SEARCH_NAME });
    if (ev.text === '确定' || ev.text === '确认') return performWorkflowDeleteConfirm(page, ev.text, target);
  }
