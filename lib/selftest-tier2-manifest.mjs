...
export function validateSuiteManifestDoc(doc) {
  ...
  // replayGrantPath：批级回放授权票据的落点（p9-replay-authority-split）。
  // 与 winProbeResultPath / outOfBandReceiptPath 同构——清单只声明路径，件本身每批重签。
  for (const [key, prefix] of [['winProbeResultPath', 'runs/'], ['outOfBandReceiptPath', 'runs/'], ['replayGrantPath', 'runs/']]) {
