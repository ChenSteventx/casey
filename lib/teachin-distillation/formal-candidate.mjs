...
export function materializeDistilledFormalCandidate(options = {}) {
  let caseId;
  let captureSha256;
  let captureCreatedAt;
  let captureStartPath;
  let projection;
  let resolved;
  let structural;
  let compiledEvents;
  ...
  if (typeof captureCreatedAt !== 'string'
    || !Number.isFinite(Date.parse(captureCreatedAt))) {
    return denied('FORMAL_CANDIDATE_NOT_READY');
  }
  const normalizedStart = normalizeTopologyPath({
    value: captureStartPath,
    format: 'capture',
  });
  if (!normalizedStart.ok) return denied('FORMAL_CANDIDATE_NOT_READY');
  ...
  // 与现役 bin/compile 的 events.json 同形：document.url 优先绑定 compiler-authored
  // 首个 nav 目标；无 nav 才回落 admitted capture.startPath。recordedAt 复用 capture
  // provenance，禁止 wall clock 令同一输入重复物化出不同 bytes/hash。
  const firstNav = events.find((event) => event.action === 'nav');
  let documentUrl = `{{baseUrl}}${normalizedStart.path}`;
  if (firstNav) {
    const normalizedNav = normalizeTopologyPath({ value: firstNav.url, format: 'formal' });
    if (!normalizedNav.ok) return denied('FORMAL_CANDIDATE_NOT_READY');
    documentUrl = firstNav.url;
  }
  const eventsDocumentCandidate = {
    schemaVersion: 2,
    channel: 'web',
    caseId,
    url: documentUrl,
    recordedAt: captureCreatedAt,
    compiledBy: 'casey-teachin/atom-roundtrip',
    authored: false,
    events,
  };
