...
const TRUSTED_RUN_ISSUER_PUBLISHED = false;
...
export function readFrozenEntityLockSetAuthority(input = {}) {
  try {
    ...
    const publication = Object.prototype.hasOwnProperty.call(ENTITY_SEMANTIC_LOCK_PUBLICATIONS, contractId)
      ? ENTITY_SEMANTIC_LOCK_PUBLICATIONS[contractId]
      : null;
    if (!publication
      || !['runtime', 'historical'].includes(publication.mode)
      || !['release-resource', 'signed-manifest'].includes(publication.source)) {
      return authorityDenied('ENTITY_LOCK_AUTHORITY_NOT_PUBLISHED');
    }
    const frozenDigest = publication && Object.prototype.hasOwnProperty.call(publication.locks, lockSetKey)
      ? publication.locks[lockSetKey]
      : null;
    if (!frozenDigest) return authorityDenied('ENTITY_LOCK_AUTHORITY_NOT_PUBLISHED');
    if (!lockSetKey.startsWith('release/entity-semantic-lock/')) {
      return authorityDenied('ENTITY_LOCK_AUTHORITY_NOT_PUBLISHED');
    }
    if (publication.mode === 'runtime' && !publication.actionPolicy) {
      return authorityDenied('ENTITY_ACTION_POLICY_NOT_PUBLISHED');
    }

    const lockSetBytes = readPublishedFile(lockSetKey, frozenDigest);
    if (!lockSetBytes) return authorityDenied('ENTITY_LOCK_AUTHORITY_CHECKSUM_MISMATCH');
    ...
    if (publication.actionPolicy && !actionPolicyBytes) {
      return authorityDenied('ENTITY_ACTION_POLICY_CHECKSUM_MISMATCH');
    }
...
/** Report whether production runtime authority can execute real reads today. */
export function readEntityRuntimeReadiness() {
  if (TRUSTED_RUNTIME_DRIVERS.size === 0) {
    return Object.freeze({ available: false, route: 'human', reason: 'DRIVER_NOT_PUBLISHED' });
  }
  if (!TRUSTED_RUN_ISSUER_PUBLISHED) {
    return Object.freeze({ available: false, route: 'human', reason: 'RUN_ISSUER_NOT_PUBLISHED' });
  }
...
export function createEntityRuntimeAdapter(input = {}) {
  try {
    ...
    const published = adapterId && SAFE_ID.test(adapterId) ? TRUSTED_RUNTIME_DRIVERS.get(adapterId) : null;
    if (!raw || !published || typeof published.read !== 'function') {
    ...
    RUNTIME_ADAPTER_STATE.set(adapter, Object.freeze({ adapterId, read: published.read }));
