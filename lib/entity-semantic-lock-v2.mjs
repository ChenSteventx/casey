...
import { ENTITY_RUNTIME_AUTHORITY_BUNDLES } from './entity-runtime-authority-bundles.mjs';
...
export function readFrozenEntityLockSetAuthority(input = {}) {
  try {
    ...
    const runtimeRoot = material.publication.mode === 'runtime'
      ? inspectRuntimeAuthorityBundle(material.publication.rootId)
      : null;
    const runtimeBundleVerified = Boolean(runtimeRoot
      && runtimeRoot.state === 'verified-ready'
      && runtimeRoot.contractId === contractId
      && runtimeRoot.adapterId === material.publication.adapterId
      && runtimeRoot.issuerId === material.publication.issuerId);

    const authority = Object.freeze({
      artifactKind: 'frozen-entity-lock-set-authority',
      contractId,
      rootId: runtimeBundleVerified ? runtimeRoot.rootId : null,
      lockSetKey,
    });
...
function callableImplementationSha256(value) {
  try {
    if (typeof value !== 'function' || isProxy(value)) return null;
...
function snapshotRuntimeAuthorityBundle(rootKey, value) {
  const raw = closedObject(value, [
    'rootId', 'contractId', 'adapterId', 'issuerId', 'driver', 'issuer',
  ]);
  if (!raw) return null;
  ...
  const driver = closedObject(raw.driver, ['read']);
  const issuer = closedObject(raw.issuer, ['issue']);
  if (!rootId || rootId !== rootKey || !SAFE_ID.test(rootId)
    || !contractId || !CONTRACT_ID.test(contractId)
    || !adapterId || !SAFE_ID.test(adapterId)
    || !issuerId || !SAFE_ID.test(issuerId)
    || !driver || !issuer
    || !callableImplementationSha256(driver.read)
    || !callableImplementationSha256(issuer.issue)) return null;
  return Object.freeze({
...
function runtimeAuthorityBundleEntries() {
  try {
    if (isProxy(ENTITY_RUNTIME_AUTHORITY_BUNDLES)) return [];
    const descriptors = Object.getOwnPropertyDescriptors(ENTITY_RUNTIME_AUTHORITY_BUNDLES);
    return Object.entries(descriptors)
      .map(([rootId, descriptor]) => Object.prototype.hasOwnProperty.call(descriptor, 'value')
        ? snapshotRuntimeAuthorityBundle(rootId, descriptor.value)
        : null)
      .filter(Boolean);
  } catch {
    return [];
...
function parseRuntimeAuthorityHealthProof(value) {
  ...
  const raw = projectPlainObject(value, [
    ...
    'rootId', 'contractId', 'adapterId', 'issuerId',
    'driverImplementationSha256', 'issuerImplementationSha256',
  ...
  if (!rootId || !SAFE_ID.test(rootId)
    ...
    || !exactSha(raw.driverImplementationSha256)
    || !exactSha(raw.issuerImplementationSha256)
  ...
  return Object.freeze({
    ...
    driverImplementationSha256: raw.driverImplementationSha256,
    issuerImplementationSha256: raw.issuerImplementationSha256,
    lockSetSha256: raw.lockSetSha256,
    actionPolicySha256: raw.actionPolicySha256,
  });
}

function publishedBundleResult(bundle, reason = 'RUNTIME_AUTHORITY_HEALTH_NOT_VERIFIED') {
  return Object.freeze({
    state: 'published',
    reason,
...
// This is frozen structural readiness only. It deliberately does not call the
// driver or issuer and must never be presented as online executable health.
function inspectRuntimeAuthorityBundle(rootId) {
  const bundle = runtimeAuthorityBundleEntries().find((entry) => entry.rootId === rootId);
  if (!bundle) return null;
  const rawPublication = Object.prototype.hasOwnProperty.call(ENTITY_SEMANTIC_LOCK_PUBLICATIONS, bundle.contractId)
    ? ENTITY_SEMANTIC_LOCK_PUBLICATIONS[bundle.contractId]
    : null;
  const publication = snapshotEntityLockPublication(rawPublication);
  if (!publication || publication.mode !== 'runtime'
    || publication.rootId !== bundle.rootId
    || publication.adapterId !== bundle.adapterId
    || publication.issuerId !== bundle.issuerId
    || !publication.actionPolicy || !publication.healthProof) return publishedBundleResult(bundle);
  ...
  if (!health
    || health.rootId !== bundle.rootId
    || health.contractId !== bundle.contractId
    || health.adapterId !== bundle.adapterId
    || health.issuerId !== bundle.issuerId
    || health.driverImplementationSha256 !== callableImplementationSha256(bundle.read)
    || health.issuerImplementationSha256 !== callableImplementationSha256(bundle.issue)
    ...
    || publication.actionPolicy.sha256 !== health.actionPolicySha256) return publishedBundleResult(bundle);
  ...
  const actionPolicyBytes = readPublishedFile(publication.actionPolicy.key, publication.actionPolicy.sha256);
  ...
  if (!parsedLock || !parsedEvents || !parsedPolicy) return publishedBundleResult(bundle);
  ...
  if (!events || !lockSet || !actionPolicyByAction
    || !verifyRequiredActionRoles(events, lockSet, actionPolicyByAction)) return publishedBundleResult(bundle);
  return Object.freeze({
...
function invalidReadiness(reason = 'RUNTIME_READINESS_ROOT_INVALID') {
  return Object.freeze({ available: false, state: 'unpublished', route: 'human', reason });
}

/** Normalize one coherent runtime-authority root. It never mints authority. */
export function deriveEntityRuntimeReadiness(input = {}) {
  const raw = closedObject(input, [
    'state', 'rootId', 'contractId', 'adapterId', 'issuerId', 'reason',
  ]);
  if (!raw) return invalidReadiness();
  const rootId = exactString(raw.rootId);
  const contractId = exactString(raw.contractId);
  const adapterId = exactString(raw.adapterId);
  const issuerId = exactString(raw.issuerId);
  if (!rootId || !SAFE_ID.test(rootId)
    || !contractId || !CONTRACT_ID.test(contractId)
    || !adapterId || !SAFE_ID.test(adapterId)
    || !issuerId || !SAFE_ID.test(issuerId)) return invalidReadiness();
  if (raw.state === 'verified-ready' && raw.reason === null) {
    return Object.freeze({
      available: true, state: 'verified-ready', route: null, reason: null,
      rootId, contractId, adapterId, issuerId,
    });
  }
  if (raw.state === 'published' && isExactString(raw.reason)) {
    return Object.freeze({
      available: false, state: 'published', route: 'human', reason: raw.reason,
      rootId, contractId, adapterId, issuerId,
    });
  }
...
/** Report readiness only from one self-consistent module-private bundle. */
export function readEntityRuntimeReadiness() {
  const bundles = runtimeAuthorityBundleEntries();
  if (bundles.length === 0) return invalidReadiness('DRIVER_NOT_PUBLISHED');
  let published = null;
  for (const bundle of bundles) {
    const inspected = inspectRuntimeAuthorityBundle(bundle.rootId);
    if (!inspected) continue;
    if (inspected.state === 'verified-ready') return deriveEntityRuntimeReadiness({
      state: inspected.state,
      rootId: inspected.rootId,
      contractId: inspected.contractId,
      adapterId: inspected.adapterId,
      issuerId: inspected.issuerId,
      reason: inspected.reason,
    });
    published ??= inspected;
  }
  return published
    ? deriveEntityRuntimeReadiness({
      state: published.state,
      rootId: published.rootId,
      contractId: published.contractId,
      adapterId: published.adapterId,
      issuerId: published.issuerId,
      reason: published.reason,
    })
    : invalidReadiness('RUNTIME_AUTHORITY_HEALTH_NOT_VERIFIED');
}

function resolveVerifiedRuntimeBundle(rootId, contractId, memberId, memberKey) {
  const inspected = inspectRuntimeAuthorityBundle(rootId);
  return inspected && inspected.state === 'verified-ready'
    && inspected.contractId === contractId
    && inspected[memberKey] === memberId
    ? inspected.bundle
    : null;
}

/** Resolve one health-verified driver from a coherent hand-published bundle. */
export function createEntityRuntimeAdapter(input = {}) {
  try {
    const raw = closedObject(input, ['rootId', 'contractId', 'adapterId']);
    const rootId = raw && exactString(raw.rootId);
    const contractId = raw && exactString(raw.contractId);
    const adapterId = raw && exactString(raw.adapterId);
    const bundle = rootId && contractId && adapterId
      ? resolveVerifiedRuntimeBundle(rootId, contractId, adapterId, 'adapterId')
      : null;
    if (!bundle) return runtimeDenied('ENTITY_RUNTIME_ADAPTER_INVALID');
    const adapter = Object.freeze({ artifactKind: 'entity-runtime-adapter', adapterId });
    RUNTIME_ADAPTER_STATE.set(adapter, Object.freeze({
      rootId, contractId, adapterId, read: bundle.read,
    }));
...
function writeIssuedRunContext(bundle, value) {
...
/** Invoke one health-verified issuer and seal its coherent root into state. */
export async function issueEntityRunContext(input = {}) {
  try {
    const raw = closedObject(input, ['rootId', 'contractId', 'issuerId']);
    const rootId = raw && exactString(raw.rootId);
    const contractId = raw && exactString(raw.contractId);
    const issuerId = raw && exactString(raw.issuerId);
    const bundle = rootId && contractId && issuerId
      ? resolveVerifiedRuntimeBundle(rootId, contractId, issuerId, 'issuerId')
      : null;
    if (!bundle) return runtimeDenied('ENTITY_RUN_CONTEXT_ISSUER_INVALID');
    const issued = await bundle.issue(Object.freeze({ rootId, contractId, issuerId }));
    const runContext = writeIssuedRunContext(bundle, issued);
