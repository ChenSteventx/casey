...
const RUNTIME_PROVENANCE_BUDGET = Object.freeze({
  maxManifestBytes: 8 * 1024 * 1024,
  maxNodes: 4096,
  maxEdges: 4096,
  maxResourceBytes: 1024 * 1024,
  maxTotalResourceBytes: 8 * 1024 * 1024,
});
...
const RUNTIME_READINESS_INSPECTION_STATE = new WeakMap();
...
function closedArray(value, maxLength = Number.MAX_SAFE_INTEGER, budgetState = null) {
  try {
    ...
    if (!initialLengthDescriptor
      || !Object.prototype.hasOwnProperty.call(initialLengthDescriptor, 'value')) return null;
    const initialLength = initialLengthDescriptor.value;
    if (!Number.isSafeInteger(initialLength) || initialLength < 0) return null;
    if (initialLength > maxLength) {
      if (budgetState) budgetState.exceeded = true;
      return null;
    }
    const descriptors = Object.getOwnPropertyDescriptors(value);
    const ownKeys = Reflect.ownKeys(descriptors);
...
function readPublishedFile(relativePath, expectedSha256, options = null) {
  ...
  const countTowardTotal = options?.countTowardTotal !== false;
  let fd;
  try {
    ...
    const exceedsSingle = !Number.isSafeInteger(openedSize) || openedSize < 0 || openedSize > maxBytes;
    const exceedsTotal = byteBudget && countTowardTotal && openedSize > byteBudget.remainingBytes;
    if (exceedsSingle || exceedsTotal) {
      if (byteBudget) byteBudget.exceeded = true;
      return null;
    }
    ...
    if (byteBudget && countTowardTotal) byteBudget.remainingBytes -= bytes.length;
...
function snapshotEntityLockPublication(value) {
  const raw = closedObjectWithOptional(value, ['source', 'mode', 'locks'], [
    'rootId', 'adapterId', 'issuerId', 'actionPolicy', 'healthProof', 'manifest',
  ]);
  if (!raw
  ...
  const rootId = raw.rootId === undefined ? null : exactString(raw.rootId);
  const adapterId = raw.adapterId === undefined ? null : exactString(raw.adapterId);
  const issuerId = raw.issuerId === undefined ? null : exactString(raw.issuerId);
  if ((raw.rootId !== undefined && (!rootId || !SAFE_ID.test(rootId)))
    || (raw.adapterId !== undefined && (!adapterId || !SAFE_ID.test(adapterId)))
    || (raw.issuerId !== undefined && (!issuerId || !SAFE_ID.test(issuerId)))) return null;
  const identityCount = [rootId, adapterId, issuerId].filter(Boolean).length;
  ...
  const healthProof = raw.healthProof === undefined ? null : snapshotPublishedResource(raw.healthProof);
  ...
  if (raw.healthProof !== undefined && !healthProof) return null;
...
export function readFrozenEntityLockSetAuthority(input = {}) {
  try {
    ...
    // A provenance manifest is data-only in this contract. No executable
    // loader exists, so a runtime publication cannot mint runtime authority.
    const runtimeBundleVerified = false;
...
export function verifyEntityLockSet(input = {}) {
  try {
    ...
    const runtimeAuthorized = authorityState.publicationMode === 'runtime'
      && authorityState.runtimeBundleVerified === true
      && Boolean(authorityState.rootId)
    ...
    HANDLE_STATE.set(handle, {
      ...
      rootId: authorityState.rootId,
      contractId: authorityState.contractId,
      runtimeAuthorized,
...
// Discrimination facts only: strip the release semantics off compareCandidate.
// This unwraps the raw comparison down to the pure comparison facts
// (status/reason/candidateCount) and drops allowAction and nextAction, so a
// caller of the unit face below can exercise the discrimination logic yet can
// never obtain allowAction:true.  The sole authority to decide allowAction stays
// behind the runtimeAuthorized hard gate in evaluateEntityAction; this function,
// and the whole export face, never emits allowAction.
function compareCandidateFacts(receipt, candidate, policy) {
  const compared = compareCandidate(receipt, candidate, policy);
  return Object.freeze({
    status: compared.status,
    reason: compared.reason,
    candidateCount: compared.candidateCount ?? null,
  });
}

// Read-only unit face: frozen references to the existing discrimination pure
// functions, so unit goldens can attack the discrimination logic at the
// pure-function layer without minting a runtime capability (Steven 2026-07-18
// dual-track hermetic lane). This is an export only: it changes no behavior,
// mints no authority, and adds no executable loader.  The face exposes only
// comparison facts with no release semantics: allowAction's sole decision
// authority is reserved for evaluateEntityAction behind the runtimeAuthorized
// hard gate, and this export face never emits allowAction (compareCandidate is
// wrapped by the allowAction-stripped compareCandidateFacts; parse* and
// identityKey produce no release semantics either).  The verdict dispatch in
// evaluateEntityAction and the stateful successor path are unchanged and remain
// covered by the still-red forward baseline. Production bin/ and mcp/ paths must
// never import this face (asserted by the unit golden).
...
function sameRuntimeAuthorityRoot(left, right) {
...
function parseRuntimeAuthorityHealthProof(value) {
  // The health-proof file itself is a digest-frozen release resource. Embedded
  // producer metadata (including arbitrary *ReceiptSha256 fields) is never a
  // second authority root, so project only the fields consumed below.
  const raw = projectPlainObject(value, [
    'schemaVersion', 'artifactKind', 'status',
    'rootId', 'contractId', 'adapterId', 'issuerId',
    'caseId', 'lockSetKey', 'lockSetSha256', 'events', 'actionPolicySha256',
  ]);
  if (!raw || raw.schemaVersion !== 1
    || raw.artifactKind !== 'entity-runtime-authority-health-proof'
    || raw.status !== 'PASS') return null;
  const rootId = exactString(raw.rootId);
  const contractId = exactString(raw.contractId);
  const adapterId = exactString(raw.adapterId);
  const issuerId = exactString(raw.issuerId);
  const caseId = exactString(raw.caseId);
  const lockSetKey = exactReleaseResourcePath(raw.lockSetKey);
  const events = snapshotPublishedResource(raw.events);
  if (!rootId || !SAFE_ID.test(rootId)
    || !contractId || !CONTRACT_ID.test(contractId)
    || !adapterId || !SAFE_ID.test(adapterId)
    || !issuerId || !SAFE_ID.test(issuerId)
    || !caseId || !lockSetKey || !events
    || !exactSha(raw.lockSetSha256)
    || !exactSha(raw.actionPolicySha256)) return null;
  return Object.freeze({
    rootId, contractId, adapterId, issuerId, caseId, lockSetKey, events,
    lockSetSha256: raw.lockSetSha256,
    actionPolicySha256: raw.actionPolicySha256,
  });
}

function parseRuntimeAuthorityDataResource(value, graphBudget) {
  const raw = closedObject(value, ['id', 'resource', 'dependencies']);
  if (!raw) return null;
  const id = exactString(raw.id);
  const resource = snapshotPublishedResource(raw.resource);
  const dependencyRows = closedArray(
    raw.dependencies,
    RUNTIME_PROVENANCE_BUDGET.maxEdges,
    graphBudget,
  );
  if (!id || !SAFE_ID.test(id) || !resource || !dependencyRows) return null;
  graphBudget.edges += dependencyRows.length;
  if (graphBudget.edges > RUNTIME_PROVENANCE_BUDGET.maxEdges) {
    graphBudget.exceeded = true;
    return null;
  }
  const dependencies = dependencyRows.map(exactString);
  if (dependencies.some((item) => !item || !SAFE_ID.test(item))) return null;
  if (new Set(dependencies).size !== dependencies.length) return null;
  return Object.freeze({ id, resource, dependencies: Object.freeze(dependencies) });
}

function parseRuntimeAuthorityDataManifest(value, byteBudget) {
  const graphBudget = { edges: 0, exceeded: false };
  const raw = closedObject(value, [
    'schemaVersion', 'artifactKind',
    'rootId', 'contractId', 'adapterId', 'issuerId', 'entryId', 'resources',
  ]);
  if (!raw || raw.schemaVersion !== 1
    || raw.artifactKind !== 'entity-runtime-authority-data-manifest') return null;
  const rootId = exactString(raw.rootId);
  const contractId = exactString(raw.contractId);
  const adapterId = exactString(raw.adapterId);
  const issuerId = exactString(raw.issuerId);
  const entryId = exactString(raw.entryId);
  const rows = closedArray(
    raw.resources,
    RUNTIME_PROVENANCE_BUDGET.maxNodes,
    graphBudget,
  );
  if (!rootId || !SAFE_ID.test(rootId)
    || !contractId || !CONTRACT_ID.test(contractId)
    || !adapterId || !SAFE_ID.test(adapterId)
    || !issuerId || !SAFE_ID.test(issuerId)
    || !entryId || !SAFE_ID.test(entryId)
    || !rows || rows.length === 0) {
    if (graphBudget.exceeded) byteBudget.exceeded = true;
    return null;
  }
  const resources = [];
  for (const row of rows) {
    const resource = parseRuntimeAuthorityDataResource(row, graphBudget);
    if (!resource) {
      if (graphBudget.exceeded) byteBudget.exceeded = true;
      return null;
    }
  ...
  for (const resource of resources) {
    if (byId.has(resource.id)) return null;
    byId.set(resource.id, resource);
  }
  if (!byId.has(entryId)) return null;
  for (const resource of resources) {
    if (resource.dependencies.some((dependencyId) => !byId.has(dependencyId))) return null;
  }

  // Explicit frames keep traversal independent of the JavaScript call stack.
  // State 1 means visiting; state 2 means fully visited.
  const visitState = new Map();
  const stack = [{ id: entryId, nextDependency: 0 }];
  visitState.set(entryId, 1);
  let visitedCount = 0;
  while (stack.length > 0) {
    const frame = stack[stack.length - 1];
    const resource = byId.get(frame.id);
    if (frame.nextDependency < resource.dependencies.length) {
      const dependencyId = resource.dependencies[frame.nextDependency];
      frame.nextDependency += 1;
      const state = visitState.get(dependencyId);
      if (state === 1) return null;
      if (state !== 2) {
        visitState.set(dependencyId, 1);
        stack.push({ id: dependencyId, nextDependency: 0 });
      }
      continue;
    }
  ...
  if (visitedCount !== resources.length) return null;
  for (const resource of resources) {
    if (!readPublishedFile(resource.resource.key, resource.resource.sha256, {
      maxBytes: RUNTIME_PROVENANCE_BUDGET.maxResourceBytes,
      byteBudget,
    })) return null;
  }
  return Object.freeze({ rootId, contractId, adapterId, issuerId, entryId });
}

function runtimeAuthorityPublicationEntries() {
  try {
    if (isProxy(ENTITY_SEMANTIC_LOCK_PUBLICATIONS)) return [];
    const descriptors = Object.getOwnPropertyDescriptors(ENTITY_SEMANTIC_LOCK_PUBLICATIONS);
    return Object.entries(descriptors).map(([contractId, descriptor]) => {
      ...
      return publication && publication.mode === 'runtime'
        ? Object.freeze({ contractId, publication })
        : null;
    }).filter(Boolean);
  } catch {
    return [];
...
function sealRuntimeReadinessInspection(state) {
  const inspection = Object.freeze({ artifactKind: 'entity-runtime-readiness-inspection' });
  RUNTIME_READINESS_INSPECTION_STATE.set(inspection, Object.freeze(state));
  return inspection;
}

function publishedInspection(entry, reason = 'RUNTIME_AUTHORITY_PROVENANCE_NOT_VERIFIED') {
  return sealRuntimeReadinessInspection({
    state: 'published',
    reason,
    rootId: entry.publication.rootId,
    contractId: entry.contractId,
    adapterId: entry.publication.adapterId,
    issuerId: entry.publication.issuerId,
  });
}

// Publication modules are the existing executable trust table. Candidate
// bundle/dependency resources below are read only as bytes and never imported,
// parsed as JavaScript, evaluated or called.
function inspectRuntimeAuthorityPublication(entry) {
  const { contractId, publication } = entry;
  const byteBudget = {
    remainingBytes: RUNTIME_PROVENANCE_BUDGET.maxTotalResourceBytes,
    exceeded: false,
  };
  const unavailable = () => publishedInspection(
    entry,
    byteBudget.exceeded
      ? 'RUNTIME_AUTHORITY_PROVENANCE_BUDGET_EXCEEDED'
      : 'RUNTIME_AUTHORITY_PROVENANCE_NOT_VERIFIED',
  );
  if (!publication.rootId || !publication.adapterId || !publication.issuerId
    || !publication.actionPolicy || !publication.healthProof || !publication.manifest) {
    return unavailable();
  }
  const manifestBytes = readPublishedFile(publication.manifest.key, publication.manifest.sha256, {
    maxBytes: RUNTIME_PROVENANCE_BUDGET.maxManifestBytes,
    byteBudget,
    countTowardTotal: false,
  });
  if (!manifestBytes) return unavailable();
  const parsedManifest = manifestBytes && parseJsonBytesOnce(manifestBytes);
  const manifest = parsedManifest && parseRuntimeAuthorityDataManifest(parsedManifest.value, byteBudget);
  if (!manifest
    || manifest.rootId !== publication.rootId
    || manifest.contractId !== contractId
    || manifest.adapterId !== publication.adapterId
    || manifest.issuerId !== publication.issuerId) return unavailable();

  const healthBytes = readPublishedFile(publication.healthProof.key, publication.healthProof.sha256, {
    maxBytes: RUNTIME_PROVENANCE_BUDGET.maxResourceBytes,
    byteBudget,
  });
  if (!healthBytes) return unavailable();
  const parsedHealth = healthBytes && parseJsonBytesOnce(healthBytes);
  const health = parsedHealth && parseRuntimeAuthorityHealthProof(parsedHealth.value);
  if (!health
    || health.rootId !== publication.rootId
    || health.contractId !== contractId
    || health.adapterId !== publication.adapterId
    || health.issuerId !== publication.issuerId
    || !Object.prototype.hasOwnProperty.call(publication.locks, health.lockSetKey)
    || publication.locks[health.lockSetKey] !== health.lockSetSha256
    || publication.actionPolicy.sha256 !== health.actionPolicySha256) return unavailable();

  const lockSetBytes = readPublishedFile(health.lockSetKey, health.lockSetSha256, {
    maxBytes: RUNTIME_PROVENANCE_BUDGET.maxResourceBytes,
    byteBudget,
  });
  if (!lockSetBytes) return unavailable();
  const eventsBytes = readPublishedFile(health.events.key, health.events.sha256, {
    maxBytes: RUNTIME_PROVENANCE_BUDGET.maxResourceBytes,
    byteBudget,
  });
  if (!eventsBytes) return unavailable();
  const actionPolicyBytes = readPublishedFile(
    publication.actionPolicy.key,
    publication.actionPolicy.sha256,
    {
      maxBytes: RUNTIME_PROVENANCE_BUDGET.maxResourceBytes,
      byteBudget,
    },
  );
  if (!actionPolicyBytes) return unavailable();
  const parsedLock = lockSetBytes && parseJsonBytesOnce(lockSetBytes);
  const parsedEvents = eventsBytes && parseJsonBytesOnce(eventsBytes);
  const parsedPolicy = actionPolicyBytes && parseJsonBytesOnce(actionPolicyBytes);
  if (!parsedLock || !parsedEvents || !parsedPolicy) return unavailable();
  const events = parseEventsDocument(parsedEvents.value, health.caseId);
  const lockSet = events && parseLockSet(
    parsedLock.value,
    health.caseId,
    sha256(parsedEvents.bytes),
    events,
  );
  const actionPolicyByAction = parseActionPolicyDocument(parsedPolicy.value);
  if (!events || !lockSet || !actionPolicyByAction
    || !verifyRequiredActionRoles(events, lockSet, actionPolicyByAction)) return unavailable();
  return sealRuntimeReadinessInspection({
    state: 'provenance-recorded',
    reason: 'RUNTIME_AUTHORITY_EXECUTABLE_LOADER_NOT_PUBLISHED',
    rootId: publication.rootId,
    contractId,
    adapterId: publication.adapterId,
    issuerId: publication.issuerId,
  });
}

function invalidReadiness(reason = 'RUNTIME_READINESS_INSPECTION_INVALID') {
  return Object.freeze({ available: false, state: 'unpublished', route: 'human', reason });
}

/** Normalize only a module-private opaque inspection. It never mints authority. */
export function deriveEntityRuntimeReadiness(input = {}) {
  const state = input && (typeof input === 'object' || typeof input === 'function')
    ? RUNTIME_READINESS_INSPECTION_STATE.get(input)
    : null;
  if (!state) return invalidReadiness();
  return Object.freeze({
    available: false,
    state: state.state,
    route: 'human',
...
/** Report non-executable provenance only from module-private inspections. */
export function readEntityRuntimeReadiness() {
  const entries = runtimeAuthorityPublicationEntries();
  if (entries.length === 0) return invalidReadiness('DRIVER_NOT_PUBLISHED');
  let published = null;
  for (const entry of entries) {
    let inspection;
    try {
      inspection = inspectRuntimeAuthorityPublication(entry);
      const state = RUNTIME_READINESS_INSPECTION_STATE.get(inspection);
      if (state?.state === 'provenance-recorded') return deriveEntityRuntimeReadiness(inspection);
    } catch {
      inspection = publishedInspection(entry, 'RUNTIME_AUTHORITY_PROVENANCE_INSPECTION_FAILED');
    }
    published ??= inspection;
  }
  return published ? deriveEntityRuntimeReadiness(published) : invalidReadiness();
}

/** No executable loader is published; callers cannot resolve a driver. */
export function createEntityRuntimeAdapter(input = {}) {
  try {
    closedObject(input, ['rootId', 'contractId', 'adapterId']);
...
/** No executable loader is published; callers cannot invoke an issuer. */
export async function issueEntityRunContext(input = {}) {
  try {
    closedObject(input, ['rootId', 'contractId', 'issuerId']);
    return runtimeDenied('ENTITY_RUN_CONTEXT_ISSUER_INVALID');
  } catch {
    return runtimeDenied('ENTITY_RUN_CONTEXT_ISSUER_INVALID');
  }
}
...
export async function readEntityRuntimeCapability(input = {}) {
  try {
    ...
    if (!sameRuntimeAuthorityRoot(adapterState, runState)) {
      return runtimeDenied('ENTITY_RUNTIME_AUTHORITY_ROOT_MISMATCH');
    }
    ...
    RUNTIME_CAPABILITY_STATE.set(capability, Object.freeze({
      adapter: raw.adapter,
      rootId: adapterState.rootId,
      contractId: adapterState.contractId,
...
export function evaluateEntityAction(input = {}) {
  try {
    ...
    if (!capabilityState) return deny('ENTITY_RUNTIME_CAPABILITY_INVALID');
    if (!sameRuntimeAuthorityRoot(state, capabilityState)) {
      return deny('ENTITY_RUNTIME_AUTHORITY_ROOT_MISMATCH');
    }
    ...
    if (bindingKey(capabilityState.binding) !== bindingKey(requested)) {
...
export function createRunSuccessorProof(input = {}) {
  try {
    ...
    const capabilityState = RUNTIME_CAPABILITY_STATE.get(raw.runtimeCapability);
    if (!capabilityState) return successorDenied('ENTITY_SUCCESSOR_INPUT_INVALID');
    if (!sameRuntimeAuthorityRoot(state, capabilityState)) {
      return successorDenied('ENTITY_RUNTIME_AUTHORITY_ROOT_MISMATCH');
    }
    const requested = parseActionBinding(raw.binding);
    const transitionName = exactString(raw.transitionId);
    const previousHash = exactSha(raw.previousHeadHash);
    if (!requested || !transitionName || !previousHash
