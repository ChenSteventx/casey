...
function parseRuntimeAuthorityDataResource(value) {
  ...
  const dependencyRows = closedArray(raw.dependencies);
...
function parseRuntimeAuthorityDataManifest(value) {
  const raw = closedObject(value, [
  ...
  const rows = closedArray(raw.resources);
  if (!rootId || !SAFE_ID.test(rootId)
    || !contractId || !CONTRACT_ID.test(contractId)
    || !adapterId || !SAFE_ID.test(adapterId)
    || !issuerId || !SAFE_ID.test(issuerId)
    || !entryId || !SAFE_ID.test(entryId)
    || !rows || rows.length === 0) return null;
  const resources = rows.map(parseRuntimeAuthorityDataResource);
  if (resources.some((item) => !item)) return null;
  const byId = new Map();
  ...
  if (!visit(entryId) || visited.size !== resources.length) return null;
  for (const resource of resources) {
    if (!readPublishedFile(resource.resource.key, resource.resource.sha256)) return null;
  }
...
function inspectRuntimeAuthorityPublication(entry) {
  ...
  if (!publication.rootId || !publication.adapterId || !publication.issuerId
    ...
    return publishedInspection(entry);
  }
  ...
  const manifest = parsedManifest && parseRuntimeAuthorityDataManifest(parsedManifest.value);
  if (!manifest
    || manifest.rootId !== publication.rootId
    || manifest.contractId !== contractId
    || manifest.adapterId !== publication.adapterId
    || manifest.issuerId !== publication.issuerId) return publishedInspection(entry);
  ...
  if (!health
    ...
    || publication.actionPolicy.sha256 !== health.actionPolicySha256) return publishedInspection(entry);
  ...
  if (!parsedLock || !parsedEvents || !parsedPolicy) return publishedInspection(entry);
  ...
  if (!events || !lockSet || !actionPolicyByAction
    || !verifyRequiredActionRoles(events, lockSet, actionPolicyByAction)) return publishedInspection(entry);
  return sealRuntimeReadinessInspection({
...
export function readEntityRuntimeReadiness() {
  ...
  for (const entry of entries) {
    const inspection = inspectRuntimeAuthorityPublication(entry);
