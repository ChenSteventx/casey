...
function committedTransactionValid(entry) {
  ...
  if (!entry || entry.transactionState !== 'committed'
    ...
    || typeof entry.driverReceiptSha256 !== 'string' || !/^sha256:[a-f0-9]{64}$/.test(entry.driverReceiptSha256)
    || typeof entry.driverKeyId !== 'string' || !entry.driverKeyId || entry.driverKeyId.length > 128
    || typeof entry.driverSessionNonce !== 'string' || !/^[A-Za-z0-9._:-]+$/.test(entry.driverSessionNonce)
    || entry.driverSessionNonce.length > 128
...
const TOPOLOGY_HANDOFF_ACTION = 'newpage';
...
export function projectCapture(doc) {
  ...
  events.forEach((ev, i) => {
    ...
    if (isTopologyAction(action)) {
      const trigger = projection[projection.length - 1];
      projection.push({
        intentId: trigger?.intentId || ('i' + seq),
        eventSeq: seq,
        action: TOPOLOGY_HANDOFF_ACTION,
        pathHint,
        topologyRole: 'handoff',
      });
      return;
    }
