...
// accepted intake authority 是进程内不透明能力句柄：品牌只存在模块私有 WeakMap，句柄自身无可复制字段。
// 后续 observation trusted API 必须调用 acceptedIntakeAuthorityFacts() 验品牌；不得接受
// {source:'accepted-intake-ledger', ...hashes} 一类 plain object 自证来源。
const ACCEPTED_INTAKE_AUTHORITIES = new WeakMap();

function mintAcceptedIntakeAuthority(facts) {
  const handle = Object.freeze(Object.create(null));
  ACCEPTED_INTAKE_AUTHORITIES.set(handle, Object.freeze({ ...facts }));
  return handle;
}

export function acceptedIntakeAuthorityFacts(handle) {
  const facts = handle && typeof handle === 'object' ? ACCEPTED_INTAKE_AUTHORITIES.get(handle) : null;
  return facts ? Object.freeze({ ok: true, facts }) : Object.freeze({ ok: false, reason: 'ACCEPTED_INTAKE_AUTHORITY_INVALID' });
}

// TOCTOU 语义核（GRILL D4）：校当前 capture 字节仍匹配 intake 台账 accept 条目的 captureSha256。
// reason 只回类别码，绝不回哈希/脏内容。
export function verifyIntaken({ caseId, ledgerEntries, currentSha256 }) {
...
export function verifyIntakenPackage({
  caseId,
  ledgerEntries,
  packageAuthority,
  ...
  try {
    ...
    const authority = mintAcceptedIntakeAuthority(packageFacts);
    return { ok: true, reason: null, authority };
  } catch {
