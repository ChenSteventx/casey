...
//
//   buildFlow(testcase, mapping) -> flow（{id,name,category,steps:[{atom,params}]}）
...
import { expandLearnedFlow } from './capture-atom-promotion.mjs';
...
export function buildFlow(testcase, mapping, { registry } = {}) {
  const raw = {
    ...
    category: 'normal',
    steps: (mapping || []).map((m) => ({ atom: m.atom, params: m.params ?? {} })),
  };
  if (!registry) return raw;
  const expanded = expandLearnedFlow(raw, registry);
  if (!expanded.ok) throw new Error(`学习宏展开失败：${expanded.problems.join(' / ')}`);
  return expanded.flow;
}
...
export function validateBridge(testcase, mapping, { registry } = {}) {
  ...
  mapping.forEach((m, i) => {
    ...
    if (m.params !== undefined && (typeof m.params !== 'object' || m.params === null || Array.isArray(m.params))) problems.push(`mapping[${i}] (${m.atom})：params 须为对象`);
    // 编译知识允许集闸：不在注册表由 validateDraft 报，此处专挡「册内但无编译知识」（真缝）——点名区分。
    const inRegistry = !!(registry && registry.atoms && registry.atoms[m.atom]);
    const learnedMacro = !!(inRegistry && registry.atoms[m.atom].macro);
    if (inRegistry && !learnedMacro && !isCompilableAtom(m.atom)) problems.push(`mapping[${i}]：原子「${m.atom}」在册但暂无编译知识（扩表走飞轮排期，桥前置拦，护栏 #17）`);
  });
  ...
  if (!registry) problems.push('缺 registry：无法跑 compile-gate 结构/前缀/状态机闸');
    ...
    const rawFlow = buildFlow(testcase, mapping);
    const rawGate = validateDraft(rawFlow, { prefix: testcase.uniquePrefix, registry, initialStates: testcase.preconditions || [] });
    if (!rawGate.ok) problems.push(...rawGate.problems);
    const expanded = expandLearnedFlow(rawFlow, registry);
    if (!expanded.ok) problems.push(...expanded.problems.map((p) => `学习宏展开：${p}`));
    else {
      const expandedGate = validateDraft(expanded.flow, { prefix: testcase.uniquePrefix, registry, initialStates: testcase.preconditions || [] });
      if (!expandedGate.ok) problems.push(...expandedGate.problems);
    }
  }
