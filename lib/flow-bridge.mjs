...
const ENTITY_BINDING_ROLES = new Set(['subject', 'source', 'target']);
...
function copyEntityBindings(bindings, mappingIndex) {
  ...
  return bindings.map((binding, bindingIndex) => {
    ...
    // subject/source/target 是上游显式语义：缺失或未知角色必须拒绝，不得按顺序猜测。
    if (!Object.hasOwn(binding, 'role') || !ENTITY_BINDING_ROLES.has(binding.role)) {
      throw new TypeError(`${label}.role 必须显式为 subject、source 或 target`);
    }
...
export function validateBridge(testcase, mapping, { registry } = {}) {
  ...
  if (!registry) problems.push('缺 registry：无法跑 compile-gate 结构/前缀/状态机闸');
    ...
    // 与 execute/replay 共用固定 side-effect policy：mutation 恰 subject，relation 恰 source+target，read 恰空。
    // bridge 只验证 CLI 外 LLM 的显式候选，不代猜角色/编号、不代签。
    const entityPolicy = requiredFlowEntityBindings(flow);
    if (!Array.isArray(entityPolicy)) problems.push(`实体绑定策略未闭合（${entityPolicy?.reason || 'ENTITY_BINDING_POLICY_INVALID'}）`);
