...
import {
  anyUnsupportedScope,
  normalizeUnsupportedScopes,
} from './unsupported-scopes.mjs';
...
function blockReason({ settled, truncated, unsupportedScopes }) {
  ...
  if (anyUnsupportedScope(unsupportedScopes)) return 'UNSUPPORTED_SCOPE';
  return null;
}
...
export async function observePage({ driver, intentId, maxCandidates = 20 } = {}) {
  ...
  const unsupportedScopes = normalizeUnsupportedScopes(snapshot.unsupportedScopes);
