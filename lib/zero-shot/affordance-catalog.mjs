...
function semanticOf(raw) {
  if ([raw?.accessibleName, raw?.label, raw?.text].some(isSensitivePublicText)) {
    return null;
  }
...
function digestCatalog({ truncated, totalCandidates, affordances }) {
  const body = canonical({ truncated, totalCandidates, affordances });
