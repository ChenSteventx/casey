...
import { isMagnifierSearchClick } from './workflow-delete-spec.mjs';
...
async function performActionOnPage(page, ev, ctx) {
  ...
  // 语义名回填（semantic-name-instantiate，位置按评审 r1 pi Medium 上移）：events 的 semantic.name 落盘
  // 保留 {{占位符}}（回放才能按各自 ctx 回填），定位那一刻必须拿实例化名——否则以字面 {{…}} 找元素
  // 恒 0 命中（B4 九跑实证）。放在全部动作分支之前，使「定位前必须回填」对通用路径与专用身份门
  // （doOpenNode / doDragTo / doSetNodeField placeholder 等消费 semantic.name 者）一律成立。
  // 纯投影：只在真含占位时产浅拷贝，落盘 event 与上游对象永不被改；无占位时返回同一引用，
  // 既有事件字节零漂移。下游 gateAndAct 与漂移探针不消费 semantic.name（只看 action/value/atom/
  // targetName），deleteByName 分支的 label 是固定字面（回填恒等），故行为面只此一处。
  ...
  // 放大镜搜索触发器（replay-magnifier-dispatch 契约）：前置闸白名单放行的同一形状（谓词与
  // workflow-delete-spec.mjs 单点共享，判据字面不落本文件），非破坏、无目标绑定义务——不入下方
  // 破坏性三 click 的绑定分支，落到函数末尾的通用锁定门真执行。此前分发层无本支，白名单事件
  // 运行时被无条件拒点（2026-08-10 真机三例实证：从未执行、清理意图折叠必败恒 NEEDS_HUMAN）。
  // 执行语义零特殊化：多候选 ambiguous 不点、零候选 none 不点、点不动照落 action_failed。
  if (ev.action === 'click' && ev.atom === 'workflow.deleteByName' && !isMagnifierSearchClick(ev)) {
