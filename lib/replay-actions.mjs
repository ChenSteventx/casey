...
async function doAgentSearchOpen(page, ev, ctx) {
  ...
  // 容器覆写同参（codex R1-M1）：与编译侧同一 profile.agents.itemContainer 通道（回放 ctx 携 profile）。
...
async function readBoundAgentSelectValue(trigHandle) {
  try {
...
async function doBindAgent(page, ev, ctx) {
  ...
  let optHandle = null;
  try {
    // 触发器身份门：被钉抽屉域内可见恰一 + 物理句柄绑定（codex R2：初始钉扎后须全程重判，绝不带疑落笔）。
    ...
    if (!trigHandle) return { resolution: 'action_failed', candidateCount: 1, identityReadback: { ok: false } };
    // 落笔前重判（三闸，与编译门同刻）。
    const pre = await verifyPinnedNodeDrawer(page, ev.nodeName, bound.pin, bound.rootHandle);
    if (pre.status !== 'ok'
      || !(await locatorStillBound(page, triggers, 0, trigHandle))
      || !(await handleInsideRoot(trigHandle, bound.rootHandle))) {
      return { resolution: 'action_failed', candidateCount: pre.count, identityReadback: { ok: false } };
    }
    // 条件展开（脚手架步，不改动作账）：选项浮层未开才点触发器（经物理句柄）。
    if ((await page.locator('.agent-bind-option:visible').count().catch(() => 0)) === 0) {
      try { await trigHandle.click({ timeout: 5000 }); } catch {
        return { resolution: 'action_failed', candidateCount: 1, identityReadback: { ok: false } };
      }
    }
    ...
    try { optHandle = await options.first().elementHandle({ timeout: 3000 }); } catch { optHandle = null; }
    if (!optHandle) return { resolution: 'action_failed', candidateCount: 1, identityReadback: { ok: false } };
    // 选项落笔前重判（浮层 teleport 到 body 无祖先包含可验——可见唯一闸 + 物理句柄 + 抽屉域重判兜底）。
    const atOpt = await verifyPinnedNodeDrawer(page, ev.nodeName, bound.pin, bound.rootHandle);
    if (atOpt.status !== 'ok'
      || !(await locatorStillBound(page, triggers, 0, trigHandle))
      || !(await handleInsideRoot(trigHandle, bound.rootHandle))
      || !(await locatorStillBound(page, options, 0, optHandle, 1))) {
      return { resolution: 'action_failed', candidateCount: atOpt.count, identityReadback: { ok: false } };
    }
    try { await optHandle.click({ timeout: 5000 }); } catch {
      return { resolution: 'action_failed', candidateCount: 1, identityReadback: { ok: false } };
    }
    // 点击后/回读前重判 + 后置核验（codex R1-H1）：被钉抽屉域内【可见】选中值恰一 + 精确等值。
    const afterClick = await verifyPinnedNodeDrawer(page, ev.nodeName, bound.pin, bound.rootHandle);
    if (afterClick.status !== 'ok'
      || !(await locatorStillBound(page, triggers, 0, trigHandle))
      || !(await handleInsideRoot(trigHandle, bound.rootHandle))) {
      return { resolution: 'action_failed', candidateCount: afterClick.count, identityReadback: { ok: false } };
    }
    // 回读走已绑定触发器物理句柄内查询（codex R3-H2：封替身触发器——惰性 locator 会读到替身；
    // 物理句柄内可见恰一在页内计算，与编译门同刻）。
...
async function doSetNodeField(page, ev, ctx) {
  ...
  const actual = encodeInputReadback({ nodeName: label, placeholder, exact, value: got });
  if (!actual) return { resolution: 'action_failed', candidateCount: 1, identityReadback: { ok: false }, wideCandidateValues: await wideFieldSnapshot(page, placeholder, exact) };
  return { resolution: 'unique', candidateCount: 1, identityReadback: { ok: true, actual }, wideCandidateValues: await wideFieldSnapshot(page, placeholder, exact) };
  } finally {
