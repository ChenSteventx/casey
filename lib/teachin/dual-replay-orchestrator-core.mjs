...
const PROFILE_SOURCE_PLAN_KEYS = [...SOURCE_PLAN_KEYS, 'channelProfileBytes'];
const SOURCE_KEYS = [
...
function validateInput(input) {
  ...
  // 旧 cycle-plan 没有 exact profile bytes，保持既有合同；same-capture 新路径
  // 多携一份私有 Buffer，除此之外仍为闭合键集。
  if (!exactKeys(input.sourcePlan, SOURCE_PLAN_KEYS)
    && !exactKeys(input.sourcePlan, PROFILE_SOURCE_PLAN_KEYS)) return false;
