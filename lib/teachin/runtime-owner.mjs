...
import { admissionPolicyForAtom } from '../entity-semantic-lock-preflight.mjs';
...
const profileRouteDenied = () => frozen({
  ok: false,
  reason: 'AUTHORING_PROFILE_READ_ENVELOPE_INVALID',
});
...
export function createAuthoringRuntimeSeam({
  ...
  async function openFreshAuthoringRuntime({
    ...
    baselineByRuntime.set(authoringRuntimeAuthority, {
      ...
      sourceRunNamespace: baseline.sourceRunNamespace,
      sourcePlanRecord: baseline.sourcePlanRecord,
      baselineProjectionSha256: baseline.baselineProjectionSha256,
      compilerEventsSha256: null,
    });
    const openedRuntime = {
      ok: true,
  ...
  function bindCompilerAuthoredReadEnvelope(options = {}) {
    if (!exactKeys(options, [
      'authoringRuntimeAuthority', 'caseId', 'eventsBytes',
    ])) return failed('AUTHORING_COMPILE_FAILED');
    const { authoringRuntimeAuthority, caseId, eventsBytes } = options;
    const owner = readRuntimeOwner(authoringRuntimeAuthority);
    const baseline = baselineByRuntime.get(authoringRuntimeAuthority);
    if (!owner || !baseline || typeof caseId !== 'string' || !caseId
      || caseId !== baseline.caseId || !Buffer.isBuffer(eventsBytes)
      || eventsBytes.length === 0 || baseline.compilerEventsSha256 !== null) {
      return failed('AUTHORING_COMPILE_FAILED');
    }
    let events;
    try {
      events = JSON.parse(eventsBytes.toString('utf8'));
    } catch {
      return failed('AUTHORING_COMPILE_FAILED');
    }
    if (!Array.isArray(events) || events.length === 0 || events.some((event) => !isRecord(event))) {
      return failed('AUTHORING_COMPILE_FAILED');
    }
    baseline.compilerEventsSha256 = bytesDigest(eventsBytes);
    return frozen({ ok: true });
  }

  async function closeAuthoringRuntime({ authoringRuntimeAuthority }) {
    const baseline = baselineByRuntime.get(authoringRuntimeAuthority) || null;
    ...
    return frozen({
      ok: true,
      authoringClosureAuthority: sealCanonicalAuthoringClosureAuthority({
        runNamespace: closed.owner.runNamespace,
        executionTargetAuthority: closed.owner.executionTargetAuthority,
        witness: closed.witness,
      }, baseline && baseline.compilerEventsSha256 ? {
        caseId: baseline.caseId,
        sourceRunNamespace: baseline.sourceRunNamespace,
        sourcePlanRecord: baseline.sourcePlanRecord,
        compilerEventsSha256: baseline.compilerEventsSha256,
      } : null),
    });
  }
  ...
  return frozen({
    ...
    bindCompilerAuthoredReadEnvelope,
...
function mintAuthoringClosureAuthority(record, profileReadBinding = null) {
  const authoringClosureAuthority = frozen(Object.create(null));
  AUTHORING_CLOSURE_STATE.set(authoringClosureAuthority, {
    ...record,
    consumed: false,
    profileReadBinding: profileReadBinding
      ? { ...profileReadBinding, consumed: false }
      : null,
  });
  return authoringClosureAuthority;
}

// exported legacy sealer 永不接受 profile/compiler binding；只有同模块 canonical close
// 能调用 private mint 写入这组 capability-backed 事实。
export function sealAuthoringClosureAuthority(record) {
  return mintAuthoringClosureAuthority(record, null);
}

function sealCanonicalAuthoringClosureAuthority(record, profileReadBinding) {
  return mintAuthoringClosureAuthority(record, profileReadBinding);
}

// canonical entity verifier 的窄 successor：只消费 genuine authoring closure 私绑的
// exact source-plan profile + compiler events。失败探针不消费；成功后本 route proof one-shot。
export function consumeAuthoringProfileReadEnvelope(options = {}) {
  if (!exactKeys(options, [
    'authoringClosureAuthority', 'caseId', 'eventsBytes',
  ])) return profileRouteDenied();
  const { authoringClosureAuthority, caseId, eventsBytes } = options;
  const closure = isRecord(authoringClosureAuthority)
    ? AUTHORING_CLOSURE_STATE.get(authoringClosureAuthority)
    : null;
  const binding = closure?.profileReadBinding || null;
  if (!closure || closure.consumed || !binding || binding.consumed
    || typeof caseId !== 'string' || !caseId
    || !Buffer.isBuffer(eventsBytes) || eventsBytes.length === 0
    || binding.caseId !== caseId) return profileRouteDenied();

  const plan = binding.sourcePlanRecord;
  const profileBytes = plan?.bytes?.channelProfileBytes;
  if (!plan || plan.caseId !== caseId
    || plan.runNamespace !== binding.sourceRunNamespace
    || plan.executionTargetAuthority !== closure.executionTargetAuthority
    || !Buffer.isBuffer(profileBytes)
    || bytesDigest(profileBytes) !== plan.digests?.channelProfileDigest) {
    return profileRouteDenied();
  }
  const parsedProfile = parseAuthoringChannelProfileBytes(profileBytes);
  if (!parsedProfile.ok || !parsedProfile.present
    || typeof parsedProfile.listRoute !== 'string' || !parsedProfile.listRoute) {
    return profileRouteDenied();
  }
  ...
  if (!isRecord(document) || document.caseId !== caseId
    || !Array.isArray(document.events) || document.events.length === 0
    || bytesDigest(Buffer.from(JSON.stringify(document.events), 'utf8'))
      !== binding.compilerEventsSha256) return profileRouteDenied();

  const expectedUrl = `{{baseUrl}}${parsedProfile.listRoute}`;
  if (document.url !== expectedUrl) return profileRouteDenied();
  const triples = new Set();
  for (const event of document.events) {
    if (!isRecord(event)
      || !['stepId', 'intentId', 'atom', 'action']
        .every((field) => typeof event[field] === 'string' && event[field].trim())
      || Object.hasOwn(event, 'pre')) return profileRouteDenied();
    const triple = JSON.stringify([event.stepId, event.intentId, event.atom]);
    if (triples.has(triple)) return profileRouteDenied();
    triples.add(triple);
    const policy = admissionPolicyForAtom(event.atom)?.rule;
    if (policy?.admissionClass !== 'unbound-read'
      || !policy.allowedActions?.includes(event.action)) return profileRouteDenied();
    if (event.atom === 'nav.workflowManagement') {
      if (event.action !== 'nav' || event.url !== expectedUrl) return profileRouteDenied();
    } else if (event.atom === 'assert.textVisible') {
      if (event.action !== 'assert'
        || typeof event.text !== 'string' || !event.text.trim()
        || Object.hasOwn(event, 'url')) return profileRouteDenied();
    } else {
      return profileRouteDenied();
    }
  }
