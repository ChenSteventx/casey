...
export function createRuntimeCycleAdapter(deps = {}) {
  ...
  async function runAndSealDistilledCandidate({
    ...
    // 只序列化 formal-candidate 物化的完整 document，禁止裸 event 数组进入 replay 链。
    if (!isRecord(candidate?.eventsDocumentCandidate)) return denied('REPLAY_EVENTS_INVALID');
    const eventsBytes = Buffer.from(JSON.stringify(candidate.eventsDocumentCandidate), 'utf8');
