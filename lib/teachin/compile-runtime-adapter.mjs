...
export function createCompileRuntimeAdapter(deps = {}) {
  ...
  const bindCompilerAuthoredReadEnvelope = typeof deps.bindCompilerAuthoredReadEnvelope === 'function'
    ? deps.bindCompilerAuthoredReadEnvelope
    : null;
  ...
  async function compileAuthoringCandidate(options = {}) {
    ...
    const compileAdapter = async ({ candidateTestCase, flow, lineagePlan }) => {
      ...
      try {
        ...
        // canonical owner seam 在 close 前私绑 compiler-authored exact event 数组；legacy
        // injected seam 缺席时保持原 close exact-input 与候选形状不变。
        if (bindCompilerAuthoredReadEnvelope) {
          const bound = bindCompilerAuthoredReadEnvelope({
            authoringRuntimeAuthority,
            caseId: frozenTestCase.caseId,
            eventsBytes: Buffer.from(JSON.stringify(compiledEvents), 'utf8'),
          });
          if (bound?.ok !== true) throw new AuthoringStageError('AUTHORING_COMPILE_FAILED');
        }
      } catch (error) {
