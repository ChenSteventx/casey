...
export const IMPLEMENTED_KINDS = new Set(['noErrorEnvelope', 'urlPathname', 'streamReplyReceived', 'countChange', 'noPageError', 'textVisible', 'noErrorToast', 'replyContains', 'replyMatches', 'textHidden', 'buttonState', 'inputReadback']);

// inputReadback 的冻结值仍是 schema 允许的 string，但内容必须是稳定规范载荷：把节点、字段锚、精确开关和值
// 一起签住，避免“错误节点/错误 placeholder 恰写入同值”只比 value 而假通过。固定键序由本函数唯一生成。
export function encodeInputReadback({ nodeName, placeholder, exact = false, value } = {}) {
  if (typeof nodeName !== 'string' || !nodeName.trim()) return undefined;
  if (typeof placeholder !== 'string' || !placeholder.trim()) return undefined;
  if (typeof exact !== 'boolean' || typeof value !== 'string') return undefined;
  return JSON.stringify({ nodeName, placeholder, exact, value });
}

export function decodeInputReadback(encoded) {
  if (typeof encoded !== 'string') return null;
  let parsed;
  try { parsed = JSON.parse(encoded); } catch { return null; }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return null;
  const keys = Object.keys(parsed);
  if (keys.length !== 4 || !['nodeName', 'placeholder', 'exact', 'value'].every((key) => keys.includes(key))) return null;
  const canonical = encodeInputReadback(parsed);
  return canonical === encoded ? parsed : null;
}

// inputReadback 只消费动作门对“同一物理字段”的成功回读，不在断言阶段重查 DOM：
// resolution/identityReadback 任一矛盾、缺失或 actual 非字符串都证不出，绝不凭 expected 值反推成功。
export function inputReadbackFromAction(action) {
  if (!action || typeof action !== 'object' || Array.isArray(action)) return undefined;
  if (action.resolution !== 'unique') return undefined;
  const readback = action.identityReadback;
  if (!readback || typeof readback !== 'object' || Array.isArray(readback) || readback.ok !== true) return undefined;
  return decodeInputReadback(readback.actual) ? readback.actual : undefined;
}
...
function evalOne(a, c) {
  ...
  switch (a.kind) {
    ...
    case 'inputReadback': {
      // 缺采集与非 equals op 一律 false/null；严格相等，拒绝 legacy 子串语义把半填/尾缀洗绿。
      if (!decodeInputReadback(c.inputReadback) || !decodeInputReadback(a.value)) return { ok: false, actual: null };
      return { ok: a.op === 'equals' && c.inputReadback === a.value, actual: c.inputReadback };
    }
    case 'noPageError': {
    ...
    case 'buttonState': {
      ...
      // enabled/disabled（btn-enable-ops D3，fail-safe）：判据采集在 buttonDisabledHits（disabled 属性 ∨
      // aria-disabled ∨ profile.buttons.disabledClass 补判，双通道同 buttonHits）；enabled = 全部命中可用、
      // disabled = 全部命中禁用；混合态/零命中/缺采集一律不判真（护栏 #14）。
      ...
      if (a.op === 'enabled' || a.op === 'disabled') {
        const dis = c.buttonDisabledHits ? c.buttonDisabledHits[a.value] : undefined;
        // 证据异常 fail-safe（codex R1-F1）：hits/dis 须非负整数——NaN<=0 为假会漏进 fail-open 判 enabled 真。
        if (!Number.isInteger(dis) || dis < 0) return { ok: false, actual: null }; // 缺/坏禁用态采集 = 证不出
        if (!Number.isInteger(hits) || hits < 0) return { ok: false, actual: null };
        const actual = `hits=${hits},disabled=${dis}`;
        if (hits === 0) return { ok: false, actual }; // 不在场谈不上状态
        return { ok: a.op === 'enabled' ? dis === 0 : dis === hits, actual };
      }
      return { ok: false, actual: hits };
    }
    // 其余 kind（dropdownReadback / requiredFilled / switchState）
