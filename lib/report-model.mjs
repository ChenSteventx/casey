...
const VISUAL_REVIEW_STATES = new Set(['CONSISTENT', 'INCONSISTENT', 'INDETERMINATE']);
const SAFE_FILE_TOKEN = /^(?!\.+$)[A-Za-z0-9._-]+$/;

function isSafeRelativeArtifactPath(v) {
  return typeof v === 'string' && v.length > 0 && !v.includes('%')
    && v.split('/').every((seg) => SAFE_FILE_TOKEN.test(seg));
}
...
// 多匹配的唯一合法字面量 = ambiguous（与 verdict.mjs 识别端一字同步；死字面量 fallback_first/multi 与幽灵 coord_fallback 同删）。
...
export function projectCaseNarrative(meta = {}, steps = []) {
  if (meta.naturalLanguage != null) {
    if (typeof meta.naturalLanguage !== 'string') throw new Error('assembleReportModel: meta.naturalLanguage 须 string 或省略（schema string|null）');
    return redactScalar(meta.naturalLanguage);
  }
  if (meta.preconditions != null && (!Array.isArray(meta.preconditions) || meta.preconditions.some((v) => typeof v !== 'string'))) {
    throw new Error('assembleReportModel: meta.preconditions 须字符串数组');
  }
  if (meta.intents != null && (!Array.isArray(meta.intents) || meta.intents.some((v) => !v || typeof v !== 'object' || typeof v.text !== 'string'))) {
    throw new Error('assembleReportModel: meta.intents 须含 text 字符串的对象数组');
  }
  const preconditions = (meta.preconditions || []).map(redactScalar);
  const intents = (meta.intents || []).map((it) => redactScalar(it.text));
  const parts = [];
  if (preconditions.length) parts.push(`前置条件：\n${preconditions.map((v, i) => `${i + 1}. ${v}`).join('\n')}`);
  if (intents.length) parts.push(`测试步骤：\n${intents.map((v, i) => `${i + 1}. ${v}`).join('\n')}`);
  if (parts.length) return parts.join('\n');
  const seen = new Set();
  const fallback = [];
  for (const s of steps) if (s.intentText && !seen.has(s.intentText)) { seen.add(s.intentText); fallback.push(s.intentText); }
  return fallback.length ? fallback.join('\n') : null;
}

export function projectVisualReview(input) {
  if (input == null) return null;
  if (typeof input !== 'object' || Array.isArray(input)) throw new Error('assembleReportModel: visualReview 须对象');
  if (!VISUAL_REVIEW_STATES.has(input.status)) throw new Error('assembleReportModel: visualReview.status 非法');
  if (typeof input.reviewer !== 'string' || !input.reviewer) throw new Error('assembleReportModel: visualReview.reviewer 须非空字符串');
  if (typeof input.reviewedAt !== 'string' || !isValidIsoDateTime(input.reviewedAt)) throw new Error('assembleReportModel: visualReview.reviewedAt 须真实 ISO date-time');
  if (typeof input.summary !== 'string' || !input.summary) throw new Error('assembleReportModel: visualReview.summary 须非空字符串');
  if (input.verdictImpact != null && input.verdictImpact !== 'none') throw new Error('assembleReportModel: visualReview.verdictImpact 只能为 none');
  if (input.evidence != null && !Array.isArray(input.evidence)) throw new Error('assembleReportModel: visualReview.evidence 须数组');
  const evidence = (input.evidence || []).map((e) => {
    if (!e || typeof e !== 'object' || !isSafeRelativeArtifactPath(e.file)) throw new Error('assembleReportModel: visualReview.evidence.file 须安全相对文件名');
    if (e.videoAt != null && (typeof e.videoAt !== 'number' || !Number.isFinite(e.videoAt) || e.videoAt < 0)) throw new Error('assembleReportModel: visualReview.evidence.videoAt 须非负有限数');
    if (e.note != null && typeof e.note !== 'string') throw new Error('assembleReportModel: visualReview.evidence.note 须字符串');
    return { file: e.file, videoAt: e.videoAt ?? null, note: e.note == null ? null : redactScalar(e.note) };
  });
  return { status: input.status, reviewer: redactScalar(input.reviewer), reviewedAt: input.reviewedAt, summary: redactScalar(input.summary), verdictImpact: 'none', evidence };
}

export function buildArtifactManifest(caseId, videoMeta = null, visualReview = null) {
  if (typeof caseId !== 'string' || !/^[A-Za-z0-9_-]+$/.test(caseId)) throw new Error('assembleReportModel: caseId 不能安全生成附件名');
  // 本函数是导出面，不能假设调用方必先经过 assembleReportModel 的 videoMeta 校验。
  // manifest 中的 file 会直接进入 HTML/Markdown 链接；在就地边界再次拒绝 URI/穿越路径。
  if (videoMeta != null && (!videoMeta || typeof videoMeta !== 'object' || Array.isArray(videoMeta)
    || !isSafeRelativeArtifactPath(videoMeta.file))) {
    throw new Error('assembleReportModel: videoMeta.file 须安全相对路径');
  }
  const files = [
    ['html', `${caseId}.report.html`, 'HTML 报告'], ['markdown', `${caseId}.report.md`, 'Markdown 报告'],
    ['json', `${caseId}.report.json`, 'JSON 报告'], ['verdict', 'verdict.json', '确定性裁定'],
    ['axes', 'axes.json', '三轴证据'], ['history', 'run-history.jsonl', '回放历史'], ['metrics', 'run-metrics.json', '回放指标'],
  ];
  if (videoMeta) files.push(['videoMeta', 'video.json', '录像元数据'], ['video', videoMeta.file, '回放录像']);
  if (visualReview) files.push(['visualReview', 'visual-review.json', '视觉复核']);
  return files.map(([kind, file, label]) => ({ kind, file, label }));
}
// url 路径逐段脱敏（含 percent-decode 双判）。真实目标 origin 受护栏 #7 保护，任何来源都不得进入报告。
function redactUrlPath(pathname) {
  return pathname.split('/').map((seg) => (seg && tokHit(seg) ? '[redacted]' : seg)).join('/');
}
// 护栏 #7：url 只保留 pathname，剥 origin/query/hash（真目标、token/email/PII 不得进报告）。
// 非 http(s)/ws(s) scheme、协议相对地址和不以单斜杠开头的非法值一律占位。
export function sanitizeUrl(u) {
  if (u == null) return '';
  const s = String(u);
  try {
    const parsed = new URL(s);
    if (!['http:', 'https:', 'ws:', 'wss:'].includes(parsed.protocol)) return '<redacted:non-http-url>';
    return redactUrlPath(parsed.pathname || '/');
  } catch {
    const raw = s.split('#')[0].split('?')[0];
    if (!raw.startsWith('/') || raw.startsWith('//') || raw.includes('://')) return '<redacted:non-http-url>';
    return redactUrlPath(raw);
  }
}
 ...
 * @param {object|null} [p.promptsetMeta]  被测参数元数据（regress-promptset）：{ promptId, category, source, promptText, note? }——
 *   投影为顶层 promptset 块（promptText/note 走 redactScalar 脱敏，护栏 #7）；null 时零行为差（不产 promptset 键）。
 */
export function assembleReportModel({ caseId, channel, verdict, axes, observed = null, events = null, meta = {}, videoMeta = null, promptsetMeta = null, visualReview: visualReviewInput = null }) {
  ...
  const naturalLanguage = projectCaseNarrative(meta, steps);
  const visualReview = projectVisualReview(visualReviewInput);
  ...
  // 工作流画布结构（report-workflow-structure，report-spec #11）：任一画布构造原子在场 → 据 PASS 覆盖判三类；非画布用例 null。
  // 五项（开始/结束节点、连线、开始/结束配置）诚实收缩成三类——model 不携开始/结束节点身份（prd observability）。
  const CANVAS_ATOMS = ['workflow.addNode', 'workflow.connectNodes', 'workflow.openNode'];
  let workflowStructure = null;
  if (steps.some((s) => CANVAS_ATOMS.includes(s.atom))) {
    const passedAtom = (atom) => steps.some((s) => s.atom === atom && s.verdict === 'PASS');
    const hasNodes = passedAtom('workflow.addNode');
    const hasConnection = passedAtom('workflow.connectNodes');
    const hasNodeConfig = passedAtom('workflow.openNode');
    workflowStructure = { hasNodes, hasConnection, hasNodeConfig, structureComplete: hasNodes && hasConnection && hasNodeConfig };
  }
  ...
  const model = {
    ...
    artifacts: buildArtifactManifest(cid, videoMeta, visualReview),
    visualReview,
    cleanupEvidence,
    workflowStructure,
  ...
  // 被测参数块（regress-promptset）：数据驱动行的 promptId/category/source + 脱敏后的 promptText/note；
  // 供逐行报告标注「测了哪条被测参数」与多用例聚合分组分段。不改任何裁定字段。
  const promptsetBlock = projectPromptset(promptsetMeta);
  if (promptsetBlock) model.promptset = promptsetBlock;
  return model;
}

// 被测参数元数据校验 + 脱敏投影（fail-closed：形状错即产物损坏抛）。promptText/note 走 redactScalar
// （同 naturalLanguage/toast 自由文本待遇，护栏 #7 纵深防御；末道闸仍是 bin/report.mjs credentialGate）。
const PROMPTSET_CATS = new Set(['normal', 'boundary', 'security']);
const PROMPTSET_SOURCES = new Set(['user', 'builtin']);
function projectPromptset(pm) {
  if (pm == null) return undefined;
  if (typeof pm !== 'object' || Array.isArray(pm)) throw new Error('assembleReportModel: promptsetMeta 须对象，fail-closed');
  if (typeof pm.promptId !== 'string' || !/^[a-z0-9_]+$/.test(pm.promptId)) throw new Error(`assembleReportModel: promptsetMeta.promptId 非法（^[a-z0-9_]+$）：${JSON.stringify(pm.promptId)}`);
  if (!PROMPTSET_CATS.has(pm.category)) throw new Error(`assembleReportModel: promptsetMeta.category 非法：${JSON.stringify(pm.category)}`);
  if (!PROMPTSET_SOURCES.has(pm.source)) throw new Error(`assembleReportModel: promptsetMeta.source 非法：${JSON.stringify(pm.source)}`);
  if (typeof pm.promptText !== 'string') throw new Error('assembleReportModel: promptsetMeta.promptText 须字符串');
  const out = { promptId: pm.promptId, category: pm.category, source: pm.source, promptText: redactScalar(pm.promptText) };
  if (pm.note != null) {
    if (typeof pm.note !== 'string') throw new Error('assembleReportModel: promptsetMeta.note 须字符串');
    out.note = redactScalar(pm.note);
  }
  return out;
}

// ── 多用例聚合报告数据源（regress-promptset，兑现 report-spec §7）─────────────────────
// 纯函数：吃 N 份已解析 <caseId>.report.json 旁车 → 按 promptset.category 分段 + 置顶横幅 + content-expect 黄标。
// 内核硬约束：verdictTotals 只累加各旁车自带 verdictSummary（其本身已排除 soft——verdict 忽略 soft，护栏 #17），
// content-expect 软期望只从旁车 soft 项 surface 到 case.softExpect 黄标、绝不进任何裁定计数。畸形旁车 fail-closed。
const AGG_CATS = ['normal', 'boundary', 'security'];
const VERDICT_KEYS = ['PASS', 'SUT_DEFECT', 'HARNESS_ERROR', 'NEEDS_HUMAN'];
function pickVerdictSummary(vs) { const o = {}; for (const k of VERDICT_KEYS) o[k] = vs[k]; return o; }
export function assembleAggregateModel({ reports, generatedAt }) {
  if (!Array.isArray(reports)) throw new Error('assembleAggregateModel: reports 须数组，fail-closed');
  if (reports.length === 0) throw new Error('assembleAggregateModel: reports 为空，fail-closed');
  if (!generatedAt || typeof generatedAt !== 'string' || !isValidIsoDateTime(generatedAt)) throw new Error('assembleAggregateModel: generatedAt 须真实 ISO 8601 date-time');
  const verdictTotals = { PASS: 0, SUT_DEFECT: 0, HARNESS_ERROR: 0, NEEDS_HUMAN: 0 };
  const banner = [];
  const byCat = new Map();
  for (const r of reports) {
    if (!r || typeof r !== 'object' || Array.isArray(r)) throw new Error('assembleAggregateModel: 旁车非对象，fail-closed');
    if (typeof r.caseId !== 'string' || r.caseId === '') throw new Error('assembleAggregateModel: 旁车缺 caseId，fail-closed');
    const vs = r.verdictSummary;
    if (!vs || typeof vs !== 'object' || Array.isArray(vs)) throw new Error(`assembleAggregateModel: 旁车(${r.caseId}) 缺 verdictSummary，fail-closed`);
    for (const k of VERDICT_KEYS) {
      if (!Number.isInteger(vs[k])) throw new Error(`assembleAggregateModel: 旁车(${r.caseId}) verdictSummary.${k} 非整数，fail-closed`);
      verdictTotals[k] += vs[k];
    }
    if (!Array.isArray(r.steps)) throw new Error(`assembleAggregateModel: 旁车(${r.caseId}) steps 非数组，fail-closed`);
    const ps = r.promptset && typeof r.promptset === 'object' && !Array.isArray(r.promptset) ? r.promptset : null;
    const category = ps && AGG_CATS.includes(ps.category) ? ps.category : 'normal';
    const promptId = ps && typeof ps.promptId === 'string' ? ps.promptId : null;
    const promptText = ps && typeof ps.promptText === 'string' ? ps.promptText : null; // 旁车 promptText 已在装配期脱敏
    const flagged = vs.SUT_DEFECT > 0 || vs.NEEDS_HUMAN > 0;
    // content-expect 黄标：只取旁车 steps[].assertions 的 soft===true 项（绝不进 verdictTotals）。
    const softExpect = [];
    for (const s of r.steps) {
      for (const a of (s && Array.isArray(s.assertions) ? s.assertions : [])) {
        if (a && a.soft === true) softExpect.push({ kind: strOrNull(a.kind), op: strOrNull(a.op), value: a.value ?? null, actual: a.actual ?? null, ok: a.ok === true });
      }
    }
    if (r.reportHref != null && !isSafeRelativeArtifactPath(r.reportHref)) throw new Error(`assembleAggregateModel: 旁车(${r.caseId}) reportHref 非安全相对路径`);
    const entry = { caseId: r.caseId, promptId, promptText, reportHref: r.reportHref ?? null, verdictSummary: pickVerdictSummary(vs), flagged, softExpect };
    if (flagged) banner.push({ caseId: r.caseId, promptId, category, verdictSummary: pickVerdictSummary(vs) });
    if (!byCat.has(category)) byCat.set(category, []);
    byCat.get(category).push(entry);
  }
  const groups = AGG_CATS.filter((c) => byCat.has(c)).map((c) => ({ category: c, cases: byCat.get(c) }));
  return { schemaVersion: 1, generatedAt, caseCount: reports.length, verdictTotals, banner, groups };
}
