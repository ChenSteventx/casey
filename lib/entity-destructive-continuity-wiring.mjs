...
export function installOutboundMutationGuard(page, { atom, ref, urlPattern, requirePattern, onDecision, onAbort } = {}) {
  ...
  // codex round-3 High「所谓精确 mutation pattern 未成立」收口：缺 urlPattern 旧实现兜底装 **/*，会拦同一动作窗内的
  // 背景/刷新请求 → 误 abort → 触整步 pageerror 排除。requirePattern:true（生产 replay 传入）下缺精确 pattern 即
  // fail-CLOSED 不装（不兜 **/*）——由 caller 据此拒执行破坏动作（route:human 真机由 profile.mutationUrlPattern 供精确值）。
  // 未传 requirePattern（既有接线金牌 w1-w5 的纯适配器契约）保留 **/* 兜底默认，零行为差、不破人签冻结面。
  const patternOk = typeof urlPattern === 'string' && urlPattern.trim();
  if (requirePattern === true && !patternOk) {
    return { installed: false, reason: 'MUTATION_URL_PATTERN_REQUIRED' };
  }
  const pattern = patternOk ? urlPattern : '**/*';
  const handler = async (route) => {
    ...
    try {
      if (decision.ok && decision.released) {
        ...
        // codex round-3 High「abort 因果排除」：把被中止请求的 url 外露给 caller（onAbort），供 pageerror 逐请求
        // 因果归因用中止 url 作精确锚（非按 step 全吞）；再执行请求发出前中止（SUT 未改）。
        if (typeof onAbort === 'function') onAbort(built.url ?? null);
  ...
  // High-1：真 Playwright page.route() 返 Promise（handler 注册是异步落定）；同步 mock 返 undefined。
  // 把该 Promise 作 ready 外露，caller 在放行破坏动作前 await——堵「安装器返 installed:true 时 handler 尚未注册、
  // mutation 已出站」的时序洞（真机时序正确性仍走 route:human）。
  const routeResult = page.route(pattern, handler);
  const ready = Promise.resolve(routeResult);
  // Medium：生命周期收口——返 unroute 解除器，caller 于破坏步收尾解除本拦截器，避免 **/* 持久拦截后续列表刷新/
  // 归零查询（精确 urlPattern 由 profile.mutationUrlPattern 供、真机取值走 route:human）。mock page 无 unroute 时 no-op。
  const unroute = () => (page && typeof page.unroute === 'function' ? page.unroute(pattern, handler) : undefined);
  return { installed: true, pattern, ready, unroute };
}
