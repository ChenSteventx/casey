...
async function tickSnapshot(page, loadingGate) {
  ...
  try {
    ...
    // 兼容既有 zero-SUT 桩只返回长度；生产 page.evaluate 返回上面的对象。
    if (typeof raw === 'number' && Number.isFinite(raw)) return { len: raw, placeholderGone: true };
...
export async function settleBeforeCapture(page, opts = {}) {
  ...
  while (Date.now() - loopStart < budgetMs) {
    ...
    const loadingReady = loadingGate && continues && placeholderGone && prevPlaceholderGone;
