...
// 一次性授权的唯一消费面。可注入隔离 WeakMap 做零 SUT 行为测试；生产默认使用模块私有存储。
export function consumePendingDeleteAuthorization(page, store = pendingDeleteByPage) {
  const pending = store.get(page);
  if (!pending) return null;
  store.delete(page);
  return pending;
}

export function validateDeleteConfirmationAuthorization(pending, confirmName, expectedName = null) {
  if (!pending || typeof pending !== 'object') return false;
  const wanted = String(confirmName == null ? '' : confirmName).trim();
  if (!['确定', '确认'].includes(wanted)) return false;
  const expected = expectedName == null ? null : String(expectedName).trim();
  return !expected || pending.targetName === expected;
}
...
export async function performWorkflowDeleteTrigger(page, expectedName = null) {
  ...
  const named = await targetNameFromSearch(page, expected);
...
export async function performWorkflowDeleteConfirm(page, confirmName, expectedName = null) {
  const pending = consumePendingDeleteAuthorization(page);
  if (!pending) return axis('action_failed', 0);
  // 一次性消费已发生在任何校验或点击之前；失败也不得复用旧授权。
  try {
    if (!validateDeleteConfirmationAuthorization(pending, confirmName, expectedName)) return axis('action_failed', 0);
    const wanted = String(confirmName).trim();
