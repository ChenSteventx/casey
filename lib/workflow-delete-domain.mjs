...
async function performLockedAction(page, { root, candidateCount, actionText, rescan, mode = 'text' }) {
...
async function performCardMenuDeleteTrigger(page, { root, rescan }) {
  ...
  let menu = null;
  // 收拾自己开的浮层。失败绝不吞：具名落 menuCleanup，且任何取值都不参与 resolution 判定。
  const settle = async (result) => {
    if (!menuOpened) return result;
  ...
  try {
    ...
    let entry;
    try {
      entry = await performLockedAction(page, { root, candidateCount: 1, actionText: null, rescan, mode: 'more' });
      if (entry.resolution === 'none' || entry.resolution === 'ambiguous') return entry;
      // action_failed 时点击可能已经发生（例如 click 抛错），菜单状态未知 → 一律走收拾。
      if (entry.resolution !== 'unique' || entry.identityReadback?.ok !== true) { menuOpened = true; return await settle(entry); }
      menuOpened = true;

      const causal = await waitForCausalMenu(page, baselineMenus);
      if (causal.resolution !== 'unique' || !causal.dialog) {
        return await settle(axis(causal.resolution === 'unique' ? 'action_failed' : causal.resolution, causal.candidateCount));
      }
      menu = causal.dialog;
    } finally {
