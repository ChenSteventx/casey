...
async function performCardMenuDeleteTrigger(page, { root, rescan }) {
  // 只在更多操作入口【真的被点下去】之后才收拾——「还没点就败了」不可能有自己开的浮层，
  // 记 closed 会把没发生的收拾动作写成发生过（实现审 M3）。beforeClick 在落笔前一刻置位，
  // 故 click 自身抛错（浮层可能已浮出）仍算点过，照样收拾。
  let entryClicked = false;
  let menu = null;
  // 收拾自己开的浮层。失败绝不吞：具名落 menuCleanup，且任何取值都不参与 resolution 判定。
  const settle = async (result) => {
    if (!entryClicked) return result;
  ...
  try {
    ...
    try {
      entry = await performLockedAction(page, {
        ...
        beforeClick: () => { entryClicked = true; },
      });
      if (entry.resolution !== 'unique' || entry.identityReadback?.ok !== true) return await settle(entry);
