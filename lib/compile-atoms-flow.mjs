...
import { createCompileAtomExecutionFailure } from './compile-execution-failure.mjs';
...
// lineagePlan 是加法式兼容扩展：旧调用不传第三参数时行为一字不变；给出 plan 时先在任何页面动作前
// 验证整份长度/mappingKey/sourceIntentId，再用既有 firstEvent 边界如实记录每步产出的 stepIds。
// 它不按最终 atom 名猜，也不另写编译器。
export async function compileFlow(run, flow, { lineagePlan } = {}) {
  const steps = flow.steps || [];
  const plan = Array.isArray(lineagePlan) ? lineagePlan : null;
  if (plan) {
    if (plan.length !== steps.length) throw new Error('COMPILE_LINEAGE_PLAN_INVALID');
    for (let index = 0; index < plan.length; index += 1) {
      const row = plan[index];
      if (!row || typeof row !== 'object'
        || typeof row.mappingKey !== 'string' || !row.mappingKey
        || row.sourceIntentId !== steps[index].sourceIntentId) {
        throw new Error('COMPILE_LINEAGE_PLAN_INVALID');
      }
    }
  }
  const lineage = plan ? [] : null;
  let planIndex = 0;
  for (let stepIndex = 0; stepIndex < steps.length; stepIndex += 1) {
    ...
    try {
      ...
      // 真实页面/网络/编译器异常可能携 URL、参数、DOM 文本或身份值。异常越过 compileFlow
      // 边界前只保留稳定码、1-based 结构序号与注册表内 atom；未知 atom 不回显原值。
      // 未登记 atom 属输入/能力拒绝而非已知 atom 的执行失败，保留既有具名类别文案但同样遮掉原值。
      if (!COMPILE_KNOWN_ATOMS.has(step?.atom)) throw new Error('原子暂无编译知识（扩表走飞轮排期，护栏 #17 加法式）');
      throw createCompileAtomExecutionFailure({
        stepOrdinal: stepIndex + 1,
        atom: step?.atom,
        knownAtoms: COMPILE_KNOWN_ATOMS,
      });
    }
    // teach-in lineage plan 在这里仍同时持有 authored sourceIntentId 与本步刚刚产生的
    // exact event slice；compiler-local intent_N 只服务编译运行态，不得进入 formal replay。
    // 因此身份在事实产生点直接重绑，不在下游按 atom、数组位置或标签反推。
    if (lineage && run.events.length > firstEvent) {
      const sourceIntentId = plan[planIndex].sourceIntentId;
      const seenStepIds = new Set();
      for (const event of run.events.slice(firstEvent)) {
        if (!event || typeof event !== 'object'
          || typeof event.stepId !== 'string' || !event.stepId
          || seenStepIds.has(event.stepId)) {
          throw new Error('COMPILE_LINEAGE_EVENT_INVALID');
        }
        seenStepIds.add(event.stepId);
        event.intentId = sourceIntentId;
      }
      // createCompileRun.emit 会把 compiler-local identity 留在 lastIntentId；后继
      // assert.* 不产 event，而是折叠到这一锚。事件既已按 source lineage 重绑，
      // 该折叠锚必须同步成同一 authored identity，不能让 intent_N 从旁路泄回。
      if (Object.hasOwn(run, 'lastIntentId')) run.lastIntentId = sourceIntentId;
    }
    // 标准路径意图号重绑（compile-intent-lineage-rebind）：authored `sourceIntentId` 才是正式产物的意图
    // 身份（CONTEXT.md `intentId` 词条），compiler-local intent_N 只服务编译运行态——上方 teach-in 块原则的
    // 标准路径补齐（两块并列不合并：teach-in 系 PRD 冻结面大，字节零漂移优先于去重）。逐步存在即绑：带号步
    // 事件重绑并同步 lastIntentId（断言折叠锚不让 intent_N 旁路泄回）；裸步（nav/assert 惯例，见
    // entity-ui-wiring.bindagent-replay 夹具口径）保持自生号与遗留折叠行为，混合流合法。
    if (!plan && run.events.length > firstEvent
      && typeof step?.sourceIntentId === 'string' && step.sourceIntentId.trim()) {
      const sourceIntentId = step.sourceIntentId;
      const reboundStepIds = new Set();
      for (const event of run.events.slice(firstEvent)) {
        event.intentId = sourceIntentId;
        reboundStepIds.add(event.stepId);
      }
      // observed/verification 在 emit 时刻已带走自生号（grok r1 H1）：不同步则 draft 存在性闸按 observed
      // 全集判 authored 草稿为幽灵 intent 必拒——三通道逐 stepId 同号，authored 号才真正进全部正式产物。
      // 防御性取值同 blockers 先例：极简 run 桩可缺这两字段。
      for (const row of Array.isArray(run.observed) ? run.observed : []) {
        if (reboundStepIds.has(row.stepId)) row.intentId = sourceIntentId;
      }
      for (const row of Array.isArray(run.verification) ? run.verification : []) {
        if (reboundStepIds.has(row.stepId)) row.intentId = sourceIntentId;
      }
      if (Object.hasOwn(run, 'lastIntentId')) run.lastIntentId = sourceIntentId;
    }
    ...
    if (lineage) {
      lineage.push({
        mappingKey: plan[planIndex].mappingKey,
        sourceIntentId: plan[planIndex].sourceIntentId,
        stepIds: run.events.slice(firstEvent).map((event) => event.stepId),
      });
    }
    planIndex += 1;
  }
...
async function compileAtomStep(run, { atom, params = {} }) {
  ...
  throw new Error('原子暂无编译知识（扩表走飞轮排期，护栏 #17 加法式）');
}
