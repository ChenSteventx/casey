...
import {
  fetchCreatedWorkflowListScan,
  installCreatedWorkflowDeletionGuard,
} from '../entity-created-workflow-continuity-v3.mjs';
import {
  replayHistoryLine,
  replayPathOf,
  waitReplyStable,
...
export async function runReplayEvents({
    ...
    for (const event of events) {
      ...
      if (atomRejection) {
        ...
        if (event.action === 'click'
          && ctx.createdWorkflowController
          && event.stepId === ctx.createdWorkflowController.deleteTerminalStepId) {
          const installed = installCreatedWorkflowDeletionGuard(page, {
            controller: ctx.createdWorkflowController,
            stepId: event.stepId,
            onDecision: (decision) => {
              if (!decision.ok) {
                guardAborts.push({
          ...
          if (installed.installed === false) {
            ...
            log(`v3 created-workflow guard install failed (${installed.reason})`);
          } else {
            guardReady = installed.ready;
            await guardReady;
            guardTeardown = installed.unroute;
          }
      ...
      if (isLast) {
        ...
        if (ctx.createdWorkflowController && event.atom === 'workflow.create') {
          const fetched = await fetchCreatedWorkflowListScan(page, {
            profile,
            entityName: ctx.createdWorkflowController.entityName,
          });
          if (fetched.ok) ctx.createdWorkflowController.captureCreatedScan(fetched.scan);
        }
        if (ctx.createdWorkflowController && event.atom === 'workflow.deleteByName') {
          for (let sampleIndex = 0; sampleIndex < 3; sampleIndex += 1) {
            ...
            if (fetched.ok) {
              ctx.createdWorkflowController.recordAbsenceScan(fetched.scan, Date.now());
            }
          }
        }
  ...
  return {
    ...
    quietWaitMs,
    videoSteps,
    createdWorkflowCleanup: ctx.createdWorkflowController
      ? ctx.createdWorkflowController.cleanupEvidence()
      : null,
  };
}
