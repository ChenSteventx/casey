...
export function parseReplayArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (argument === '--created-workflow-authority') options.createdWorkflowAuthority = argv[++index];
    else if (argument === '--replay-grant') options.replayGrant = argv[++index];
    else if (argument === '--replay-grant-ledger') options.replayGrantLedger = argv[++index];
    else if (argument === '--flow') options.flow = argv[++index];
    else if (argument === '--case-meta' || argument === '--testcase') options.testcase = argv[++index];
    else if (argument === '--compile-provenance') options.compileProvenance = argv[++index];
    else if (argument === '--batch-token') options.batchToken = argv[++index];
