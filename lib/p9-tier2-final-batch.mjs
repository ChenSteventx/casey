...
export function createP9Tier2BatchContext({ batchId, batchToken, memberIds } = {}) {
  const ids = Array.isArray(memberIds) ? memberIds : [];
  if (typeof batchId !== 'string' || !batchId || typeof batchToken !== 'string' || !batchToken
    || ids.length !== CASE_IDS.length || new Set(ids).size !== CASE_IDS.length
    || !CASE_IDS.every((caseId) => ids.includes(caseId))) {
    return { ok: false, reason: 'P9_TIER2_BATCH_CONTEXT_INVALID' };
  }
  const cleanup = Object.fromEntries(CLEANUP_CASE_IDS.map((caseId, index) => {
    const uniqueNameToken = `${batchToken}-case-${index + 1}`;
    return [caseId, Object.freeze({
      caseId,
      batchId,
      batchToken,
      uniqueNameToken,
      derivedEntityName: `atl_${uniqueNameToken}`,
    })];
  }));
  return Object.freeze({
    ...
    batchId,
    batchToken,
    memberIds: Object.freeze([...CASE_IDS]),
