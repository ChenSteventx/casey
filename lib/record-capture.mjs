...
const ALLOWED_ACTIONS = new Set(['click', 'dblclick', 'fill', 'press', 'nav']);
...
const SENSITIVE_FIELD_RE = /(password|passwd|passcode|pass|pwd|token|secret|credential|authorization|cookie|apikey|api-key|x-api-key|otp|captcha|cvv|\bpin\b|密码|口令|密钥|密保|验证码|校验码|动态码|短信码)/i;
...
function hasEmbeddedScheme(str) {
  return str.includes('://') || /%3a%2f%2f/i.test(str);
}
...
export function sanitizeRecordEvent(input, index = 0) {
  ...
  const action = ALLOWED_ACTIONS.has(raw.action) ? raw.action : 'click';
  const out = {
    seq: index + 1,
    action,
