...
function defectTicketHtml(stepId, dt) {
  ...
  const traceRef = dt.traceRef
    ? safeRelMediaPath(dt.traceRef)
      ? `<div class="attach">trace：<a href="${escHtml(dt.traceRef)}" download>${escHtml(dt.traceRef)}</a></div>`
      : '<div class="attach">trace：路径非法，已拒绝显示</div>'
    : '';
...
function attachmentsHtml(step) {
  ...
  if (at.trace) parts.push(safeRelMediaPath(at.trace)
    ? `trace <a href="${escHtml(at.trace)}" download>${escHtml(at.trace)}</a>`
    : 'trace 路径非法，已拒绝显示');
  if (at.stepPage) parts.push(safeRelMediaPath(at.stepPage)
    ? `子页 <a href="${escHtml(at.stepPage)}">${escHtml(at.stepPage)}</a>`
    : '子页路径非法，已拒绝显示');
...
function renderHtml(model, diag) {
  ...
  return htmlDoc(model.title || model.caseId, head + naturalLanguageHtml(model) + atomicStepsHtml(model) + replayVideoHtml(model) + artifactsHtml(model) + visualReviewHtml(model) + summary + banner + cleanupEvidenceHtml(model) + workflowStructureHtml(model) + groups + diagnosticsHtml(diag), diag.any ? DIAG_CSS : '');
}
...
function renderJson(model) {
  ...
  if ('replayVideo' in model) projection.replayVideo = model.replayVideo == null
    ? null
    : { ...model.replayVideo, file: safeRelMediaPath(model.replayVideo.file) ? model.replayVideo.file : null };
  if (Array.isArray(model.artifacts)) projection.artifacts = model.artifacts.map((artifact) => ({
    ...artifact,
    file: safeRelMediaPath(artifact?.file) ? artifact.file : null,
  }));
  if ('visualReview' in model) projection.visualReview = model.visualReview == null
    ? null
    : {
        ...model.visualReview,
        evidence: Array.isArray(model.visualReview.evidence)
          ? model.visualReview.evidence.map((evidence) => ({
              ...evidence,
              file: safeRelMediaPath(evidence?.file) ? evidence.file : null,
            }))
          : [],
      };
...
function aggCaseHtml(c) {
  ...
  const reportHref = safeRelMediaPath(c.reportHref) ? c.reportHref : null;
  const title = reportHref ? `<a href="${escHtml(reportHref)}">${escHtml(key)}</a>` : escHtml(key);
...
export function renderAggregate(m) {
  ...
  // 上游装配器已拒绝危险 reportHref；渲染边界仍重建一份安全模型，避免直接调用渲染器时
  // javascript:/绝对路径/穿越值进入 HTML 或 JSON 旁车。
  const safeModel = {
    ...m,
    groups: m.groups.map((group) => ({
      ...
      cases: (group.cases || []).map((entry) => ({
        ...entry,
        reportHref: safeRelMediaPath(entry?.reportHref) ? entry.reportHref : null,
      })),
    })),
  };
  return {
    html: renderAggregateHtml(safeModel),
    markdown: renderAggregateMd(safeModel),
    json: JSON.stringify(safeModel, null, 2) + '\n',
  };
}
