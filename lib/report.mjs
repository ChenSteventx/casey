...
function replayVideoHtml(model) {
  ...
  const body = v && v.file ? `<video src="${escHtml(v.file)}" controls></video>` : '<p class="no-video">无录像</p>';
...
function renderMarkdown(model, diag) {
  ...
  if ('replayVideo' in model) {
    lines.push('');
    lines.push('## 回放录像');
    lines.push(model.replayVideo && model.replayVideo.file ? `[${model.replayVideo.file}](${model.replayVideo.file})` : '无录像');
  }
  if (Array.isArray(model.artifacts)) {
    lines.push('', '## 附件');
    model.artifacts.forEach((a) => lines.push(`- [${a.label || a.kind}](${a.file}) \`${a.file}\``));
  }
  if ('visualReview' in model) {
    ...
    if (!model.visualReview) lines.push('待视觉复核；不得据此声称真机验收完成。');
      ...
      (v.evidence || []).forEach((e) => lines.push(`  - [${e.file}](${e.file})${e.videoAt == null ? '' : ` · ${fmtMs(e.videoAt)}`}${e.note ? ` · ${e.note}` : ''}`));
    }
  }
