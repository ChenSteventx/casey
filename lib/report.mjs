...
function renderHtml(model, diag) {
  ...
  return htmlDoc(model.title || model.caseId, head + naturalLanguageHtml(model) + atomicStepsHtml(model) + replayVideoHtml(model) + summary + banner + cleanupEvidenceHtml(model) + workflowStructureHtml(model) + groups + diagnosticsHtml(diag), diag.any ? DIAG_CSS : '');
}
...
function aggCaseHtml(c) {
  ...
  return `<section class="agg-case" id="case-${escHtml(safeSlug(key))}"><h3>${escHtml(key)} <code>${escHtml(c.caseId)}</code></h3><div class="meta">${summ}</div>${prompt}${soft}</section>`;
}
