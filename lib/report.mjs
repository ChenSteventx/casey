...
function renderHtml(model, diag) {
  ...
  return htmlDoc(model.title || model.caseId, head + naturalLanguageHtml(model) + atomicStepsHtml(model) + replayVideoHtml(model) + summary + banner + cleanupEvidenceHtml(model) + groups + diagnosticsHtml(diag), diag.any ? DIAG_CSS : '');
}
