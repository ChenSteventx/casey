...
async function compileAtomStep(run, { atom, params = {} }) {
  ...
  if (atom === 'workflow.publish') return compileWorkflowPublish(run);
  if (atom === 'workflow.clickEditorButton') return compileWorkflowClickEditorButton(run, params);
  if (atom === 'workflow.closeDrawer') return compileWorkflowCloseDrawer(run);
