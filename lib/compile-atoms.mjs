...
import { requiresTargetContinuityRef } from './entity-destructive-continuity.mjs';
...
export function createCompileRun({ page, forensics, state, sut, uniqueName, site, listRoute, agentListRoute, profile, identityLedger }) {
  return {
...
async function compileAgentToolRecipe(run, atom, params) {
  ...
  armDestructiveTargetContinuity(run, atom, params.name ?? params.toolName ?? null);
  return results;
}
...
function armDestructiveTargetContinuity(run, atom, name) {
  ...
  const observation = (run.identityObservations || []).find(
    (row) => row && typeof row === 'object' && row.platformId && row.name === targetName,
  );
