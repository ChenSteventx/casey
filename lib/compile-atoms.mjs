...
export function createCompileRun({ page, forensics, state, sut, uniqueName, site, listRoute }) {
  return {
        ...
        if (customAct) {
          ...
          if (candidateCount === 1) {
            ...
            if (optCount === 1) {
              ...
              resolution = optCount > 1 ? 'multi' : 'absent';
...
async function compileAtomStep(run, { atom, params = {} }) {
  ...
  // chat 维度（chiefcomplaint-smoke，飞轮第二条）：
  if (atom === 'nav.agentManagement') return compileNavAgentManagement(run);
  if (atom === 'agent.searchOpen') return compileAgentSearchOpen(run, params);
  if (atom === 'agent.openTestPanel') return compileAgentOpenTestPanel(run);
  if (atom === 'chat.sendAndWait') return compileChatSendAndWait(run, params);
  if (atom === 'chat.closeTestPanel') return compileChatCloseTestPanel(run);
...
async function compileNavAgentManagement(run) {
  ...
  // 先落壳页（登录预备动作后会话已在；hermetic --skip-login 时假 SUT 首页直出侧栏），再点侧栏。
  await run.emit({ intentId: i, atom: 'nav.agentManagement', action: 'nav', url: '{{baseUrl}}/' });
  await run.emit({ intentId: i, atom: 'nav.agentManagement', action: 'click', text: '智能体管理', fallbackCss: AGENT_MENU_ITEM_CSS });
  // 后置条件（registry：provides 在智能体管理页）：搜索框可见；SPA 路由慢时给足窗，锚不上由下一步计数如实暴露。
  await run.page.getByRole('textbox', { name: AGENT_SEARCH_NAME }).waitFor({ state: 'visible', timeout: 30000 }).catch(() => {});
}
...
async function compileAgentSearchOpen(run, params) {
  ...
  await run.emit({ intentId: i, atom: 'agent.searchOpen', action: 'click', semantic: { kind: 'text', name: params.openName, exact: false }, text: params.openName });
  // 后置条件（registry：provides 智能体详情已开）：「测试」按钮可见。
...
async function compileAgentOpenTestPanel(run) {
  const i = run.newIntent();
  await run.emit({ intentId: i, atom: 'agent.openTestPanel', action: 'click', semantic: { kind: 'role', role: 'button', name: '测试', exact: true }, text: '测试' });
  // regress 的「点空重点一次」自愈不搬（回放器守卫不抛、点击身份门管）；面板没开由后续步计数如实暴露。
  await run.page.getByRole('textbox', { name: '请输入消息' }).waitFor({ state: 'visible', timeout: 30000 }).catch(() => {});
}
...
async function compileChatSendAndWait(run, params) {
  ...
  const MSG = { kind: 'role', role: 'textbox', name: '请输入消息', exact: true };
  ...
  await run.emit({ intentId: i, atom: 'chat.sendAndWait', action: 'click', fallbackCss: params.sendIconSelector || CHAT_SEND_ICON });
  ...
  const replyText = await waitReplyStableCompile(run.page, replySelector);
