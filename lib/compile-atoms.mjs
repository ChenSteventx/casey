...
async function compileWorkflowDelete(run, params) {
  ...
  const searchSpec = { semantic: { kind: 'role', role: 'textbox', name: SEARCH_BOX_NAME }, fieldLabel: null };
  await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'fill', semantic: searchSpec.semantic, value: params.name });
  await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'press', semantic: searchSpec.semantic, key: 'Enter' });
