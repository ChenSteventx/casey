...
async function compileWorkflowAddNode(run, params) {
  ...
  const exactRe = new RegExp(`^${nodeName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`);
  const panelItem = () => run.page.locator('.node-item').filter({ hasText: exactRe });
  ...
  const n = await panelItem().count().catch(() => null);
  if (n !== 1) {
    run.blockers.push(`workflow.addNode 源面板项「${nodeName}」count=${n ?? '证不出'} 非唯一/缺席 → 硬阻断 fail-closed 不拖（route:human）`);
    run.notes.push(run.blockers[run.blockers.length - 1]);
    return;
  }
  await run.emit({ intentId: i, atom: 'workflow.addNode', action: 'dragTo', semantic: { kind: 'text', name: nodeName, exact: true }, text: nodeName, ox, oy }, async () => {
    const graph = run.page.locator('.lf-graph');
    if ((await graph.count()) !== 1) throw new Error('画布容器 .lf-graph 非唯一/缺席（证不出）');
    const gbox = await graph.boundingBox();
    const sbox = await panelItem().first().boundingBox();
    ...
    try {
      await run.page.mouse.move(gbox.x + ox, gbox.y + oy, { steps: 12 });
      await run.page.waitForTimeout(300);
    } finally { await run.page.mouse.up(); }
  });
  // 后置核验（registry post：画布节点数 +1）：计数不 +1（含出界未落、基线/终值证不出）→ blockers 硬阻断。
  const after = await nodeCount();
  if (before == null || after == null || after !== before + 1) {
    run.blockers.push(`workflow.addNode「${nodeName}」后置核验 .lf-node 计数 ${before ?? '证不出'}→${after ?? '证不出'} 非 +1 → 硬阻断（fail-closed，落点不 clamp、落不进如实阻断）`);
    run.notes.push(run.blockers[run.blockers.length - 1]);
    return;
  }
  run.notes.push(`workflow.addNode「${nodeName}」.lf-node 计数 ${before}→${after}（+1 后置核验过）`);
}
