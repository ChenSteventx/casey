...
import { requiresTargetContinuityRef, selectObservationForDestructiveTarget } from './entity-destructive-continuity.mjs';
...
// 在册命名断言原子：与 assert.* 同属“只留断言意图、不产动作 event”的特殊分支。
// 不放进 COMPILE_ATOM_COMPILERS，避免纯断言伪装成可执行动作并抬高动作编译器计数。
...
export function createCompileRun({ page, forensics, state, sut, uniqueName, site, listRoute, agentListRoute, profile, identityLedger, identityProfileDigest }) {
  return {
    ...
    if (run.events.length > firstEvent) {
      ...
      if (run.pendingIdentityObservation) {
        ...
        if (!pendingObservation.evidenceStepId || !primaryBinding || !step.sourceIntentId) {
          run.blockers.push(`agent.searchOpen 身份观察归档不齐（evidenceStepId=${pendingObservation.evidenceStepId ?? 'null'}、binding=${primaryBinding ? '有' : '缺'}、sourceIntentId=${step.sourceIntentId ?? '缺'}）→ 硬阻断，绝不静默降级出无观察产物`);
          ...
          run.identityObservations.push({
            ...
            sourcePath: (run.profile && run.profile.agents && run.profile.agents.listApi && run.profile.agents.listApi.pathname) || 'unknown',
          });
        }
      }
    }
...
async function compileAgentToolRecipe(run, atom, params) {
  ...
  // C3 破坏性/targeting 原子（agent.delete / picker.selectFirstTool）编译处武装目标连续性 ref（gated、加法式）。
  // High-2：picker.selectFirstTool 真参数是 under（非 name/toolName），补进目标名解析，别恒拿不到目标名。
  armDestructiveTargetContinuity(run, atom, params.name ?? params.toolName ?? params.under ?? null);
  return results;
}
...
function armDestructiveTargetContinuity(run, atom, name) {
  ...
  const selected = selectObservationForDestructiveTarget({ observations: run.identityObservations || [], targetName });
  if (!selected.ok) {
...
async function compileWorkflowDelete(run, params) {
  ...
  // C3 破坏性 workflow.deleteByName 编译处武装目标连续性 ref（gated、加法式；确认落笔后武装）。
  armDestructiveTargetContinuity(run, 'workflow.deleteByName', params.name);
