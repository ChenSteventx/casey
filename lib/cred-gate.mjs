...
export const FORBIDDEN_KEYWORDS = ['authorization', 'set-cookie', 'password', 'apikey', 'x-api-key', 'bearer ', 'secret', 'credential'];
...
//   - successField/successValue/background：通道剖面三件（CONTEXT 定义 = 非凭据配置，与 site.json 凭据
//     严格分离），observed 的 errorEnvelope.field 与背景轮询 URL 必含。
// 密钥/口令/站点地址类字面量（含 p7 golden 钉的 value 收集语义）照收不误。
const NON_CREDENTIAL_KEYS = new Set(['role', 'successField', 'successValue', 'background']);
export function collectSecretLiterals() {
  ...
  for (const f of files) {
    ...
    (function walk(n, key) {
      ...
      if (typeof n === 'string' && n.length >= 6 && !NON_CREDENTIAL_KEYS.has(key)) lits.add(n);
    })(j, null);
  }
  return [...lits];
}
