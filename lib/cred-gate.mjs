...
export function collectSecretLiterals() {
  ...
  // 医生站 / Hi 小助账户引用也是本地敏感标识：只收 accountRef，不把固定能力字段误当凭据字面量。
  const desktopAccountFile = join(AUTH_DIR, 'desktop-account.json');
  if (existsSync(desktopAccountFile)) {
    try {
      const j = JSON.parse(readFileSync(desktopAccountFile, 'utf8'));
      if (typeof j?.accountRef === 'string' && j.accountRef.length >= 6) lits.add(j.accountRef);
    } catch { /* 坏本地账户配置由 account status / doctor 报形状无效 */ }
  }
  return [...lits];
}
