...
export async function resolveAgentCardTarget(page, { mode = AGENT_IDENTITY_MODE_LEGACY, openName, containerSelector, cardFields } = {}) {
  const name = typeof openName === 'string' ? openName : '';
  if (!name.trim() || !containerSelector || !cardFields
    || typeof cardFields.name !== 'string'
    || ![AGENT_IDENTITY_MODE_LEGACY, AGENT_IDENTITY_MODE_NETWORK_CODE].includes(mode)
    || (mode === AGENT_IDENTITY_MODE_LEGACY && typeof cardFields.code !== 'string')
    || (mode === AGENT_IDENTITY_MODE_NETWORK_CODE && Object.prototype.hasOwnProperty.call(cardFields, 'code'))) {
  ...
  try {
    ...
    const code = mode === AGENT_IDENTITY_MODE_LEGACY
      ? await card.evaluate((el, codeSel) => {
...
export async function clickAgentCardWithin(card, { mode = AGENT_IDENTITY_MODE_LEGACY, cardFields, name, code, timeoutMs = 10000 } = {}) {
  if (!card || typeof card.evaluate !== 'function' || typeof card.$ !== 'function'
    || !cardFields || typeof name !== 'string'
    || ![AGENT_IDENTITY_MODE_LEGACY, AGENT_IDENTITY_MODE_NETWORK_CODE].includes(mode)
    || (mode === AGENT_IDENTITY_MODE_LEGACY && typeof code !== 'string')
    || (mode === AGENT_IDENTITY_MODE_NETWORK_CODE && Object.prototype.hasOwnProperty.call(cardFields, 'code'))) return false;
  const stillSame = await card
    .evaluate((el, { skipCodeCheck, nameSel, codeSel, wantedName, wantedCode }) => {
      if (!el.isConnected) return false;
      if (!(el.getClientRects().length > 0)) return false;
      const nameNode = el.querySelector(nameSel);
      if (!nameNode || nameNode.textContent !== wantedName) return false;
      if (skipCodeCheck) return true;
      const codeNode = el.querySelector(codeSel);
      return !!codeNode && codeNode.textContent === wantedCode;
    }, {
      skipCodeCheck: mode === AGENT_IDENTITY_MODE_NETWORK_CODE,
      nameSel: cardFields.name,
      codeSel: cardFields.code,
      wantedName: name,
      wantedCode: code,
    })
    .catch(() => false);
