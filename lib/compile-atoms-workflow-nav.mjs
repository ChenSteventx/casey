...
export async function compileWorkflowOpen(run, params) {
  ...
  const openProbe = run.page.getByText(instantiate(params.openName, run.ctx), { exact: true });
  ...
  if (!openTargetSeen && (await openSearch.count().catch(() => 0)) >= 1) {
    ...
    await run.emit({ intentId: i, atom: 'workflow.open', action: 'click', fieldLabel: '搜索', fallbackCss: '.hr-input__suffix .search-icon' });
    openSearched = true;
  }
  ...
  // 条件预算（评审两轮 pi Medium 方向的失败路径总额封顶）：已见目标或真发起了搜索才给本锚新
  // 预算 15000；否则前奏已为同一目标等满、本锚让行——「双缺席」失败路径总额恒 ~15s 不叠加。
  const openDeadline = (openTargetSeen || openSearched) ? Date.now() + 15000 : Date.now();
  while (Date.now() < openDeadline) {
    if ((await openTarget.count().catch(() => 0)) >= 1) break;
    await sleep(250);
  }
