...
import { ROUTE_LIST, SEARCH_BOX_NAME, auditDeleteCount, sleep } from './compile-atoms-support.mjs';

async function readCreatedWorkflowIdentity(run, { evidenceStepId, name }) {
  const entityName = instantiate(name, run.ctx);
  let lastReason = 'CREATED_WORKFLOW_LIST_READBACK_EMPTY';
  for (const waitMs of [0, 500, 1500]) {
    if (waitMs > 0) await run.page.waitForTimeout(waitMs);
    const fetched = await fetchCreatedWorkflowListScan(run.page, {
      profile: run.profile,
      entityName,
    });
    if (!fetched.ok) {
    ...
    const scan = fetched.scan;
    const matches = scan.complete === true
      ? scan.records.filter((row) => row.name === entityName)
      : [];
    if (matches.length !== 1) {
      lastReason = matches.length > 1
        ? 'CREATED_WORKFLOW_LIST_READBACK_AMBIGUOUS'
        : 'CREATED_WORKFLOW_LIST_READBACK_ABSENT';
      continue;
    }
    run.identityGateOutcome = { resolution: 'unique', reason: null };
    return {
      matched: {
        name: matches[0].name,
        code: matches[0].code,
      ...
      evidenceStepId,
      kind: 'workflow',
      sourcePath: run.profile.workflows.listApi.pathname,
    };
  }
  run.identityGateOutcome = { resolution: 'action_failed', reason: lastReason };
  run.blockers.push(`workflow.create 后平台列表未能唯一读回当轮工作流（${lastReason}）→ 硬阻断 fail-closed，不继续删除链`);
  run.notes.push(run.blockers[run.blockers.length - 1]);
  return null;
}

function compileMutationAdapter(profile) {
  const adapter = profile?.workflows?.mutationAdapter;
  if (!adapter || typeof adapter !== 'object' || Array.isArray(adapter)
    || typeof adapter.method !== 'string' || adapter.method !== adapter.method.toUpperCase()
    || typeof adapter.path !== 'string' || !adapter.path.startsWith('/')
    || typeof adapter.idLocation !== 'string'
    || !/^(?:body|query)(?:\.[A-Za-z][A-Za-z0-9_]*)+$/.test(adapter.idLocation)) return null;
  return { method: adapter.method, path: adapter.path, idLocation: adapter.idLocation };
}

function mutationIdOccurrences(url, body) {
  const out = [];
  try {
    const parsed = new URL(url, 'http://casey.invalid');
    for (const [key, value] of parsed.searchParams) {
      if (/id$/i.test(key)) out.push({ location: `query.${key}`, value });
    }
  } catch {
    out.push({ location: 'query.__invalid__', value: null });
  }
  const walk = (value, prefix, seen = new Set()) => {
    if (value == null || typeof value !== 'object' || seen.has(value)) return;
    seen.add(value);
    if (Array.isArray(value)) {
      value.forEach((child, index) => walk(child, `${prefix}.${index}`, seen));
      return;
    }
    for (const [key, child] of Object.entries(value)) {
      const location = `${prefix}.${key}`;
      if (/id$/i.test(key)) out.push({ location, value: child });
      walk(child, location, seen);
    }
  };
  walk(body, 'body');
  return out;
...
async function installCompileWorkflowDeletionGuard(page, { adapter, platformId }) {
  let decide;
  ...
  const handler = async (route) => {
    ...
    const path = (() => { try { return new URL(request.url()).pathname; } catch { return null; } })();
    ...
    const ids = mutationIdOccurrences(request.url(), body);
    const ok = request.method() === adapter.method
      && ids.length === 1
      && ids[0].location === adapter.idLocation
      && typeof ids[0].value === 'string'
      && ids[0].value === platformId;
    finish({ ok, reason: ok ? null : 'COMPILE_WORKFLOW_DELETE_REQUEST_MISMATCH' });
...
export async function compileWorkflowCreate(run, params) {
  ...
  // 入口就绪锚（wf-create-entry-anchor-wait）：列表页 SPA 在 load 后仍有渲染尾巴（2026-08-06 真机
  // 实测经隧道 load 11.4s、按钮 +3.5s 才可定位），emit 的定位窗（quietPoint 2.5s + 附着 1.5s）压线必抖。
  // 对同一语义锚做有界轮询：可定位即点；预算耗尽不改判——照旧发 emit，由身份门如实记 absent
  // （失败路径零行为差，fail-closed 语义不动）。
  const entryBtn = run.page.getByRole('button', { name: '新增工作流', exact: true });
  const entryDeadline = Date.now() + 15000;
  while (Date.now() < entryDeadline) {
    if ((await entryBtn.count().catch(() => 0)) >= 1) break;
    await sleep(250);
  }
  await run.emit({
  ...
  if (confirmName) {
    ...
    if (footN >= 1) {
      ...
      await run.emit({ intentId: iCreate, atom: 'workflow.create', action: 'click', text: footText, fallbackCss: footCss, compilePhase: 'terminal' });
    } else {
  ...
  if (run.identityLedger && run.profile && run.profile.workflows && run.profile.workflows.listApi) {
    ...
    const readback = await readCreatedWorkflowIdentity(run, {
      evidenceStepId: terminalStep ? terminalStep.stepId : null,
      name: params.name,
    });
...
export async function compileWorkflowDelete(run, params) {
  ...
  // 就绪锚（post-nav-anchor-wait）：nav 后搜索框渲染尾巴竞态（四跑实证 count=0 误落 CASE_DEFECT 候选）。
  // 有界轮询；预算 15000 耗尽不改判——真缺席照走下方 CASE_DEFECT 候选分支（零行为差）。
  const searchDeadline = Date.now() + 15000;
  while (Date.now() < searchDeadline) {
    if ((await search.count().catch(() => 0)) >= 1) break;
    await sleep(250);
  }
  ...
  // seam-1（wf-open-search-first 七跑实证 + 清偿链实战同姿势 + 读回门契约同源）：Enter 不过滤、
  // 放大镜才过滤——真过滤靠下方放大镜 click；未过滤列表缺目标卡时计数门必 unknown 截断
  // （十一跑实证）。press Enter 保留：真机十一跑已证其惰性无副作用，且冻结金牌
  // post-nav-anchor-wait S2 以包含式钉 fill/press 照发（三 PRD 冻结面）——实现让路
  // 零字面变更（semantic-name 首版教训先例），不动冻结件不回人签。
  await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'press', semantic: searchSemantic, key: 'Enter' });
  // 纯 fallbackCss 照 chat.sendAndWait 送出图标先例（防未来同名 label 抢锚）。
  const filterClick = await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'click', fallbackCss: '.hr-input__suffix .search-icon' });
  // 过滤后就绪锚（post-nav-anchor-wait 家族第四处）：真过滤（unique && acted）才授新预算，对
  // 「目标文本落在记录容器内」有界轮询（容器语义同 open 前奏三候选）；预算耗尽不改判——
  // 审计照跑、既有 fail-closed 路径零行为差。
  if (filterClick.resolution === 'unique' && filterClick.acted === true) {
    const delTarget = run.page.locator('.hr-table-row, .hr-card.hr-card--bordered, .agent-card').getByText(instantiate(params.name, run.ctx), { exact: true });
    const filterDeadline = Date.now() + 15000;
    while (Date.now() < filterDeadline) {
      if ((await delTarget.count().catch(() => 0)) >= 1) break;
      await sleep(250);
    }
  }
  ...
  const mutationAdapter = compileMutationAdapter(run.profile);
  if (!mutationAdapter || typeof delRef.ref?.platformId !== 'string') {
    run.blockers.push('workflow.deleteByName 缺精确 mutation adapter 或当轮字符串 platformId → 确认删除前硬阻断');
    run.notes.push(run.blockers[run.blockers.length - 1]);
    return;
  }
  const outboundGuard = await installCompileWorkflowDeletionGuard(run.page, {
    adapter: mutationAdapter,
    platformId: delRef.ref.platformId,
  });
  const confirmResult = await run.emit(
    { intentId: iDel, atom: 'workflow.deleteByName', action: 'click', semantic: { kind: 'role', role: 'button', name: confirm.confirmName, exact: true }, text: confirm.confirmName, value: params.name, compilePhase: 'terminal' },
    () => performWorkflowDeleteConfirm(run.page, confirm.confirmName, instantiate(params.name, run.ctx)),
  );
  const outboundDecision = await Promise.race([
    outboundGuard.decision,
    run.page.waitForTimeout(5000).then(() => ({ ok: false, reason: 'COMPILE_WORKFLOW_DELETE_REQUEST_NOT_OBSERVED' })),
  ]);
  await outboundGuard.close().catch(() => {});
  if (outboundDecision.ok !== true) {
    run.blockers.push(`workflow.deleteByName 出站精确 ID 守卫未放行（${outboundDecision.reason}）→ 请求已在 SUT 前中止`);
    run.notes.push(run.blockers[run.blockers.length - 1]);
    return;
  }
  ...
  // 重搜（删后归零断言的取数前提，属动作不属断言）。seam-1 同款：Enter 不触发重查，
  // 放大镜才触发——不然下游「删后归零」断言读到的是陈旧列表（假 SUT_DEFECT 向）。
  await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'fill', semantic: searchSemantic, value: params.name });
  await run.emit({ intentId: iDel, atom: 'workflow.deleteByName', action: 'click', fallbackCss: '.hr-input__suffix .search-icon' });
}
