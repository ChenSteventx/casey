...
export function checkIdentityObservationCardinality({ events, observations, registry = ENTITY_OBSERVATION_REGISTRY } = {}) {
  ...
  for (const stepId of terminalStepIds) {
    const entry = registry.get(clickByStepId.get(stepId).atom);
    if (!entry || !Array.isArray(entry.requiredRoles) || entry.requiredRoles.length === 0) return rej('OBSERVATION_REGISTRY_ENTRY_INVALID');
    if ((obsCountByTerminalStep.get(stepId) || 0) !== entry.requiredRoles.length) return rej('OBSERVATION_TERMINAL_WITHOUT_MATCHING_OBSERVATION');
  }
