...
// 观察基数门（codex R1-C1 的 kind-无关泛化 + codex round-2 High-1 反向基数封缝）：声明身份通道时，观察行与
// 【身份定向终端 click】之间须是【双向双射】——旧实现只验「观察→某 click」单向（每观察 evidenceStepId ∈ 已发 click
// stepId 集、互不重复），丢了反向与 atom 对应，令三案 fail-open 返 ok:true（codex 亲验）：①有身份定向 click 却 0 观察
// （静默产无观察件）；②两身份 click 只一个观察（漏一条身份读回）；③观察锚到【别原子】的 click（atom 张冠李戴）。
//
// ── 双向双射判据 ──────────────────────────────────────────────────────────────────────────────
// (正向 观察→click，含 atom 对应)：每条观察行的 evidenceStepId 须锚到一个真实 click 事件，且该 click 的 atom
//   须【逐字等于】观察行自报 atom（关死 ③ 跨原子锚定）、且该 atom 须登记为观察原子（观察不得自报非身份原子）。
//   evidenceStepId 互不重复（一 click 至多担一观察）。
// (反向 click→观察，按 intent 分组)：每个「身份定向终端 intent」（存在 ≥1 个 atom 登记为观察原子的 click 事件的
//   intentId，按 registry 判定）须恰被 requiredRoles 数量的观察行覆盖（关死 ①②）。为何按 intent 而非按 click 分组：
//   agent.searchOpen 每步一 intent 一 click（fill/press/click 同 intent，仅 click 是终端）→ 每 intent 恰一终端
//   click 恰一观察，与旧「agent.searchOpen click 数 == 观察数」逐字等价、零回归；workflow.create 单步一 intent 多
//   click（新增/分类/确认同 iCreate，仅确认 click 武装观察）→ 该 intent 内多个 workflow.create click 收敛为一个
//   终端义务、恰一观察，不误杀（wiring 金牌 H1b：ic 内两 workflow.create click + 一观察 → ok）。intent 分组即
//   「多 click 收敛单终端」与「单 click 单终端」的统一判据，其反向基数严等原 agent 门（不松不紧）。
//   身份定向 click 缺 intentId → fail-closed（无从按 intent 收敛，OBSERVATION_TERMINAL_INTENT_MISSING）。
export function checkIdentityObservationCardinality({ events, observations, registry = ENTITY_OBSERVATION_REGISTRY } = {}) {
  if (!Array.isArray(events) || !Array.isArray(observations)) {
    return Object.freeze({ ok: false, reason: 'OBSERVATION_CARDINALITY_INPUT_MALFORMED' });
  }
  // 1) click 事件按 stepId 索引（同 stepId 两 click = 畸形，fail-closed）。仅 click 参与身份终端判定。
  const clickByStepId = new Map();
  for (const e of events) {
    if (!e || typeof e !== 'object' || e.action !== 'click') continue;
    if (!isNonEmptyString(e.stepId)) return Object.freeze({ ok: false, reason: 'OBSERVATION_CARDINALITY_INPUT_MALFORMED' });
    if (clickByStepId.has(e.stepId)) return Object.freeze({ ok: false, reason: 'OBSERVATION_CLICK_STEPID_DUPLICATE' });
    clickByStepId.set(e.stepId, e);
  }
  // 2) 身份定向终端义务（反向源）：atom 登记为观察原子的 click 按 (intentId, atom) 归组。非身份原子 click（孤儿
  //    policy 如 workflow.saveDraft）不产义务。身份定向 click 缺 intentId → fail-closed（无从收敛终端）。
  //    terminalIntents: Map<atom, Set<intentId>>。
  const terminalIntents = new Map();
  for (const e of clickByStepId.values()) {
    if (!isNonEmptyString(e.atom) || !registry.has(e.atom)) continue; // 未登记 atom：无身份义务（D4 不误拒）。
    if (!isNonEmptyString(e.intentId)) return Object.freeze({ ok: false, reason: 'OBSERVATION_TERMINAL_INTENT_MISSING' });
    if (!terminalIntents.has(e.atom)) terminalIntents.set(e.atom, new Set());
    terminalIntents.get(e.atom).add(e.intentId);
  }
  // 3) 正向逐观察行校验（evidenceStepId 锚定 + atom 对应 + 唯一）+ 按 (intentId, atom) 计观察数（反向计数用）。
  const seenEvidence = new Set();
  const obsByIntentAtom = new Map(); // key `${intentId} ${atom}` → 观察计数
  for (const o of observations) {
    if (!o || typeof o !== 'object' || !isNonEmptyString(o.evidenceStepId)) {
      return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_STEP_MISSING' });
    }
    if (!isNonEmptyString(o.atom)) return Object.freeze({ ok: false, reason: 'OBSERVATION_ATOM_MISSING' });
    if (seenEvidence.has(o.evidenceStepId)) return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_STEP_DUPLICATE' });
    seenEvidence.add(o.evidenceStepId);
    const click = clickByStepId.get(o.evidenceStepId);
    if (!click) return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_STEP_NOT_A_TERMINAL_CLICK' });
    // atom 对应（关死 ③）：观察 evidenceStepId 锚定的 click 其 atom 须逐字等于观察自报 atom，且须为登记观察原子。
    if (click.atom !== o.atom) return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_ATOM_MISMATCH' });
    if (!registry.has(click.atom)) return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_ATOM_UNREGISTERED' });
    // click 已登记 atom → 其 intentId 必已在 terminalIntents（步 2 收集），isNonEmptyString 已保。
    const key = `${click.intentId} ${click.atom}`;
    obsByIntentAtom.set(key, (obsByIntentAtom.get(key) || 0) + 1);
  }
  // 4) 反向基数（关死 ①②）：每身份定向终端 intent 须恰被 requiredRoles 数量观察行覆盖（少=漏读回、多=越供）。
  for (const [atom, intents] of terminalIntents) {
    const entry = registry.get(atom);
    if (!entry || !Array.isArray(entry.requiredRoles) || entry.requiredRoles.length === 0) {
      return Object.freeze({ ok: false, reason: 'OBSERVATION_REGISTRY_ENTRY_INVALID' });
    }
    const need = entry.requiredRoles.length;
    for (const intentId of intents) {
      if ((obsByIntentAtom.get(`${intentId} ${atom}`) || 0) !== need) {
        return Object.freeze({ ok: false, reason: 'OBSERVATION_TERMINAL_WITHOUT_MATCHING_OBSERVATION' });
      }
    }
  }
