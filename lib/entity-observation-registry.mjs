...
export const ENTITY_OBSERVATION_REGISTRY = new Map([
  ...
  // C2 workflow source 读回观察原子（纯数据加条目，引擎 C0 已数据驱动）：workflow 创建/打开即绑为 source
  // （母规格 point 3「read 回 join 到 source binding」），requiredRoles 恰 [source]；issuer 双锚 compile-envelope+本原子；
