...
// 观察基数门（codex R1-C1 的 kind-无关泛化）：声明身份通道时，每条已归档观察行须锚定【唯一】真实终端 click 事件
// （其 evidenceStepId ∈ 已发 click 事件 stepId 集），evidenceStepId 互不重复。多/少/错位/重复 → fail-closed 具名拒。
// 与旧「agent.searchOpen click 数 == 观察数」等价性：agent click 仅在双证通过时发出、且同时武装观察，故 click↔观察
// 一一对应；本门改锚 evidenceStepId 后对 agent 结论不变（1 观察锚 1 click），却对 workflow（1 观察锚 workflow.create
// 确认 click，流内另有多个非终端 workflow.create click）不再误杀。
export function checkIdentityObservationCardinality({ events, observations } = {}) {
  ...
  const clickStepIds = new Set();
  for (const e of events) {
    if (e && typeof e === 'object' && e.action === 'click' && isNonEmptyString(e.stepId)) clickStepIds.add(e.stepId);
  }
  const evidence = [];
  for (const o of observations) {
    ...
    evidence.push(o.evidenceStepId);
  }
  if (new Set(evidence).size !== evidence.length) {
    return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_STEP_DUPLICATE' });
  }
  for (const id of evidence) {
    if (!clickStepIds.has(id)) return Object.freeze({ ok: false, reason: 'OBSERVATION_EVIDENCE_STEP_NOT_A_TERMINAL_CLICK' });
  }
