...
function mapAtom(atom, observed) {
  ...
  switch (atom.atom) {
    ...
    case 'assert.textHidden':
      // assert-visibility-semantics：注册表把本原子定义为「toBeHidden，含未挂载」，采集侧已按可见性
      // 纠偏（lib/replay/intent-observation.mjs）。kind/op 与 bin/check.mjs 词表、IMPLEMENTED_KINDS
      // 一致，故硬映射不落待补、也不得标 soft（标 soft 即绕硬裁定，D2）。
      if (typeof p.text !== 'string' || !p.text) return null;
      return { kind: 'textHidden', op: 'absent', value: p.text };
