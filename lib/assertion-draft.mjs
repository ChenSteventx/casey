...
function mapAtom(atom, observed) {
  ...
  switch (atom.atom) {
    ...
    case 'assert.buttonState':
      // wf-publish-states（D1 翻转）：present/absent 已实现才映射；enabled/disabled 回 null → pending[]
      // route:human（随 publish_blocked 契约带实现回归词表后再收，挂账不静默丢）。
      if (p.state !== 'present' && p.state !== 'absent') return null;
