...
export function runOneCase({ member, sut, exitCodeLegend, authorizedMutation }) {
  ...
  const dir = files.ok ? createExclusiveRunDir(caseId) : null;
  if (!files.ok || !dir) {
    const receipt = baseReceipt({
      caseId, member, authorizedMutation, cls: 'stage_failed', ran: true,
      runDirRel: dir ? dir.runDirRel : null,
      extra: { preflightProblems: files.ok ? ['排他 run 目录创建失败'] : files.problems },
    });
  ...
  if (typeof member.uniqueNameToken === 'string' && member.uniqueNameToken) args.push('--unique-name', member.uniqueNameToken);
...
export async function executeTier2({ parsed, manifest }) {
  ...
  for (const member of selected) {
    ...
    const outcome = runOneCase({ member, sut: parsed.sut, exitCodeLegend, authorizedMutation });
  ...
  return { result: runTier2({ ...envBase, cases }), consumerProblems, judgeEvidence };
}
