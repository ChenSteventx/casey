...
import { screenTier2ReplayGrant, readCreatedWorkflowReplayGrant } from './entity-created-workflow-replay-grant.mjs';
...
export function runOneCase({ member, sut, exitCodeLegend, authorizedMutation, batchContext = null, replayGrantPathAbs = null, replayGrantLedgerRoot = null }) {
  ...
  if (caseContext) {
    args.push(
      ...
      // 回放授权票据与核销台账根（实现审 H2）：与 batch-token 同级必填，
      // 台账根整批共用一个，交给浏览器前门做 launch 前原子占用。
      '--replay-grant', replayGrantPathAbs,
      '--replay-grant-ledger', replayGrantLedgerRoot,
...
// 整批读一次票据：路径由已签清单声明（replayGrantPath），件本身每批重签。
// 读回要把本批各 v3 成员的结构件精确字节一起喂进去——成链是逐例核的，
// 少喂一例就等于那一例没核过链。
export function readBatchReplayGrant(manifest, batchContext) {
  const rel = manifest?.json?.replayGrantPath;
  const cleanupIds = batchContext?.cleanup ? Object.keys(batchContext.cleanup) : [];
  if (!cleanupIds.length) return { grantRead: { ok: true, caseIds: [] }, ledgerRoot: null, grantPathAbs: null, problems: [] };
  if (typeof rel !== 'string' || !rel) {
    return { grantRead: { ok: false, reason: 'REPLAY_GRANT_PATH_NOT_DECLARED' }, ledgerRoot: null, problems: ['清单未声明 replayGrantPath'] };
  }
  const abs = join(PROJECT_ROOT, rel);
  let grantBytes;
  try { grantBytes = readFileSync(abs); }
  catch { return { grantRead: { ok: false, reason: 'REPLAY_GRANT_UNREADABLE' }, ledgerRoot: null, problems: ['回放授权票据不在场或不可读'] }; }
  const cases = [];
  for (const caseId of cleanupIds) {
    const member = (manifest.members || []).find((row) => row.caseId === caseId);
    const authorityAbs = member?.verifiedFiles?.['created-workflow-authority.frozen.json'];
    if (!authorityAbs) {
      return { grantRead: { ok: false, reason: 'REPLAY_GRANT_STRUCTURAL_AUTHORITY_INVALID' }, ledgerRoot: null, problems: [`成员 ${caseId} 的结构授权件不在核过清单里`] };
    }
    try { cases.push({ caseId, authorityBytes: readFileSync(authorityAbs) }); }
    catch { return { grantRead: { ok: false, reason: 'REPLAY_GRANT_STRUCTURAL_AUTHORITY_INVALID' }, ledgerRoot: null, problems: [`成员 ${caseId} 的结构授权件不可读`] }; }
  }
  const grantRead = readCreatedWorkflowReplayGrant({ grantBytes, cases, now: new Date().toISOString() });
  // 台账根与 bin/replay 共用单一根（plan §3.4）：按票据 nonce 归档，批间不串。
  const ledgerRoot = join(PROJECT_ROOT, 'runs', '_tier2', 'replay-grant-ledger');
  return {
    grantRead,
    ledgerRoot,
    grantPathAbs: abs,
    problems: grantRead.ok ? [] : [`回放授权票据未过（${grantRead.reason}）`],
  };
}
...
export async function executeTier2({ parsed, manifest }) {
  ...
  // 回放授权票据：整批读一次（清单声明路径），逐成员筛。
  // 实现审 H1：此前 screenTier2CaseReplayGrant 是死导出——成员循环无条件 spawn，
  // s4 验收因此是假绿。闸必须在这里真调，且拒跑时零 spawn、回执照落。
  const replayGrant = readBatchReplayGrant(manifest, batchContext);
  const cases = [];
  for (const member of selected) {
    ...
    // 只对接了 v3 清理生产链的成员要票（豁免面 plan §4.1：无结构件的成员不要求）。
    if (batchContext?.cleanup?.[member.caseId]) {
      const grantScreen = screenTier2CaseReplayGrant({
        members: [member],
        grantRead: replayGrant.grantRead,
      });
      if (!grantScreen.spawnAllowed) {
        const reason = grantScreen.refusals[0]?.refusalReason || 'replay_grant_absent_or_invalid';
        const refusal = writeRefusalReceipt({ member, refusalReason: reason, authorizedMutation });
        entry.receipt = refusal.receipt;
        entry.receiptPersisted = refusal.receiptWritten;
        entry.projection = null;
        entry.replayGrantRefused = true;
        cases.push(entry);
        continue;   // 零 spawn
      }
    }
    ...
    const outcome = runOneCase({
      ...
      authorizedMutation,
      batchContext,
      replayGrantPathAbs: replayGrant.grantPathAbs,
      replayGrantLedgerRoot: replayGrant.ledgerRoot,
    });
  ...
  consumerProblems.push(...replayGrant.problems);
