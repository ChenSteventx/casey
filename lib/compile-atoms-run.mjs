...
export function createCompileRun({ page, forensics, state, sut, uniqueName, site, listRoute, agentListRoute, profile, identityLedger, identityProfileDigest, executionTargetAuthority, executionTargetRuntime }) {
  ...
  return {
    ...
    // 持久动作失败诊断只留闭集阶段与三态，不留参数、DOM、URL 或身份值。
    persistentActionEvidence: null,
    ...
    async emit(spec, customAct, { persistentActionBoundary = false } = {}) {
          ...
          if (count === 1) {
            ...
            if (originReady) {
              ...
              if (spec.action === 'fill') await first.fill(instantiate(spec.value, this.ctx), { timeout: 3000 });
                ...
                if (persistentActionBoundary) recordPersistentActionEvidence(this, {
                  atom: spec.atom, status: 'INDETERMINATE', stage: 'physical-click-invoked',
                });
                await first.click({ timeout: 3000 });
                if (persistentActionBoundary) recordPersistentActionEvidence(this, {
                  atom: spec.atom, status: 'CONFIRMED', stage: 'physical-click-succeeded',
                });
              }
              acted = true;
            } else {
      ...
      if (persistentActionBoundary) {
        const status = this.persistentActionEvidence?.status;
        recordPersistentActionEvidence(this, status === 'CONFIRMED'
          ? { atom: spec.atom, status, stage: 'click-event-recorded' }
          : status === 'INDETERMINATE'
            ? { atom: spec.atom, status, stage: 'click-event-recorded-indeterminate' }
            : { atom: spec.atom, status: 'NOT_ATTEMPTED', stage: 'click-not-attempted' });
      }
    ...
    async resolveTarget(ev) {
      // 语义名回填（semantic-name-instantiate）：与回放侧 performActionOnPage 对称——定位前把
      // semantic.name 的 {{占位符}} 按本 run ctx 回填成定位视图；落盘 event 与上游对象永不被改
      // （只在真含占位时产浅拷贝）。不这么做，编译期就拿字面 {{…}} 找元素、恒 0 命中（B4 九跑实证）。
      ev = instantiateEventSemantic(ev, this.ctx);
