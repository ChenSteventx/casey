...
export function publicationEntryForPath(journal, resolvedPath) {
  if (!journal || !Array.isArray(journal.entries) || typeof resolvedPath !== 'string') return null;
  const targetHash = sha(resolve(resolvedPath));
  const matches = journal.entries.filter((entry) => entry?.targetHash === targetHash && /^[0-9a-f]{64}$/.test(entry?.contentHash || ''));
...
export function createSignPublicationEngine(io = NODE_IO) {
  ...
  return function publish(rawPlan) {
    const plan = record(
      rawPlan,
      ['writes', 'journalPath', 'signedAt', 'authorityPath', 'beforeAuthorityCommit', 'readExisting'],
      ['writes', 'journalPath', 'signedAt', 'authorityPath'],
    );
    ...
    if (!plan || !sourceWrites || typeof plan.journalPath !== 'string' || !plan.journalPath
      ...
      || (Object.hasOwn(plan, 'beforeAuthorityCommit') && typeof plan.beforeAuthorityCommit !== 'function')
      || (Object.hasOwn(plan, 'readExisting') && typeof plan.readExisting !== 'function')) {
      return denied('PUBLICATION_PLAN_INVALID');
    }
    // 生产 sign 可把需物理持久的 archive target/tmp 接到 no-follow fd 读取；纯内存测试与
    // 其它调用者不传时维持注入 io 契约。所有“已存在且字节相等”的判断统一走此缝。
    const readExisting = plan.readExisting || io.read;
    const writes = [];
    ...
    try {
      ...
      if (hasJournal) {
        ...
        let existing;
        try { existing = JSON.parse(readExisting(journalPath)); } catch { return denied('PUBLICATION_JOURNAL_INVALID'); }
    ...
    for (let index = 0; index < writes.length; index++) {
      ...
      try {
        if (io.exists(path) && readExisting(path) === text) continue;
        if (io.exists(tmps[index])) {
          if (readExisting(tmps[index]) !== text) return denied('PUBLICATION_TMP_MISMATCH');
        } else {
    ...
    for (let index = 0; index < writes.length; index++) {
      const { path, text } = writes[index];
      if (index === writes.length - 1 && plan.beforeAuthorityCommit) {
        try {
          if (plan.beforeAuthorityCommit() !== true) return denied('PUBLICATION_PRE_AUTHORITY_CHECK_FAILED');
        } catch {
          return denied('PUBLICATION_PRE_AUTHORITY_CHECK_FAILED');
        }
      }
      try {
        if (io.exists(path) && readExisting(path) === text) {
          if (io.exists(tmps[index])) io.remove(tmps[index]);
          continue;
        }
        // stage 与 commit 之间仍须重核 tmp；生产 archive 读缝以 O_NOFOLLOW + fd inode
        // 取字节，拒绝把中断后换入的 symlink rename 成永久审计件。
        if (!io.exists(tmps[index]) || readExisting(tmps[index]) !== text) {
          return denied('PUBLICATION_TMP_PRECOMMIT_MISMATCH');
        }
    ...
    for (const { path, text } of writes) {
      try { if (!io.exists(path) || readExisting(path) !== text) return denied('PUBLICATION_FINAL_CHECK_FAILED', true); }
