...
// 次级定位字段必须缺席（评审 R1 grok Medium-1）：通用门的语义定位优先于 fallbackCss，白名单
// 形状若再携 fieldLabel / role / semantic，点击会落到那些字段解析出的别处元素上——修前这类
// 形状被分发层整体拒点（安全），接通后若不钉死就成了新可达面。编译器发射恒为纯 fallbackCss
// 形（六个真件与 M1 冻结钉均证），收紧零误伤。
export function isMagnifierSearchClick(ev) {
  ...
  return label == null
    && ev.semantic == null && ev.role == null && ev.fieldLabel == null
