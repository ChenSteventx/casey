...
const sortedCopy = (values) => [...(values ?? [])].sort();

// 闸二·状态机模拟。前提：闸一已过。返回每一步的确定性状态轨迹与问题列表。
// initialStates（加性，缺省 = 空集不改旧行为）：TestCase.preconditions 投影的初始状态种子——
// 「已登录」由登录预备动作在 flow 之外建立（G2，凭据红线），状态机据此不再要求 flow 内含 login 原子。
export function traceStateMachine(flow, reg, initialStates = []) {
  const problems = [];
  const steps = [];
  const atomDefs = reg.atoms;
  ...
  flow.steps.forEach((step, i) => {
    ...
    steps.push({
      ...
      atom: step.atom,
      before,
      requires: sortedCopy(def.requires),
      missing: sortedCopy(missing),
      provides: sortedCopy(def.provides),
      removes: sortedCopy(def.removes),
      after: sortedCopy(cur),
    });
  });
  return { steps, finalStates: sortedCopy(cur), problems };
}

// 兼容旧调用面：问题文本、顺序及「缺状态后补入防连锁」语义均由唯一状态机实现产生。
export function checkStateMachine(flow, reg, initialStates = []) {
  return traceStateMachine(flow, reg, initialStates).problems;
}
...
export function checkDestructivePrefix(flow, reg, prefix) {
  ...
  flow.steps.forEach((step, i) => {
    ...
    if (typeof v !== 'string' || v.trim() === '') {
      ...
      // output-seal A13：实体名值原值不回显（flow confirm 后可编辑、值可携任意内容），step 序号 + 前缀留定位。
      problems.push(`step[${i}] (${step.atom})：实体名必须带前缀「${prefix}」（破坏性硬闸，防误删真实数据/污染环境；原值不回显）`);
    }
  });
  return problems;
}
