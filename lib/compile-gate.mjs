...
// 闸二·状态机模拟 + 破坏性硬闸（前缀参数化）。前提：闸一已过。返回问题列表。
export function checkStateMachine(flow, reg, prefix) {
  const problems = [];
  const atomDefs = reg.atoms;
  ...
  flow.steps.forEach((step, i) => {
    ...
    if (def.entityNameParam) {
      const v = (step.params ?? {})[def.entityNameParam];
      if (typeof prefix !== 'string' || prefix.trim() === '') {
        // fail-closed：前缀缺失/为空时破坏性硬闸不能放行 —— startsWith('') 恒真会把硬闸静默清零（护栏 #14）。
        problems.push(`step[${i}] (${step.atom})：破坏性硬闸前缀缺失/为空，拒绝放行（uniquePrefix 必须是非空字符串）`);
      } else if (typeof v === 'string' && v.trim() !== '' && !v.startsWith(prefix)) {
...
// 双闸合一：闸一 →（干净且注册表 v2 含 states 时）闸二。前缀从参数注入、非写死。
export function validateDraft(draft, { prefix, registry } = {}) {
  const problems = validateStructural(draft, registry);
  if (problems.length === 0 && registry && registry.states) {
    problems.push(...checkStateMachine(draft, registry, prefix));
  }
