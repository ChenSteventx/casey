...
export function validateReplayEntityAnchors({ events, expectedDoc, profile, ctx }) {
  ...
  for (const [intentId, intentEvents] of deleteIntents) {
    ...
    // 删除副作用完成后的目标查询结果。
    const searches = intentEvents
      .map((ev, index) => ({ ev, index }))
      .filter(({ ev }) => ev.action === 'fill')
      .map(({ ev, index }) => ({ value: projectString(ev.value, ctx), index }))
      .filter(({ value }) => typeof value === 'string' && value.trim())
      .map(({ value, index }) => ({ value: value.trim(), index }));
    const searchTargets = searches.map(({ value }) => value);
    if (searchTargets.length < 2 || searchTargets.some((value) => value !== target)) {
      ...
      const destructiveClickIndexes = intentEvents
        .map((ev, index) => ({ ev, index }))
        .filter(({ ev }) => ev.action === 'click' && ['删除', '确定', '确认'].includes(deleteLabel(ev)))
        .map(({ index }) => index);
      const firstDeleteClick = destructiveClickIndexes[0];
      const lastDeleteClick = destructiveClickIndexes.at(-1);
      const hasSearchBeforeDelete = searches.some(({ index }) => index < firstDeleteClick);
      const hasSearchAfterConfirm = searches.some(({ index }) => index > lastDeleteClick);
      if (!hasSearchBeforeDelete || !hasSearchAfterConfirm) {
        problems.push({ intentId, reason: 'cleanup_search_target_not_ordered' });
      }
    }
