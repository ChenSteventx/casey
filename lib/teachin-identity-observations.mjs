...
import { verifyObservationTransactionPair } from './teachin-observation-authority-root.mjs';
import { validateClosedIdentityObservationSidecarDocument } from './teachin-identity-package-validator.mjs';
...
function validateSidecarDocument(doc, { caseId } = {}) {
  return validateClosedIdentityObservationSidecarDocument(doc, { caseId });
}
...
// 已通过 v2 closed-shape/identity reference 复核的 capture，投影回 legacy review/project 视图。
// 不放宽 reviewCapture；只移除 v2 唯一新增的 identityObservations 并恢复 schemaVersion 1。
export function projectIdentityBoundCaptureForLegacyReview(options = {}) {
  const { caseId, capture } = snapshotOptions(options, new Set(['caseId', 'capture']));
  const safeId = safeCaseId(caseId);
  validateCaptureV2(capture, safeId);
...
export function verifyAcceptedIdentityObservationBundle(options = {}) {
  ...
  const pair = verifyObservationTransactionPair({
    acceptedIntakeAuthority: input.acceptedIntakeAuthority,
    platformReadbackReceipt: input.platformReadbackReceipt,
  });
  if (!pair.ok) return reject(pair.reason || 'OBSERVATION_TRANSACTION_PAIR_INVALID');
  const authority = pair.facts;
  const receipt = pair.facts;
  const structural = reviewIdentityObservationBundle({
    caseId: input.caseId,
    captureRaw: input.captureRaw,
    observationRaw: input.observationRaw,
  ...
  if (receipt.provenance !== 'signed-driver-ed25519') return reject('READBACK_DRIVER_PROVENANCE_REQUIRED');
  if (receipt.captureSha256 !== authority.captureSha256
    || receipt.sidecarSha256 !== authority.sidecarSha256
    || receipt.manifestSha256 !== authority.manifestSha256) {
    return reject('READBACK_PACKAGE_BINDING_MISMATCH');
  }
  ...
  const eventReason = validateEventSeqReferences(capture, sidecar);
  if (eventReason) return reject(eventReason);
  const receiptObservations = Array.isArray(receipt.observations) ? receipt.observations : [];
  if (receiptObservations.length !== sidecar.observations.length) return reject('READBACK_RECEIPT_COVERAGE_MISMATCH');
  const exactKeys = [
    'kind', 'name', 'code', 'platformId', 'scopeFingerprint', 'parent', 'evidenceKind', 'eventSeq', 'evidenceSha256',
  ];
  for (let index = 0; index < sidecar.observations.length; index += 1) {
    const observation = sidecar.observations[index];
    if (!observation.evidenceSha256) return reject('READBACK_EVIDENCE_REQUIRED');
    for (const key of exactKeys) {
      if (receiptObservations[index]?.[key] !== observation[key]) {
        return reject(`READBACK_${key.replace(/[A-Z]/g, (m) => `_${m}`).toUpperCase()}_MISMATCH`);
      }
    }
  }
  return Object.freeze({
    ...structural,
    trusted: true,
    // 签名证明固定 registry 对应私钥签过；真实私钥保护与 nonce 持久防重放仍是 route:human。
    trustProvenance: 'signed-driver-ed25519',
  });
}
