...
import { acceptedIntakeAuthorityFacts } from './record-distill.mjs';
...
function snapshotOptionsWithOpaque(value, allowedKeys, opaqueKey) {
...
export function verifyAcceptedIdentityObservationBundle(options = {}) {
  ...
  try {
    input = snapshotOptionsWithOpaque(
      options,
      new Set(['caseId', 'captureRaw', 'observationRaw', 'pathFacts', 'acceptedIntakeAuthority']),
      'acceptedIntakeAuthority',
    );
  } catch (error) {
    return reject(safeErrorCode(error));
  }
  ...
  const accepted = acceptedIntakeAuthorityFacts(input.acceptedIntakeAuthority);
  ...
  const eventReason = validateEventSeqReferences(capture, sidecar);
  if (eventReason) return reject(eventReason);
  return Object.freeze({ ...structural, trusted: true });
}
