...
import { normConverge, reviewCapture } from './record-intake.mjs';
...
function safeRequiredString(value, code = 'FIELD_INVALID') {
  ...
  const normalized = normConverge(value);
...
function validateEventSeqReferences(capture, sidecar) {
  ...
  for (const event of capture.events) {
    const seq = event?.seq;
    if (!Number.isSafeInteger(seq) || seq < 1 || seqs.has(seq)) return 'CAPTURE_EVENT_SEQ_INVALID';
    seqs.add(seq);
  }
  for (const observation of sidecar.observations) {
    if (!seqs.has(observation.eventSeq)) return 'EVENT_SEQ_MISMATCH';
  }
  return null;
}
