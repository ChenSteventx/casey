...
export function reviewCapture(doc, options = {}) {
  let input;
  let reviewOptions;
  try {
    input = doc && typeof doc === 'object' ? snapshotPlainOwnData(doc) : doc;
    reviewOptions = snapshotClosedOptions(options, new Set(['caseId']));
  } catch {
    return { ok: false, status: 'rejected', reason: 'UNSAFE_DATA_SHAPE', eventCount: null };
  }
  doc = input;
  const { caseId } = reviewOptions;
...
export function intakeLedgerPath(options = {}) {
...
export function buildIntakeRecord(options = {}) {
  const {
    caseId,
    status,
    reason = null,
    ...
    'caseId', 'status', 'reason', 'eventCount', 'captureName', 'captureSha256', 'sidecarSha256',
    'manifestSha256', 'observationCount', 'observationSchemaVersion', 'intakedAt',
  ]));
  const record = {
    schemaVersion: 1,
    event: 'intake',
  ...
  // 新三件套只在调用方已完成 package 联合验证后传入；旧拒账/旧调用保持原记录形状，避免把 null
  // 冒充完整性背书。distill 的 verifyIntakenPackage 只承认五字段齐全的 accepted 记录。
  if (status === 'accepted' && typeof sidecarSha256 === 'string' && typeof manifestSha256 === 'string'
    && Number.isSafeInteger(observationCount) && observationCount >= 0
    && Number.isSafeInteger(observationSchemaVersion) && observationSchemaVersion >= 1) {
    record.sidecarSha256 = sidecarSha256;
    record.manifestSha256 = manifestSha256;
    record.observationCount = observationCount;
    record.observationSchemaVersion = observationSchemaVersion;
  }
  return record;
}

// append-only 落台账：序列化单行过凭据门（命中抛 CREDENTIAL_GATE，绝不写），追加不改（会计台账血缘）。
export function appendIntakeLedger(options = {}) {
  const { ledgerPath, record } = snapshotClosedOptions(options, new Set(['ledgerPath', 'record']));
