...
const SIDE_EFFECT_POLICY = new Map([
  ['nav.workflowManagement', Object.freeze({ effect: 'read', requiredRoles: Object.freeze([]) })],
  ['assert.textVisible', Object.freeze({ effect: 'read', requiredRoles: Object.freeze([]) })],
...
function requiredEntityBindings(document, collectionKey, allowedBindingFields, requiredBindingFields, identityFields, project) {
  ...
  for (const operation of operations) {
    const policy = policyForAtom(operation.atom);
...
export function checkReplayEntityAdmission(rawOptions) {
  const options = closedRecord(rawOptions, {
    ...
    required: ['caseId', 'eventsBytes', 'eventsDocument', 'frozenLockAuthority'],
  });
