...
// v2 身份观察行的冻结闭合面（八字段），v1 件不带。
const FROZEN_OBSERVATION_FIELDS_V2 = [
  'name', 'code', 'platformId', 'sourceIntentId', 'candidateId', 'role', 'atom', 'evidenceStepId',
];
// v3 观察行补 kind：跨类别硬闸所需（v2 冻结面把它丢了，见 destructive-continuity-ref/design-per-step-ref.md 第 ⑥ 层）。
// closedRecord 的 required 默认等于 allowed，故 v3 的 kind 是必填——正好防住「v3 锁里混进不带 kind 的旧行」。
const FROZEN_OBSERVATION_FIELDS_V3 = [...FROZEN_OBSERVATION_FIELDS_V2, 'kind'];
const DESTRUCTIVE_TARGET_KINDS = new Set(['agent', 'workflow']);
// v3 新段 destructiveContinuity 的逐条闭合面：把「哪条观察授权哪个破坏步」显式签进冻结面。
// profileFingerprint 故意不逐条存——事实源是锁顶层的 identityProfileDigest（一个事实源，不制造分歧）。
const FROZEN_CONTINUITY_STRING_FIELDS = [
  'destructiveStepId', 'destructiveIntentId', 'destructiveAtom', 'boundKind',
  'observationEvidenceStepId', 'platformId', 'scope', 'requestCorrelationId',
];
const FROZEN_CONTINUITY_FIELDS = [...FROZEN_CONTINUITY_STRING_FIELDS, 'stepOrder'];
...
export function validateFrozenEntityLockArtifact(value) {
  // v1/v2 只按 schemaVersion 判别（agent-id-readback R2-3）；v1 闭合面逐字不变。
  // v3（destructive-continuity-ref 设计一）在 v2 之上并列新增 destructiveContinuity 段与观察行 kind；
  // v1/v2 分支逐字不动——这是「旧件零行为差」的兑现处。
  const version = value && typeof value === 'object' && !Array.isArray(value) ? value.schemaVersion : null;
  const isV2 = version === 2;
  const isV3 = version === 3;
  const hasIdentityFace = isV2 || isV3;
  const V2_EXTRA = ['identityProfileDigest', 'identityObservationsSha256', 'identityObservations'];
  let fields = FROZEN_ARTIFACT_FIELDS;
  if (isV2) fields = [...FROZEN_ARTIFACT_FIELDS, ...V2_EXTRA];
  if (isV3) fields = [...FROZEN_ARTIFACT_FIELDS, ...V2_EXTRA, 'destructiveContinuity'];
  const artifact = closedRecord(value, { allowed: fields, required: fields });
  if (!artifact
    || (artifact.schemaVersion !== 1 && artifact.schemaVersion !== 2 && artifact.schemaVersion !== 3)
  ...
  if (hasIdentityFace) {
    ...
    const observationFields = isV3 ? FROZEN_OBSERVATION_FIELDS_V3 : FROZEN_OBSERVATION_FIELDS_V2;
    for (const rawRow of rows) {
      const row = closedRecord(rawRow, { allowed: observationFields, required: observationFields });
      if (!row || !observationFields.every((f) => nonEmpty(row[f]))
        || !ROLES.has(row.role)) return null;
      if (isV3 && !DESTRUCTIVE_TARGET_KINDS.has(row.kind)) return null;
    }
  }
  if (isV3) {
    // 逐破坏步授权边只做【结构】闭合校验：字段齐、类型对、值域合法。
    // 「边与 events 是否对得上」「被引观察是否同一目标」属回放期逐条不变量（I1–I8），
    // 由 bin/replay.mjs 具名拒绝——放这里会把攻击信号压成一句「锁非法」，丧失可诊断性。
    const edges = closedArray(artifact.destructiveContinuity);
    if (!edges) return null;
    for (const rawEdge of edges) {
      const edge = closedRecord(rawEdge, { allowed: FROZEN_CONTINUITY_FIELDS, required: FROZEN_CONTINUITY_FIELDS });
      if (!edge || !FROZEN_CONTINUITY_STRING_FIELDS.every((f) => nonEmpty(edge[f]))) return null;
      if (!DESTRUCTIVE_TARGET_KINDS.has(edge.boundKind)) return null;
      if (!Number.isInteger(edge.stepOrder) || edge.stepOrder < 0) return null;
    }
  }
...
export function readIdentityAdmissionAuthorityFromPrd(rawOptions) {
  ...
  try {
    ...
    const artifact = options.domain === 'execute'
      ...
      : validateFrozenEntityLockArtifact(parsed);
...
export function readFrozenIdentityObservations(frozenLockAuthority) {
  ...
  if (!artifact || (artifact.schemaVersion !== 2 && artifact.schemaVersion !== 3)
    || !Array.isArray(artifact.identityObservations)) return null;
  return artifact.identityObservations.map((row) => ({ ...row }));
}

// v3 冻结锁的逐破坏步授权边只读投影（destructive-continuity-ref 设计二的新读取器，与上方观察读取器同族）：
// 只对已铸 verify 域不透明句柄暴露；v1/v2 件回 null（旧件零行为差）。
// 权威恒是被引的观察行——边上的 platformId 只供比对，比对通过后不使用。
export function readFrozenDestructiveContinuity(frozenLockAuthority) {
  const state = stateFor(frozenLockAuthority, 'verify');
  const artifact = state?.artifact;
  if (!artifact || artifact.schemaVersion !== 3 || !Array.isArray(artifact.destructiveContinuity)) return null;
  return artifact.destructiveContinuity.map((edge) => ({ ...edge }));
}
