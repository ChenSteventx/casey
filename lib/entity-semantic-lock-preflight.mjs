...
const CHECK_FIELDS = [
  ...
  'flowBytes', 'testcaseBytes', 'eventsBytes', 'requiredBindings',
...
export function flowContainsEntityMutation(flow, registry) {
  ...
  if (flow == null || typeof flow !== 'object' || !Array.isArray(flow.steps)
    || registry == null || typeof registry !== 'object' || Array.isArray(registry) || registry.atoms == null) return true;
  return flow.steps.some((step) => {
    if (step == null || typeof step !== 'object' || !nonEmpty(step.atom)) return true;
    const definition = registry.atoms[step.atom];
    if (definition == null || typeof definition !== 'object') return true;
...
export function eventsContainEntityMutation(eventsDocument, registry) {
  const events = eventsDocument != null && typeof eventsDocument === 'object' && Array.isArray(eventsDocument.events)
    ? eventsDocument.events
    : null;
  if (!events || registry == null || typeof registry !== 'object' || Array.isArray(registry) || registry.atoms == null) return true;
  return events.some((event) => {
    if (event == null || typeof event !== 'object' || !nonEmpty(event.atom)) return true;
...
export function requiredFlowEntityBindings(flow) {
  if (flow == null || typeof flow !== 'object' || !Array.isArray(flow.steps)) return [];
  const out = [];
  for (const step of flow.steps) {
    if (step == null || typeof step !== 'object' || !Array.isArray(step.entityBindings)) continue;
...
export function requiredEventEntityBindings(eventsDocument) {
  ...
  for (const event of events) {
    if (event == null || typeof event !== 'object' || !Array.isArray(event.entityBindings)) continue;
...
export function checkCompileIdentityAdmission(rawOptions) {
  ...
  if (options.mode === 'execute') {
    const forbidden = ['frozenLockAuthority', 'eventsBytes'].some((key) => Object.hasOwn(options, key));
    ...
    if (options.containsEntityMutation !== true) return allowed('deterministic-read-only-policy');
  ...
  if (options.mode === 'verify') {
    const forbidden = ['executeAuthority', 'flowBytes', 'testcaseBytes'].some((key) => Object.hasOwn(options, key));
