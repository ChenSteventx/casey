...
const REAL_SUT_READINESS_IDS = new Set(['site-json', 'creds', 'account-acl', 'tunnel']);
...
export function checkSiteJson({ present, shapeOk } = {}, { realSut = false } = {}) {
  if (!present) {
    return { id: 'site-json', status: realSut ? 'fail' : 'warn', detail: 'site.json 未配置（本机安装可无；真实环境必需）', hint: '如需真实回放，按 README.md 凭据节配置 site.json' };
  }
  return shapeOk
    ? { id: 'site-json', status: 'ok', detail: 'site.json 在位且顶层键完整', hint: '' }
    : { id: 'site-json', status: realSut ? 'fail' : 'warn', detail: 'site.json 在位但 startUrl/devProxyUrl 双键不完整', hint: '按 README.md 凭据节补全 target 双键' };
}

// ── 建议级 7：凭据在位 + 浅形态（同 D5；只查键名在不在，绝不读值）─────
export function checkCreds({ present, shapeOk } = {}, { realSut = false } = {}) {
  if (!present) {
    return { id: 'creds', status: realSut ? 'fail' : 'warn', detail: 'AI 中台账户未配置（本机安装可无；真实环境必需）', hint: '如需真实回放，通过 Casey skill 安全配置本地账户' };
  }
  return shapeOk
    ? { id: 'creds', status: 'ok', detail: 'AI 中台账户在位、形状有效且已接登录预备动作', hint: '' }
    : { id: 'creds', status: realSut ? 'fail' : 'warn', detail: 'AI 中台账户在位但形状无效', hint: '通过 Casey skill 重新安全配置本地账户' };
}

export function checkAccountAcl({ applicable, safe } = {}, { realSut = false } = {}) {
  if (!applicable) return { id: 'account-acl', status: 'ok', detail: '无需 Windows 本地账户 ACL（POSIX 权限或环境账户）', hint: '' };
  if (safe) return { id: 'account-acl', status: 'ok', detail: 'Windows 账户目录与文件 ACL 已收紧', hint: '' };
  return { id: 'account-acl', status: realSut ? 'fail' : 'warn', detail: 'Windows 账户 ACL 未通过安全检查', hint: '通过 Casey skill 重新安全配置账户，禁止放在共享目录' };
}

// 医生站 / Hi 小助只检查本地引用与人工就绪声明；真实登录页未采样前永不声称自动登录。
export function checkDesktopAccount({ present, shapeOk, ready } = {}) {
  if (!present) {
    return { id: 'desktop-account', status: 'warn', detail: '医生站 / Hi 小助账户引用未配置', hint: '通过 Casey skill 配置本地账户引用与人工就绪声明' };
  }
  if (!shapeOk) {
    return { id: 'desktop-account', status: 'warn', detail: '医生站 / Hi 小助账户配置形状无效', hint: '通过 Casey skill 重新配置；账户值不会回显' };
  }
  if (!ready) {
    return { id: 'desktop-account', status: 'warn', detail: '医生站 / Hi 小助账户引用在位，但尚未声明安全会话就绪', hint: '人工确认专用账户、已登录且无真实患者数据' };
  }
  return { id: 'desktop-account', status: 'route-human', detail: '医生站 / Hi 小助账户引用与人工就绪声明在位；自动登录未验证', hint: '真实登录页采样前仍须人工建立安全会话' };
}

// ── route:human 级 8：隧道回环端口（在听只证本地监听器起了；端到端 route:human）──
// 只述「端口在听/不在听 + 耗时」，绝不含 target 地址、绝不含 :// 形态（GRILL D6）。
export function checkTunnel({ proxyPort, portListening, probeMs } = {}, { realSut = false } = {}) {
  ...
  if (realSut) {
    return portListening
      ? { id: 'tunnel', status: 'ok', detail: `${detail}；LOCAL_PROXY_READY`, hint: '只证本地代理；仍须真实 HTTP 与正式报告证据' }
      : { id: 'tunnel', status: 'fail', detail, hint: '先启动本平台回环代理；端口就绪后仍须真实 HTTP 证据' };
  }
...
export function runDoctor(env = {}, { realSut = false } = {}) {
  const items = [
    checkNode(env.node),
    ...
    checkSiteJson(env.siteJson, { realSut }),
    checkCreds(env.creds, { realSut }),
    checkDesktopAccount(env.desktopAccount),
    checkAccountAcl(env.accountAcl, { realSut }),
    checkTunnel(env.tunnel, { realSut }),
  ];
  const blocking = realSut ? new Set([...READINESS_IDS, ...REAL_SUT_READINESS_IDS]) : READINESS_IDS;
  const exitCode = items.some((i) => blocking.has(i.id) && i.status === 'fail') ? 1 : 0;
  const summary = realSut
    ? (exitCode === 0 ? 'LOCAL_REAL_SUT_PREREQUISITES_READY; REAL_SUT_HTTP_NOT_VERIFIED' : 'REAL_SUT_PREREQUISITES_NOT_READY; REAL_SUT_HTTP_NOT_VERIFIED')
    : (exitCode === 0 ? 'LOCAL_INSTALL_READY; REAL_SUT NOT_VERIFIED' : 'LOCAL_INSTALL_NOT_READY; REAL_SUT NOT_VERIFIED');
  return { items, exitCode, summary };
}
