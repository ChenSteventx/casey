...
export function checkCreds({ present, shapeOk } = {}) {
  if (!present) {
    return { id: 'creds', status: 'warn', detail: 'AI 中台账户未配置（hermetic 用户可无；真机才需）', hint: '如需真机回放，通过 Casey skill 安全配置本地账户' };
  }
  return shapeOk
    ? { id: 'creds', status: 'ok', detail: 'AI 中台账户在位、形状有效且已接登录预备动作', hint: '' }
    : { id: 'creds', status: 'warn', detail: 'AI 中台账户在位但形状无效', hint: '通过 Casey skill 重新安全配置本地账户' };
}
