...
import {
  inspectCompileAtomExecutionFailure,
  isLoginBootstrapFailure,
  LOGIN_BOOTSTRAP_FAILURE_CODE,
} from '../compile-execution-failure.mjs';
...
export function emitCompileCliFailure({
  ...
  if (phase === 'execute' && isLoginBootstrapFailure(failure)) {
    stderr.write(`compile: ${LOGIN_BOOTSTRAP_FAILURE_CODE}\n`);
    return 1;
  }
  const atomFailure = phase === 'execute'
    ? inspectCompileAtomExecutionFailure({ failure, knownAtoms })
    : null;
  if (atomFailure) {
    stderr.write(`compile: ${atomFailure.code} stepOrdinal=${atomFailure.stepOrdinal} atom=${atomFailure.atom}\n`);
    return 1;
  }
