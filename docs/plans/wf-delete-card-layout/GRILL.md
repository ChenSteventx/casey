...
- **定案：尽力关闭（按 `Escape`），并且绝不允许收拾动作改写动作轴**——`try/catch` 吞掉，镜像既有 `dispose` / `clearPin` 的清理纪律（源码注释原话「清理不得覆盖动作轴」）。
- 反对意见：按 `Escape` 也是一次真实交互。我认为可接受——它是取消语义，且删除链此时已判失败、后续步在编译期会被截断。若评审不同意，退路是「不收拾」，代价是回放侧残留浮层可能连累后续步定位，两害相权我选收拾。**这条列进须人裁清单。**
...
1. **确认环不改**（任务书写的「删除」候选按 §〇 纠正为不成立且有害）——请确认我可以按「确认环零改动」落地。
2. **`deleteButtons` 口径改为「可达删除面」**（§五 乙）——这是审计语义的口径变更，虽不碰冻结件，但改的是破坏性动作的准入对账口径。
3. **失败后按 `Escape` 收拾菜单**（§八）——是否接受这次额外交互。
4. **`CONTEXT.md` 登记「因果菜单授权」放到实现波**（§十）——是否同意本轮只申报。
5. §九 的三条相邻缺口（Enter 不过滤 / `countSelector` 与 `.agent-card` 不同源 / `compile-atoms-agent.mjs:194` 漏导入）**怎么排**——尤其第 3 条是现存运行时错误。
