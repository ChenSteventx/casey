...
**复跑实录见 `accept/affected-goldens.txt`**（选集规则可重跑、逐份退出码在册）：50 份，
49 绿、1 红、0 份因环境故障作废；唯一真红 `real-run-trust` 属既有陈旧红（本刀未碰
`bin/replay.mjs` 与该金牌任一文件，不可能由本刀引入），另记欠账。

数字更正（2026-08-10）：提交 `630958d` 的信息里写「串行复跑 32 份，7 份红」——**那两个数
都是错的**，且当时没落清单，异构评审因此判 `NEEDS_HUMAN`（判得对）。按上面的可重跑规则复跑，
真实结果是 50 份 / 1 红。先前那 7 份「红」不可复现，最可能是当日 `/mnt/d` 9p 静默断连期间
把绿件打成了非零退出（同一现象在本轮第一次跑批里再现过一次：30 份连续假红，其中包括已被
两路评审独立跑到 28/28 的 `wf-delete-card-layout`）。教训：批量跑金牌须同抓 stderr，
非零退出里含 `EIO`/`ENOENT` 的记作废而非记红。
