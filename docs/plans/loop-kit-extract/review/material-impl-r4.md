# loop-kit-extract 实现审 round-2 第三次跑评审料（material-impl-r4）

> 用途：喂给异构冗余实现审（`codex` / `pi`）round-2 第三次跑。此前两次跑：第一次跑 `codex` 判
> `CHANGES REQUIRED`（2 `HIGH`+1 `MED`+1 `LOW`，`pi` 同料判 `PASS`）；第二次跑 `codex` 判
> `CHANGES REQUIRED`（1 `HIGH`，`pi` 同料判 `PASS`）；两次发现均已处置，详见
> `docs/plans/loop-kit-extract/review/codex-impl-r2.md`/`r3.md`。本轮复核处置后的最终状态。
...
`casey-loop-kit-extract` 契约两轮实现审发现的第三次处置复核（round-1 七条 + round-2 两次跑各自
的发现均已处置）。请针对下文「历史发现摘要」「本轮新增处置」「diff」「门禁证据」逐条核实：

1. round-2 第二次跑的 1 条 `codex` `HIGH`（外部预置合法 ROOT 后可再改写另一合法 ROOT）是否已真实
   闭合？重点复核 `isFrozenBySelf()`/`adoptAndFreezeIfNeeded()` 的逻辑：`revalidateClaimed()`
   （幂等复用路径）与 `claimAtomic()`（同值复用分支）是否都调用了它；「当场补冻结」是否确实在
   返回值给调用方之前完成（避免调用方已经用了旧值、冻结才姗姗来迟的时序缝隙）；`isFrozenBySelf()`
   的判据（`writable===false && configurable===false`）是否确实只有本模块自己的 `writeClaimed()`
   会产生、不会被外部代码的其它冻结方式（如只设 `writable:false` 不设 `configurable:false`）意外
   混淆。可尝试独立构造复现：外部预置合法 ROOT A → 调用 `resolveRoot` → 立即尝试改写为合法 ROOT B
   → 是否正确拒绝；以及反过来，显式 `envRoot` 路径（`claimAtomic` 直接调用）下是否也同样受保护。
2. 这次修复本身是否又引入新问题（性能、误伤合法场景、逻辑死角）。
3. 历史累积的全部发现（round-1 七条 + round-2 两次跑五条）在本轮改动后是否仍然闭合。
4. 冻结纪律与凭据/术语边界同前两轮标准。
...
`round-1`（7 条采信，已修复）：`A1`（`HIGH`）ROOT 认领槽模块局部变量；`A2`（`HIGH`）D5 故障
矩阵金牌未覆盖全故障域；`A3`（`HIGH`）`boot.mjs` 不入 `testChecksums`；`A4`（`MED`）金牌原地覆写
生产信任根文件；`A5`（`MED`）观测基线规范化非字段级白名单；`A6`（`MED`）`loadLib` 字符串拼接
`file://` URL 特殊字符解析错位；`A7`（`LOW`）信号自终失败兜底码不符 `128+n` 语义。

`round-2` 第一次跑（`codex` 2 `HIGH`+1 `MED`+1 `LOW`，已修复）：`worker_threads` 跨 Worker 独立
`globalThis` 静默各自认领；槽内容预置无效值被幂等复用照单全收；`SPAWN_ERROR`/`NO_STATUS` 断言
只查退出码、可假绿；`A6` 缺失特殊字符路径回归测试；`C4` 遗留孤儿锁文件。

`round-2` 第二次跑（`codex` 1 `HIGH`，本轮处置对象）：外部预置合法、规范化的 ROOT 后，
幂等复用路径校验通过但从未冻结该属性，随后可再改写成另一个同样合法的 ROOT——同一进程先后接受
两个不同合法值，违反「首次认领后不可变」。

## 2. 本轮新增处置

`loop-kit/lib/root.mjs` 新增两个内部函数：
...
调用点：`revalidateClaimed()`（幂等复用路径，返回前调用）与 `claimAtomic()` 的「`current !== null`
且 `current === candidate`」同值分支（返回前调用）。语义：任何代码路径打算把某个槽值当「已认领」
使用之前，先确认该属性已是本模块产生的冻结形态；不是则当场补冻结——把「首次被信任使用的时刻」
当作认领时刻，不再局限于「从空槽写入」这一条路径。

独立验证（mutation/探针，非转录）：外部普通赋值预置合法 ROOT A → 调用 `resolveRoot({envRoot:''})`
→ 属性立即变为 `{writable:false, configurable:false}` → 尝试改写为合法 ROOT B → 抛
`TypeError: Cannot assign to read only property...`、认领值保持为 A。落成 C7 永久回归测试
（`外部预置合法 ROOT 后再改写另一合法 ROOT`）。
...
loop-kit-extract golden: 72/72 GREEN
...
+    "lib/root.mjs": "ae3bab336289a34621059f3c7ec87e3caf5a8bf29b051c6fa09857cc66b48557",
...
+    "loop-kit/kit-lock.json": "e509e03f01ef3261a55d612b14f5e6085b3e0b98985373f1129635dd84ad987e",
...
+    "tests/_golden/loop-kit-extract.golden.mjs": "7aa591c83318d2609033ade97735a075fd8d761c0e040c33a6be053208e40f5f",
...
+    "tests/fixtures/loop-kit-expected/package/lib/root.mjs": "ae3bab336289a34621059f3c7ec87e3caf5a8bf29b051c6fa09857cc66b48557",
...
+      "evidence": "gate@2026-07-13T17:12:01.521Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T17:12:13.656Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T17:12:29.457Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T17:13:10.758Z 全部 acceptance exit 0"
     },
     {
...
@@ -603,102 +1003,259 @@ await check('C7', 'env 有效命中：realpath 规范化后返回', async () =>
   }
 });
...
+import { isMainThread } from 'node:worker_threads';
 ...
 export class RootResolutionError extends Error {
   constructor(message) {
@@ -22,7 +62,43 @@ export class RootResolutionError extends Error {
   }
 }
...
@@ -43,6 +119,19 @@ function validate(dir, sourceLabel) {
   }
 }
...
+
 function upwardSearch(startCwd) {
   ...
   for (;;) {
@@ -60,25 +149,36 @@ function upwardSearch(startCwd) {
 }
...
@@ -86,7 +186,8 @@ export function resolveRoot({ envRoot = process.env.LOOP_KIT_ROOT, cwd = process
