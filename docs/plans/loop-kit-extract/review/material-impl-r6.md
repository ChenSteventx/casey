# loop-kit-extract 实现审 round-2 第五次跑评审料（material-impl-r6）

> 用途：喂给异构冗余实现审（`codex` / `pi`）round-2 第五次跑。此前四次跑均由 `codex` 发现问题、
> `pi` 同料判 `PASS`：第一次跑 2 `HIGH`+1 `MED`+1 `LOW`；第二次跑 1 `HIGH`；第三次跑 1 `MED`+1
> `LOW`；第四次跑 1 `HIGH`（`node:vm` `Context` 边界，新角度）+1 `LOW`。均已处置，详见
> `docs/plans/loop-kit-extract/review/codex-impl-r2.md`/`r3.md`/`r4.md`/`r5.md`。本轮复核处置后
> 的最终状态。
...
`casey-loop-kit-extract` 契约实现审第六轮复核。请针对下文「历史发现摘要」「本轮新增处置」「diff」
「门禁证据」逐条核实：

1. `round-2` 第四次跑的 1 条 `HIGH`（`node:vm` `Context` 边界，选择文档化而非运行时防护）与 1 条
   `LOW`（C7 断言过宽）是否已妥善处置？对 `HIGH`：请评估「文档化范围收窄、不加运行时防护」这一
   处置方式本身是否合理（对比 `worker_threads` 的既有先例——那条用 `isMainThread` 做了运行时拒绝；
   本条为何不能同样加运行时拒绝，是否有更好的折中方案，或维持纯文档化确实是当前最优解）。对
   `LOW`：新的精确断言是否确实修好了、有没有引入其它精度问题。
2. 通读 `root.mjs`、`boot.mjs` 全文（本轮 diff 累积后的最终版本），独立再评估是否还有其它缺口
   （不局限于历史发现列表，包括但不限于 JS 执行环境的其它变体、竞态、边界条件）。
3. 这五轮修复整体上是否引入了任何新问题、性能隐患、或对既有闭合项目的意外破坏。
4. 冻结纪律与凭据/术语边界同前几轮标准。

给出总体结论（`PASS` / `CHANGES REQUIRED`），若仍有发现请标 `HIGH`/`MED`/`LOW` 并给出文件位置与
复现方式；若你认为当前状态已经是合理的最终态（进一步收紧只有边际收益、成本不成比例），也请明确
说明并给出 `PASS` 结论。你在 `read-only` 沙箱内可以自行读文件、跑只读命令来验证。
...
`round-2` 第四次跑（`codex` 1 `HIGH`+1 `LOW`，本轮处置对象）：`node:vm` 的 `Context` 各有独立
`globalThis` 且 `isMainThread` 恒为 `true`，无法像拦 `worker_threads` 那样拦下；C7 的
`out.first.includes('a')` 断言过宽（`marker-root` 本身含字母 `a`）。

## 2. 本轮新增处置

- `loop-kit/lib/root.mjs` 语义 6 改写为「进程唯一」准确表述为「本机制运行所在的默认 Node.js 主
  realm 内唯一」，新增专节说明 `node:vm` `Context` 边界的技术原因（无可靠运行时判据）与范围收窄
  依据（`--experimental-vm-modules` 默认不可用、需攻击者已有同进程任意代码执行能力、`grep` 核验
  零使用）——纯文档变更，`isFrozenBySelf()`/`isClaimSlotFrozen()`/`adoptAndFreezeIfNeeded()` 等
  运行时逻辑零改动。
- `tests/_golden/loop-kit-extract.golden.mjs` 两处 `out.first.includes('a')` 改为
  `out.first === realpathSync.native(dirA)` 精确比对。
...
+    "lib/root.mjs": "2f2caf693c7d6aa07b82c6b0f998c311922cdc04eef1b7895b86ca3bf1541b04",
...
+    "tests/_golden/loop-kit-extract.golden.mjs": "0addc8a84aa80b9561780437179661591a0e7bf98e5be9637449134f5968a7a6",
...
+      "evidence": "gate@2026-07-13T18:04:45.511Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T18:04:57.634Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T18:05:13.141Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T18:05:54.394Z 全部 acceptance exit 0"
     },
     {
...
@@ -603,102 +1003,302 @@ await check('C7', 'env 有效命中：realpath 规范化后返回', async () =>
   }
 });
...
+import { isMainThread } from 'node:worker_threads';
 ...
 export class RootResolutionError extends Error {
   constructor(message) {
@@ -22,7 +78,48 @@ export class RootResolutionError extends Error {
   }
 }
...
@@ -43,6 +140,19 @@ function validate(dir, sourceLabel) {
   }
 }
...
+
 function upwardSearch(startCwd) {
   ...
   for (;;) {
@@ -60,25 +170,36 @@ function upwardSearch(startCwd) {
 }
...
@@ -86,7 +207,8 @@ export function resolveRoot({ envRoot = process.env.LOOP_KIT_ROOT, cwd = process
