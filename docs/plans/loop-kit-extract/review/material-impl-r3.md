# loop-kit-extract 实现审 round-2 第二次跑评审料（material-impl-r3）

> 用途：喂给异构冗余实现审（`codex` / `pi`）round-2 第二次跑（round-2 第一次跑：`codex` 判
> `CHANGES REQUIRED`——2 `HIGH`+1 `MED`+1 `LOW`；`pi` 判 `PASS`；全文见
> `docs/plans/loop-kit-extract/review/codex-impl-r2.md`/`pi-impl-r2.md`）。本轮复核处置后的最终状态，
> 只含 spec、发现原文、处置表、diff、门禁证据；不含实现者推理、不含凭据。
...
`casey-loop-kit-extract` 契约两轮实现审发现的第二次处置复核：round-1（7 条采信，3 `HIGH`/3
`MED`/1 `LOW`）已修复；round-2 第一次跑 `codex` 又发现 2 `HIGH`/1 `MED`/1 `LOW`（`pi` 同料判
`PASS`，未发现这些问题），本轮已处置。请针对下文「round-1 发现摘要」「round-2 第一次跑发现原文」
「round-2 处置表」「diff」「门禁证据」逐条核实：

1. round-2 第一次跑的 4 条 `codex` 发现（A1 的 `worker_threads` 边界 + 槽内容防篡改、A2 的
   `SPAWN_ERROR`/`NO_STATUS` 弱断言、A6 缺失特殊字符路径回归测试、C4 孤儿锁文件）是否都已真实闭合？
   重点复核：`root.mjs` 的 `isMainThread` 拒绝是否真的堵死了跨 `Worker` 场景（可用真实
   `worker_threads` 复现验证）；`Object.defineProperty` 冻结是否确实阻止了同进程后续覆写；幂等
   复用路径的重新校验是否确实拒绝了预置的无效值；`assertDegradesAllKindsWithReason` 是否确实能
   区分 `SPAWN_ERROR` 与 `NO_STATUS`（可尝试用小型 mutation 验证：若删掉 `boot.mjs` 里
   `if (r.error) {...}` 分支，新断言是否真的会转红）；特殊字符路径回归测试是否确实调用了会触达
   `pathToFileURL`/字符串拼接分叉的真实代码路径；孤儿锁文件是否不再产生。
2. 这些新修复本身是否又引入了新问题：例如 `isMainThread` 拒绝是否误伤了任何合法的主线程场景；
   `Object.defineProperty` 冻结是否会在某个合法的重复认领场景里意外抛错；幂等复用路径新增的
   重新校验（多一次 `fs.existsSync`/`realpathSync` 调用）是否有未预期的性能或行为副作用。
3. round-1 已闭合的 7 项（`A1`–`A7`）在本轮新增修改后是否仍然闭合（未被本轮改动意外破坏）。
4. 冻结纪律与凭据/术语边界同前轮标准。

给出总体结论（`PASS` / `CHANGES REQUIRED`），若有发现请标 `HIGH`/`MED`/`LOW` 并给出文件位置与
复现方式。你在 `read-only` 沙箱内可以自行读文件、跑只读命令（`node --check`、`grep`、构造最小复现
脚本，含构造真实 `worker_threads` 探针）来验证，不要求只凭料文件文本判断。

---

## 1. round-1 发现摘要（7 条采信，已修复，详见 `docs/plans/loop-kit-extract/review/arb-impl-r1.md`）

`A1`（`HIGH`）ROOT 认领槽模块局部变量→跨包目录副本零冲突；`A2`（`HIGH`）D5 故障矩阵金牌未覆盖
plan 承诺的全故障域；`A3`（`HIGH`）`boot.mjs` 不入 `testChecksums`；`A4`（`MED`）金牌原地覆写生产
信任根文件；`A5`（`MED`）观测基线规范化全局正则非字段级白名单；`A6`（`MED`）`loadLib` 字符串拼接
`file://` URL 特殊字符解析错位；`A7`（`LOW`）信号自终失败兜底码不符 128+n 语义。

## 2. round-2 第一次跑发现原文（`codex`，2 `HIGH`+1 `MED`+1 `LOW`；`pi` 同料判 `PASS`）

### HIGH — A1 仍不是严格的「进程唯一 ROOT」

`globalThis[Symbol.for(...)]` 只在同一 JavaScript 执行环境内共享，不跨 Node `worker_threads` 的独立
全局环境。同一操作系统进程中的两个 `Worker` 仍能分别认领不同 ROOT，均不冲突。只读探针实际结果：
`[{"ok":true,"root":"/mnt/d/ctx/heren/casey-loop-kit-extract"},{"ok":true,"root":"/mnt/d/ctx/heren/casey"}]`。
此外，该槽可被同进程代码预置，且读取时不校验类型、目录或根标记：
`globalThis[Symbol.for('loop-kit:root:claimed')] = '/preseeded'` 后 `resolveRoot({envRoot:'', cwd:...})`
直接返回未经验证的 `/preseeded`（实测 `{"claimed":"/preseeded","omitted":"/preseeded"}`）。同一 JS
全局环境内，`C3` 的不同物理包副本用例确实有效（第二次认领会冲突），关闭了 round-1 原始复现，但没有
兑现更强的「进程级」表述。

### HIGH — A2 的 SPAWN_ERROR 测试是可假绿的弱断言

`r.error` 和「无 `error`、无 `signal`、`status=null`」两个场景最终都只断言 `cli/guard/lint` 返回
`64/2/0`。如果删除 `if (r.error) { return degrade(...SPAWN_ERROR...); }`，`r.error` 桩会继续落到
`NO_STATUS` 分支，仍返回完全相同的 `64/2/0`，所以所谓 `SPAWN_ERROR` 用例照绿。这正是 round-1 要
消除的「删掉对应生产分支测试仍过」。A2 其余分支复核结果：信号测试确实经过 `boot.runCli`；
`status===null` 至少对 `cli` 能钉住对应降级；目标模块 import 抛错确实驱动目标模块求值错误；认领
API 抛错通过哨兵副作用证明发生在目标模块求值前；`spawnImpl` 未出现在十个生产 shim 展开结果中；
并发用例确实同时发起两次 import。

### MED — A6 没有补承诺的特殊字符路径回归测试

生产修复本身正确：两处 import 均改为 `pathToFileURL(join(...)).href`，可正确编码空格、`#`、`%`。
但 round-1 明确要求「补空格/`#`/`%` 路径用例」，当前 golden 中没有任何把包复制到这些路径并调用
`loadLib` 的测试。这意味着未来退回 `file://${dir}/` 字符串拼接时，冻结测试仍可能全绿。

### LOW — 新增 C4 用例遗留外置临时锁文件

测试把锁写到 `${dir}.lock.json`，即测试目录之外；`finally` 只删除 `dir` 和 wrapper，没有删除锁
文件。每次完整 C4 运行会在临时目录遗留多份 `loop-kit-c4-*.lock.json`。

## 3. round-2 处置表（本轮新增修改，按发现编号）

| 发现 | 处置 | 涉及文件 |
|---|---|---|
| A1（worker_threads 边界） | `resolveRoot()` 顶部检测 `isMainThread`（`node:worker_threads`），非主线程直接抛 `RootResolutionError` 拒绝；grep 核验本仓与 casey 消费侧零 `worker_threads` 使用（不构成本系统可达故障域，但仍加guard 防止「看似成立、实则静默违反」） | `loop-kit/lib/root.mjs`（包仓+期望存档同步） |
| A1（槽内容防篡改） | 首次认领后 `Object.defineProperty(globalThis, CLAIM_KEY, {writable:false, configurable:false})` 冻结；幂等复用路径新增 `revalidateClaimed()`——每次复用前重跑与首次认领相同的校验（存在性+根标记+`realpath`），失败立即抛错，不再对槽内容照单全收 | 同上 |
| A2（SPAWN_ERROR 弱断言） | 新增 `assertDegradesAllKindsWithReason` 辅助函数：额外捕获 `degrade()` 打到 `console.log`/`console.error` 的诊断文本，用互斥的 `reasonPattern` 正则区分 `SPAWN_ERROR`（"`SPAWN_ERROR`\|子进程派生失败"）与 `NO_STATUS`（"`NO_STATUS`\|`status=null` 且无信号无 `error`"），退出码与判据文本双重锁定 | `tests/_golden/loop-kit-extract.golden.mjs` |
| A6（缺失回归测试） | 新增 C4 用例：包放在同时含空格、`#`、`%` 的物理路径，驱动 `boot.loadLib` 真实 `import root.mjs` 与目标模块，断言均成功 | 同上 |
| LOW（孤儿锁文件） | 新增 `rmrfWithLock(dir)` 辅助函数（`rmrf(dir)` + `unlinkSync` 外置锁文件），9 处调用点统一改用 | 同上 |

## 4. round-2 处置的独立验证证据（mutation 测试，非转录，本次驱动员亲验）

- **A1 worker_threads**：两个 `Worker` 各自 `import` 真包 `root.mjs` 并各自 `resolveRoot`——修复后
  非主线程调用立即抛 `resolveRoot() 仅支持 Node 主线程...` 结构化错误，不再各自静默认领。
- **A1 槽内容防篡改**：`globalThis[Symbol.for('loop-kit:root:claimed')] = '/preseeded'` 后
  `resolveRoot({envRoot:''})`：pre-fix 返回 `/preseeded`（未校验）；post-fix 抛
  `RootResolutionError: 进程级认领槽（幂等复用前复核） 指向的目录不存在：/preseeded`。首次合法认领
  后尝试外部改写槽值：抛 `TypeError: Cannot assign to read only property...`，认领值不变。
- **A2 mutation 测试**：把 `boot.mjs` 复制一份并删除 `if (r.error) {...SPAWN_ERROR...}` 分支——旧版
  断言（只查退出码）：mutation 后退出码仍是 64/2/0，测试**照绿**（复现 codex 描述的假绿）；新版
  断言（退出码+判据文本）：`reasonMatches:false`（文本变成 "`NO_STATUS`"），测试**正确转红**。
- **A6 mutation 测试**：把 `boot.mjs` 的 `loadLib` 改回 `new URL('lib/root.mjs', \`file://${dir}/\`)`
  字符串拼接——针对空格/`#`/`%` 混合路径调用新增的回归测试：`{"succeeded":false,"error":"Cannot find
  module '/tmp/lib/root.mjs' imported from ..."}`（`#` 后内容被截断，正确转红）。
- **LOW 孤儿锁文件**：清空既有孤儿文件后连续跑两次完整金牌，`/tmp` 下 `loop-kit-c4-*.lock.json`
  计数保持为 0（不再增长）。
...
loop-kit-extract golden: 71/71 GREEN
...
上述 `ratchet verify` 的 2 项 `FILE_MISSING` 是本契约 prd `observability` 已记档的既有缺口（真机
运行产物，`cases/` 按 `.gitignore` 不入库，早于本契约、与本次改动无关），改动前后同为这 2 项，不属
本轮修复范围。

## 6. Diff（Casey worktree，从提取切换收口 `31e71de` 到本轮 HEAD 的累积 diff，含 round-1+round-2 全部修复）

```diff
diff --git a/loop-kit/kit-lock.json b/loop-kit/kit-lock.json
index 83ff2ce..f6a19aa 100644
...
+    "lib/root.mjs": "2d11f3fc56c6c1a11d3607cece284d17727c5965f7d65ebece6b31ef01da89d7",
...
+    "loop-kit/kit-lock.json": "78b4637473ebb3a2e8fa52ead643390a39e45201bb0c6bd09331f065f2d07c63",
...
+    "tests/_golden/loop-kit-extract.golden.mjs": "b90164b13b4e6a90fef3c39f047eea45580cefbc59aeda945cb91579251df95b",
...
+    "tests/fixtures/loop-kit-expected/package/lib/root.mjs": "2d11f3fc56c6c1a11d3607cece284d17727c5965f7d65ebece6b31ef01da89d7",
...
+      "evidence": "gate@2026-07-13T16:44:42.834Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T16:44:55.193Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T16:45:10.052Z 全部 acceptance exit 0"
     },
     {
...
+      "evidence": "gate@2026-07-13T16:45:51.224Z 全部 acceptance exit 0"
     },
     {
...
@@ -603,102 +1003,219 @@ await check('C7', 'env 有效命中：realpath 规范化后返回', async () =>
   }
 });
...
+import { isMainThread } from 'node:worker_threads';
 ...
 export class RootResolutionError extends Error {
   constructor(message) {
@@ -22,7 +52,26 @@ export class RootResolutionError extends Error {
   }
 }
...
@@ -43,6 +92,18 @@ function validate(dir, sourceLabel) {
   }
 }
...
+
 function upwardSearch(startCwd) {
   ...
   for (;;) {
@@ -60,25 +121,33 @@ function upwardSearch(startCwd) {
 }
...
@@ -86,7 +155,8 @@ export function resolveRoot({ envRoot = process.env.LOOP_KIT_ROOT, cwd = process
...
## 7. Diff（包仓 /mnt/d/ctx/heren/loop-kit，从首提交 0f34cc0 到本轮 HEAD 的累积 diff）
