export const meta = {
  name: 'bc-contracts-v2',
  description: 'B 域锁硬化 + C gen-prompts 两契约并行落地（v2：plan 设计审异构硬门无兜底；新 loop 引擎下恢复）：fable 规划 → codex 设计审 → fable 裁决 → sonnet ultracode 实现(内嵌 codex test 分析) → codex+pi 双路评审 → fable 汇裁 → 修复 → learn',
  phases: [
...
const results = await pipeline(
  ...
  (acc) => {
    ...
    // 异构硬门（Steven 2026-07-13 定）：plan/设计评审必须真 codex 出结论，同族兜底不算过门、直接停本契约。
    if (!planReview.crossFamily || String(planReview.verdict).startsWith('FALLBACK_SAME_FAMILY')) {
      log(c.slug + '：设计审未达异构硬门（须 codex 本尊、无兜底），blocked——本契约后续段全停，主会话重试')
      return null
    }
    const clean = planReview.verdict === 'PASS' && (!planReview.findings || planReview.findings.length === 0)
    if (clean) {
