...
- **消费标记只看在场、不解析内容**（实现审 M2 确认方向未反）：`wx` 建档与写内容之间有窗口，
  解析失败会把「已占用」误读成「未占用」——那是 fail-open。只看在场是设计内的 fail-closed：
  宁可把「刚建好还没写完」当已占用而多拒一次，也不肯把它当没占用而多放一次。
- **会话未落定单列拒因**（实现审 M1）：`REPLAY_GRANT_SESSION_NOT_SETTLED` 与
  `REPLAY_GRANT_BATCH_SESSION_MISMATCH` 分开——把「写者还没写完」报成「这是另一批的重放」
  会让人查错方向。胜者写完会话即 `fsync` 再放行，避免「放行了但台账上没这条」。
