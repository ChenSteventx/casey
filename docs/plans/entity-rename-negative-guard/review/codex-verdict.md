...
## 核心安全属性成立（hermetic，多层 fail-closed）

未知/不可编译原子在回放绝不执行破坏动作、绝不产 PASS：非字符串→preflight `REPLAY_EVENT_SHAPE_INVALID` exit 65；未知 string→C4 `UNKNOWN_ATOM` reject 不碰 page；reject 轴→verdict `NEEDS_HUMAN`。三层各自 fail-closed。
...
- 核心 fail-safe 属性：**成立**（未知原子回放不产 PASS、不执行破坏，多层 fail-closed，亲验）。
