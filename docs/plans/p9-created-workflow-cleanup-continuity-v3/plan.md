...
运行时 observation 至少绑定：`runId`、本批 `uniqueNameToken`、完整实体名、字符串形态
...
1. 从请求 URL/path/query 与结构化 body 中按已签 adapter 提取目标 ID；
2. 可提取位置必须符合闭集，恰有一个有效目标 ID；URL 与 body 同时携带时必须彼此相等；
...
Tier2 每个 fresh batch 必须生成新的 `uniqueNameToken`，记录其安全摘要与批次 lineage，并确定性派生
三例互不冲突的实体名。不得使用 replay 默认固定 `r1`，不得沿用上批 token，也不得用时间戳散文
替代结构化 lineage。compile 三次与最终 replay batch 分属不同生命周期，各自使用新 token。
...
1. `tests/_golden/p9-created-workflow-continuity-v3.created-in-run.zero-sut.golden.mjs`
   ...
   - mutation URL/body 逐字 ID guard 在放行前执行，错 ID 时证明发送次数为零。
