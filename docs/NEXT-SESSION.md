...
2. `docs/HANDOFF.md`（最新覆盖层即权威现状，顶节是 2026-08-08 凌晨）
...
- 2026-08-06/08 十四契约入 dev（本地零 push）：sleep 导入修 `8d0b0bc`、create 入口锚
  `158829d`、登录预算 `9bb2300`、post-nav 双锚 `db80a17`、open 搜索先行 `7f38606`、
  open 诊断 notes `97c0d83`、语义名回填 `8d0e6e3`、读回门丙路线 `a507f5c`、
  删除搜索修 `b6880e3`、观察让位 `d5ecbab`、意图号重绑 `b38c679`、基数门让位 `28e02a2`、准入让位 `87d86c1`（现役顶端）。
...
【下一步（任选其一，先对齐再动手）】
甲. 立契约修 sign 准入终端语义缝（详 HANDOFF 顶节：准入验证器逐 click 当终端 vs
   基数门「组内末 click 终端」先例；建议对齐后者）；修通重跑 sign（Steven 已签呈件
   与确认件不变）→ 续 replay 链（权威铸新 + replay grant，门拒即停）。
