...
**真机账户守卫（Steven 2026-08-10 定，最高优先级）**：本机同时跑 casey 与死亡报卡两个
项目，真机测试账户不同——**casey 是 `autotest`，死亡报卡是 `hxz`**。凭据文件位置与
环境变量是共用面，用错账户就是往别人的账户里真建真删。**每次真机操作之前都必须先跑
`node scripts/assert-sut-account.mjs autotest`，退出码非 0 一律禁止继续**（0 匹配、
3 不匹配、65 判不出）。该脚本只比对不回显，守凭据红线。
不许「本会话前面确认过」就跳过——2026-08-10 当日就是只在第一次口头确认、后续两次回放
与一次探针全没再确认（事后核为 autotest，属侥幸不属机制）。
...
casey 用**项目级** mail-loop skill（`.claude/skills/mail-loop/SKILL.md`，含在案收件与
核验地址、看门狗 `mail-watch.sh`，钩子已注册进项目 `.claude/settings.json`），项目级
覆盖用户级，本节不再重复地址。三条铁律（2026-07-30 两条 + 2026-08-10 一条）：他的每
封信必回；卡在他身上的事必发信；**没有单向信**——完工/进度通知发完也必须值守回信，
回信读不懂就追问到完全明白才动手（不设次数上限，有回信不走代签）。
