...
6. 直接下一步：① 相6 接上——`casey run` 编排器串 相3→4→装配→报告 出自包含 HTML（但其 replay 调用未透传 `--login-bootstrap`，真机跑需小接线）；② `casey draft` CLI 命令化（函数层已验）；③ G-seam 推定待 Steven 复核（prd observability）；④ 未实现 kind 补实现（`noErrorToast`/`textVisible` 优先，照缺席/存在范式）+ 重签提硬。
