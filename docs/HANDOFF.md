...
8. 直接下一步：① 报告保真度契约（上条两项）；② `casey draft` CLI 命令化（函数层已验）；③ G-seam 推定待 Steven 复核（prd observability）；④ 未实现 kind 补实现（`noErrorToast`/`textVisible` 优先，照缺席/存在范式）+ 重签提硬；⑤ `casey run` 接 `run-history`/`run-metrics` 真产出（旧挂账，编排器是天然生产者）。
