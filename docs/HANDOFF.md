...
9. 直接下一步：① `casey draft` CLI 命令化（函数层已验）；② G-seam 推定待 Steven 复核（prd observability）；③ 未实现 kind 补实现（`noErrorToast`/`textVisible` 优先，照缺席/存在范式）+ 重签提硬；④ `casey run` 接 `run-history`/`run-metrics` 真产出（旧挂账，编排器是天然生产者）；⑤ 第二条用例移植（`chiefcomplaint_smoke`，飞轮排期表既定）。
