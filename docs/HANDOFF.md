...
10. 直接下一步：① `casey draft` CLI 命令化（`draft-cli` light 契约进行中：golden 9 检查 gate GREEN、codex R1 四发现已处置——三采信一修正采纳（凭据门退出码取 compile 先例 1）、R2 复评中）；② 未实现 kind 补实现（`noErrorToast`/`textVisible` 优先）+ 重签提硬；③ `casey run` 接 `run-history`/`run-metrics` 真产出；④ 第二条用例移植（`chiefcomplaint_smoke`）。
