...
>
> **目标态、尚未实现（2026-07-08 补记）**：§3 的「测试用例自然语言块 / 原子操作块 / 录像置于裁定概览前 /
> 清理证据（删除前后命中数）/ 工作流画布结构有效 / 变量来源默认自定义变量」及其 `naturalLanguage` /
> `atomicSteps` JSON 字段，`report-model` 与 `renderReport` 均未落地、无 golden——目标规格、非现状。
> P7 实现契约挂账（源：codex 加入需求，Claude 异构核为文实不符记账）。实现前报告不得据这些未落地项判过。
...
报告生成器的硬要求（目标态——下列多数尚未在 `report-model` / `renderReport` 落地，P7 实现契约挂账，见顶部对账表；实现前报告不得据未落地项判过）：
