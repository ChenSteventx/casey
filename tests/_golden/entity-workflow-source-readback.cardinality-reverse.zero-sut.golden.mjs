#!/usr/bin/env node
// entity-workflow-source-readback（C2）round-3 反向基数金牌：封 codex round-2 High-1「泛化引入新反向基数 fail-open」。
// 直驱【真实】lib/entity-observation-registry.mjs 的 checkIdentityObservationCardinality（bin/compile.mjs:376 成品段
// 基数门的真实消费者）——零浏览器、零网络、零 SUT、零 seam-mock、零源码 grep。断言纪律：退出码 + 具名断言。
//
// ── 背景（codex round-2 亲验的三案 fail-open）──
//   round-2 的基数门只验单向「观察→某 click」（evidenceStepId ∈ click stepId 集、互不重复），丢了反向与 atom
//   对应，令三案错返 ok:true：①有身份定向 click 却 0 观察（静默产无观察件）；②两身份 click 只一观察（漏一读回）；
//   ③观察锚到别原子的 click（atom 张冠李戴）。round-3 改双向双射 + atom 对应后三案全 fail-closed。
//
// ── 与冻结 wiring 金牌（H1a–H1f）互补 ──
//   wiring 金牌 H1 段只覆盖正向（观察→click 存在/重复/缺失），未覆盖反向基数与 atom 对应（覆盖不足）。本金牌
//   新增反向三案 + 越供 + 未登记锚定，并复证 agent happy path 零回归、workflow 多 click 单终端不误杀（H1b 同形）。
//
// ── 严等原 agent 门（不松不紧）复核 ──
//   agent.searchOpen 每步一 intent（fill/press/click 同 intent，唯 click 终端）→ 每 intent 恰一终端一观察，与旧
//   「agent.searchOpen click 数 == 观察数」逐字等价；workflow.create 单步一 intent 多 click（新增/分类/确认同
//   iCreate，唯确认武装）→ 该 intent 多 click 收敛单终端恰一观察，不误杀。intent 分组即两者统一判据。
...
// ════════════════ 反向：codex round-2 亲验三案，round-3 后须 fail-closed ════════════════
{
  ...
  // ①b 身份定向 click 缺 intentId → 无从按 intent 收敛终端，fail-closed（codex 复现命令即无 intentId 形态）。
...
// ════════════════ compile.mjs:376 真实调用形态：零观察但有终端 click → exit 65（不产无观察件）════════════════
{
  // 镜像 bin/compile.mjs:376 的调用形态 { events: run.events, observations: run.identityObservations }：
  //   一个 workflow.open 终端 click 已 emit（unique），但 identityObservations 停在空（读回被丢）→ 基数门须拒，
  //   compile 据此 exit 65（bin/compile.mjs:377 !card.ok），绝不静默降级出可按 v1 签署的无观察编译件。
  ...
  assert(c.ok === false,
    `RC-4 compile:376 形态·终端 click + 零观察 → fail-closed（compile 据此 exit 65，不静默产无观察件）；实得 ${brief(c)}`);
}

// ════════════════ 越供 / 未登记锚定 ════════════════
{
  // 越供：同一终端 intent（ic）两 click 各锚一观察 → 终端义务被 2 观察覆盖（need=1）→ 拒。
  const over = card({ events: [
    ...
    { atom: 'workflow.create', evidenceStepId: 's1' },
    { atom: 'workflow.create', evidenceStepId: 's2' },
  ] });
  assert(over.ok === false && over.reason === 'OBSERVATION_TERMINAL_WITHOUT_MATCHING_OBSERVATION',
    `RC-5 单终端 intent 两观察越供 → fail-closed；实得 ${brief(over)}`);
...
// ════════════════ 正向零回归：agent happy path + workflow 多 click 单终端不误杀 ════════════════
{
  ...
  // 两独立 agent 步（各 intent）各一观察 → ok（反向基数逐 intent 满足）。
  ...
  assert(twoAgents.ok === true,
    `RC-9 两独立 agent 步各一观察 → ok（反向基数逐 intent 满足）；实得 ${brief(twoAgents)}`);

  // workflow.create 单步多 click（新增/描述/确认同 iCreate，唯确认武装）+ 1 观察 → ok（多 click 收敛单终端，不误杀）。
  // 与真实 compileWorkflowCreate 事件形态同构（同 intent 多 workflow.create click），复证 H1b 反向不翻误杀。
  ...
  assert(wfCreateOk.ok === true,
    `RC-10 workflow.create 单步多 click 单观察（收敛单终端）→ ok（反向不误杀多 click 流）；实得 ${brief(wfCreateOk)}`);
...
console.log(`ok   entity-workflow-source-readback.cardinality-reverse: ${passed}/${passed} 全过（真实基数门直驱：反向双射 + atom 对应 fail-closed，agent 零回归 + workflow 多 click 不误杀，零 SUT）`);
