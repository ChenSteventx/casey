...
await check('C5 确认执行在点击前一次性消费授权', () => {
  ...
  const consume = confirm.indexOf('consumePendingDeleteAuthorization(page)');
  const click = confirm.indexOf('performLockedAction(page');
  return consume >= 0 && click > consume;
});
...
await check('C6 一次性授权行为：首次消费后第二次为空', () => {
  const store = new WeakMap();
  const page = {};
  const pending = { targetName: 'atl_r2' };
  store.set(page, pending);
  return mod.consumePendingDeleteAuthorization(page, store) === pending
    && mod.consumePendingDeleteAuthorization(page, store) === null
    && store.has(page) === false;
});

await check('C6 授权目标不符或确认文案非法时拒绝', () => (
  mod.validateDeleteConfirmationAuthorization({ targetName: 'atl_r2' }, '确定', 'atl_r2') === true
    && mod.validateDeleteConfirmationAuthorization({ targetName: 'atl_r2' }, '确定', 'atl_r1') === false
    && mod.validateDeleteConfirmationAuthorization({ targetName: 'atl_r2' }, '取消', 'atl_r2') === false
));
